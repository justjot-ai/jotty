...
async def main():
    ...
    result = await coordinator.execute_parallel(
        swarms=swarms, task=task, merge_strategy=MergeStrategy.VOTING  # Majority wins
    )
    ...
    result = await coordinator.execute_parallel(
        swarms=swarms, task=task, merge_strategy=MergeStrategy.CONCATENATE  # Show all
    )
    ...
    result = await coordinator.execute_parallel(
        swarms=swarms, task=task, merge_strategy=MergeStrategy.BEST_OF_N  # Highest confidence wins
    )
