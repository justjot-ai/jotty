...
async def main():
    ...
    if pdf_path.exists():
        ...
        print("🚀 Generating comprehensive course...")
        ...
        if hasattr(result, 'pdf_path') and result.pdf_path and Path(result.pdf_path).exists():
            ...
            print(f"\n✅ Using Olympiad-generated PDF: {result.pdf_path}")
        else:
            print("\n❌ No PDF generated")
            return
    ...
    telegram_result = await channel_manager.send_to_telegram(
        ...
        caption="""📚 <b>Multi-Agent Systems Course</b>
...
Generated by Jotty Olympiad Learning Swarm""",
    ...
    if telegram_result.success:
        print(f"✅ Telegram: Message ID {telegram_result.message_id}\n")
    else:
        print(f"❌ Telegram failed: {telegram_result.error}\n")

    # Send to WhatsApp #my-notes
    print("📤 Sending to WhatsApp #my-notes...")
    print("   • Auto-starts WhatsApp Web client")
    print("   • Loads saved session")
    ...
    if not started:
        print("❌ WhatsApp client failed to start")
        ...
        for i in range(60):
            ...
            if client.qr_code and i == 5:
                print("\n📱 Scan QR code with WhatsApp (shown above)\n")
        else:
...
26,717 words | 53 pages"""
                ...
                if result.get('success'):
                    print(f"✅ WhatsApp: Message ID {result.get('message_id')}\n")
                else:
