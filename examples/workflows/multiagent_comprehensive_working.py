...
async def main():
    ...
    if hasattr(result, 'pdf_path') and result.pdf_path and Path(result.pdf_path).exists():
        ...
        print(f"\n✅ Using Olympiad-generated PDF:")
        print(f"   Path: {result.pdf_path}")
        print(f"   Size: {pdf_size_kb:.1f} KB")
    else:
        print("\n⚠️  No PDF generated by olympiad swarm")
        file_to_send = None
        file_format = "None"
    ...
    if not file_to_send:
        print("❌ No file to send - check olympiad swarm configuration")
        return
    ...
    telegram_result = await channel_manager.send_to_telegram(
        ...
        caption=f"""📚 <b>Multi-Agent Systems Course</b>
...
Generated by Jotty Olympiad Learning Swarm""",
    ...
    if whatsapp_to:
        print(f"\n📤 Sending to WhatsApp: {whatsapp_to}")

        whatsapp_result = await channel_manager.send_to_whatsapp(
            to=whatsapp_to,
            file_path=file_to_send,
            caption=f"""📚 *Multi-Agent Systems Course*
...
Generated by Jotty Olympiad Learning Swarm""",
        ...
        if whatsapp_result.success:
            print(f"✅ WhatsApp: Message ID {whatsapp_result.message_id}")
        else:
