...
def test_api_contract() -> bool:
    ...
    checks = {
        "Request schemas": "ChatExecuteRequest" in str(spec.get('components', {}).get('schemas', {})),
        "Response schemas": "ChatExecuteResponse" in str(spec.get('components', {}).get('schemas', {})),
        "Authentication": "BearerAuth" in str(spec.get('components', {}).get('securitySchemes', {})),
        "Error handling": "ErrorResponse" in str(spec.get('components', {}).get('schemas', {})),
        "Examples": any("example" in str(schema) for schema in spec.get('components', {}).get('schemas', {}).values())
    }
...
def main():
    ...
    print(f"{'✅' if results['structure'] else '⚠️ '} SDK Structure: {'Pass' if results['structure'] else 'Issues'}")
    print(f"{'✅' if results['contract'] else '⚠️ '} API Contract: {'Well-defined' if results['contract'] else 'Needs work'}")
    print(f"{'✅' if results['mock'] else '❌'} Mock Tests: {'Passed' if results['mock'] else 'Failed'}")
