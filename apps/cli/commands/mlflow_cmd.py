...
class MLflowCommand(BaseCommand):
    ...
    async def execute(self, args: ParsedArgs, cli: "JottyCLI") -> CommandResult:
        ...
        if tracker._mlflow is None:
            cli.renderer.error("MLflow not installed. Install with: pip install mlflow")  # type: ignore[attr-defined]
    ...
    async def _list_runs(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        cli.renderer.header(f"MLflow Runs - {experiment}")  # type: ignore[attr-defined]

        runs = await tracker.list_runs(experiment_name=experiment, max_results=max_results)

        if not runs:
            cli.renderer.info("No runs found.")  # type: ignore[attr-defined]
            return CommandResult.ok(data=[])

        # Display runs in a table format
        cli.renderer.info(f"{'Run ID':<36} {'Score':<10} {'Status':<10} {'Start Time':<20}")  # type: ignore[attr-defined]
        cli.renderer.info("-" * 80)  # type: ignore[attr-defined]
        ...
        for run in runs:
            ...
            cli.renderer.info(f"{run_id:<36} {str(score):<10} {status:<10} {str(start_time):<20}")  # type: ignore[attr-defined]
    ...
    async def _get_best_run(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        cli.renderer.header(f"Best Run - {experiment}")  # type: ignore[attr-defined]
        ...
        cli.renderer.info(f"Run ID: {run_id}")  # type: ignore[attr-defined]
        cli.renderer.info(f"Score ({metric}): {best.get(f'metrics.{metric}', 'N/A')}")  # type: ignore[attr-defined]
        cli.renderer.info(f"Status: {best.get('status', 'N/A')}")  # type: ignore[attr-defined]

        # Show all metrics
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.subheader("Metrics")  # type: ignore[attr-defined]
        for key, value in best.items():
            if key.startswith("metrics."):
                metric_name = key.replace("metrics.", "")
                if isinstance(value, float):
                    cli.renderer.info(f"  {metric_name}: {value:.4f}")  # type: ignore[attr-defined]
                else:
                    cli.renderer.info(f"  {metric_name}: {value}")  # type: ignore[attr-defined]

        # Show params
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.subheader("Parameters")  # type: ignore[attr-defined]
        for key, value in best.items():
            if key.startswith("params."):
                param_name = key.replace("params.", "")
                cli.renderer.info(f"  {param_name}: {value}")  # type: ignore[attr-defined]
    ...
    async def _load_model(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        if len(args.positional) < 2:
            cli.renderer.error("Run ID required. Usage: /mlflow load <run_id>")  # type: ignore[attr-defined]
        ...
        cli.renderer.info(f"Loading model from run: {run_id}")  # type: ignore[attr-defined]

        model = await tracker.load_model(run_id=run_id, model_name=model_name)

        if model is None:
            cli.renderer.error(f"Failed to load model from run {run_id}")  # type: ignore[attr-defined]
            return CommandResult.fail("Model load failed")

        cli.renderer.success("Model loaded successfully!")  # type: ignore[attr-defined]
        cli.renderer.info(f"Type: {type(model).__name__}")  # type: ignore[attr-defined]
    ...
    async def _compare_runs(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        cli.renderer.header("Run Comparison")  # type: ignore[attr-defined]

        comparison = await tracker.compare_runs(run_ids)

        if comparison.empty:
            cli.renderer.info("No comparison data available.")  # type: ignore[attr-defined]
            return CommandResult.ok(data=None)

        # Display comparison
        cli.renderer.info(comparison.to_string())  # type: ignore[attr-defined]
    ...
    async def _launch_ui(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        cli.renderer.info(f"Launching MLflow UI on port {port}...")  # type: ignore[attr-defined]
        cli.renderer.info(f"Tracking URI: {tracking_uri}")  # type: ignore[attr-defined]
        cli.renderer.info(f"Open http://localhost:{port} in your browser")  # type: ignore[attr-defined]
        ...
        try:
            ...
            cli.renderer.success(f"MLflow UI started (PID: {process.pid})")  # type: ignore[attr-defined]
            return CommandResult.ok(data={"pid": process.pid, "port": port})

        except FileNotFoundError:
            cli.renderer.error("MLflow CLI not found. Install with: pip install mlflow")  # type: ignore[attr-defined]
            return CommandResult.fail("MLflow CLI not available")
        except Exception as e:
            cli.renderer.error(f"Failed to launch MLflow UI: {e}")  # type: ignore[attr-defined]
    ...
    async def _list_experiments(
        ...
        cli.renderer.header("MLflow Experiments")  # type: ignore[attr-defined]

        try:
            experiments = tracker._mlflow.search_experiments()

            for exp in experiments:
                cli.renderer.info(f"  {exp.experiment_id}: {exp.name}")  # type: ignore[attr-defined]
                cli.renderer.info(f"     Artifact Location: {exp.artifact_location}")  # type: ignore[attr-defined]
                cli.renderer.info("")  # type: ignore[attr-defined]
            ...
            cli.renderer.error(f"Failed to list experiments: {e}")  # type: ignore[attr-defined]
            return CommandResult.fail(str(e))
    ...
    async def _show_status(self, args: ParsedArgs, cli: "JottyCLI", tracker: Any) -> CommandResult:
        ...
        cli.renderer.header("MLflow Status")  # type: ignore[attr-defined]
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.info(f"Current Experiment: {state.get('experiment_name', 'jotty_ml')}")  # type: ignore[attr-defined]
        cli.renderer.info(f"Last Run ID:        {state.get('last_run_id', 'None')}")  # type: ignore[attr-defined]
        cli.renderer.info(  # type: ignore[attr-defined]
        ...
        # Show tip
        cli.renderer.info("Tip: The experiment name is remembered from your last /ml --mlflow run.")  # type: ignore[attr-defined]
        cli.renderer.info("     Override with: /mlflow list --experiment <name>")  # type: ignore[attr-defined]
    ...
    def _show_help(self, cli: "JottyCLI") -> Any:
        ...
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.info(f"Current experiment: {current_exp}")  # type: ignore[attr-defined]
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.info("Available subcommands:")  # type: ignore[attr-defined]
        cli.renderer.info("  list, runs    - List runs in an experiment")  # type: ignore[attr-defined]
        cli.renderer.info("  best          - Get the best run by metric")  # type: ignore[attr-defined]
        cli.renderer.info("  load <id>     - Load model from a run")  # type: ignore[attr-defined]
        cli.renderer.info("  compare       - Compare multiple runs")  # type: ignore[attr-defined]
        cli.renderer.info("  experiments   - List all experiments")  # type: ignore[attr-defined]
        cli.renderer.info("  status        - Show current MLflow status")  # type: ignore[attr-defined]
        cli.renderer.info("  ui            - Launch MLflow UI")  # type: ignore[attr-defined]
        cli.renderer.info("  help          - Show this help")  # type: ignore[attr-defined]
        cli.renderer.info("")  # type: ignore[attr-defined]
        cli.renderer.info("Options:")  # type: ignore[attr-defined]
        cli.renderer.info("  --experiment  - Experiment name (default: jotty_ml)")  # type: ignore[attr-defined]
        cli.renderer.info("  --metric      - Metric to optimize (default: best_score)")  # type: ignore[attr-defined]
        cli.renderer.info("  --limit       - Max results to show (default: 10)")  # type: ignore[attr-defined]
