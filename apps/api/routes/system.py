...
def register_system_routes(app, api):
    ...
    async def get_capabilities():
        ...
        try:
            ...
            if not widgets:
                all_data["widgets"] = {
                    "available": fallback_widgets,
                    "categories": ["Content", "Visualization", "Data", "Media", "Export"],
                }
            if not tools:
                all_data["tools"] = {
                    "available": fallback_tools,
                    "categories": ["Research", "Files", "Development", "System", "Media", "Export"],
                }
            ...
            return {
                **all_data,
                "defaults": (
                    defaults
                    if defaults.get("widgets") or defaults.get("tools")
                    else {
    ...
    async def execute_swarm(request: SwarmRequest):
        ...
        try:
            ...
            result = await api.process_message(message=enhanced_task, session_id=session_id)
