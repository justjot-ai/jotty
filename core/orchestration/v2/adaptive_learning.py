...
class AdaptiveLearning:
    ...
    def _adjust_learning_rate(self):
        ...
        if self.state.is_plateau:
            ...
            logger.info(f"Plateau detected, increasing learning rate to {self.state.learning_rate:.2f}")
            ...
            logger.debug(f"Accelerating, maintaining learning rate at {self.state.learning_rate:.2f}")
    ...
    def _adjust_exploration_rate(self):
        ...
        if self.state.is_plateau:
            ...
            logger.info(f"Plateau detected, increasing exploration to {self.state.exploration_rate:.2f}")
        
        elif self.state.is_accelerating:
            # Focus on exploitation (use what works)
            self.state.exploration_rate = max(0.1, self.state.exploration_rate - 0.1)
            logger.debug(f"Accelerating, decreasing exploration to {self.state.exploration_rate:.2f}")
    ...
    def should_stop_early(self, min_iterations: int = 3) -> bool:
        ...
        if (self.state.is_converging and 
            len(self.state.score_history) > 0 and
            self.state.score_history[-1] >= 0.95):
            return True
