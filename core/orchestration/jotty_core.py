"""
JottyCore v6.0 - Main Orchestrator
=====================================
...
class JottyCore:
    ...
    Usage:
        ...
        # Create actor
        actor = dspy.ReAct(MySignature, tools=[...])
    ...
    def __init__(self,
                 ...
                 shared_context: Optional[Dict[str, Any]] = None):  # 🔥 A-TEAM: Accept shared_context
        """
        Initialize JottyCore.
        
        Parameters:
            actor: The DSPy agent to validate
            ...
            actor_config: ActorConfig with enable_architect/enable_auditor flags (optional)
            shared_context: SharedContext for accessing metadata (optional)
        """
        self.actor = actor
        self.config = config or JottyConfig()
        self.agent_config = actor_config  # 🔑 A-TEAM: Store actor_config for validation control
        ...
        logger.info("🏷️  TrajectoryParser initialized in JottyCore")
        
        # 🔧 A-TEAM: Tool interceptor for tracking all tool calls
        actor_name = actor_config.name if actor_config else actor.__class__.__name__
        self.tool_interceptor = ToolInterceptor(actor_name)
        logger.info(f"🔧 [JOTTY] Tool interceptor initialized for '{actor_name}'")
