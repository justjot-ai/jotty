...
# =============================================================================
# LLM-BASED Q-PREDICTOR - SOTA Agentic Value Estimation
# =============================================================================
...
class LLMQPredictorSignature(dspy.Signature):
    ...
    confidence = dspy.OutputField(desc="Confidence in prediction 0.0-1.0")
    suggested_alternative = dspy.OutputField(desc="If Q-value is low, suggest alternative action")


class LLMQPredictor:
    """
    SOTA: LLM-based Q-value predictor instead of static Q-tables.
    
    Benefits over traditional Q-tables:
    - Generalizes to unseen states (no cold start)
    ...
    def __init__(self, config: JottyConfig):
        self.config = config
        self.predictor = dspy.ChainOfThought(LLMQPredictorSignature) if DSPY_AVAILABLE else None
        
        # Experience buffer for few-shot examples
    ...
    def predict_q_value(
        ...
        # Get similar experiences for few-shot
        similar = self._get_similar_experiences(state, action)
        
        try:
            result = self.predictor(
                state_description=json.dumps(state, default=str)[:2000],
                proposed_action=json.dumps(action, default=str)[:500],
                historical_outcomes=json.dumps(similar[:5], default=str)[:1500],
                goal_context=goal[:500]
            )
            
            # A-Team Approved: No regex parsing!
            from ..foundation.robust_parsing import parse_float_robust
            
            q_value = parse_float_robust(result.q_value, default=None)
            if q_value is None:
                logger.warning(f"Could not parse Q-value from: {result.q_value}")
                return None, None, None  # Return None, not magic numbers
            q_value = max(0.0, min(1.0, q_value))
            
            confidence = parse_float_robust(result.confidence, default=0.5)
            confidence = max(0.0, min(1.0, confidence))
            
            # Use adaptive threshold, not hardcoded 0.4
            if not hasattr(self, '_q_threshold'):
                from ..foundation.robust_parsing import AdaptiveThreshold
                self._q_threshold = AdaptiveThreshold(initial_mean=0.5)
            self._q_threshold.update(q_value)
            
            alternative = result.suggested_alternative if self._q_threshold.is_low(q_value) else None
            
            return q_value, confidence, alternative
            
        except Exception as e:
            logger.warning(f"Q-prediction failed: {e}")
            return None, None, None  # Return None, let caller handle
    
    def record_outcome(self, state: Dict, action: Dict, reward: float):
        """Record experience for future predictions."""
    ...
    def _get_similar_experiences(self, state: Dict, action: Dict) -> List[Dict]:
        ...
        # Simple heuristic: recent experiences with same actor
        actor = action.get('actor', '')
        similar = [
            exp for exp in self.experience_buffer
            if exp['action'].get('actor') == actor
        ]
        return sorted(similar, key=lambda x: x['timestamp'], reverse=True)[:10]
...
class SmartContextGuard:
    ...
    def __init__(self, max_tokens: int = 28000, safety_margin: int = 2000):
    ...
    def build_context(self) -> Tuple[str, Dict[str, Any]]:
        ...
        for key, content, tokens in self.buffers[self.HIGH]:
            if total_tokens + tokens < self.usable_tokens:
                ...
                # A-Team Fix: Smart compression instead of arbitrary truncation
                available = self.usable_tokens - total_tokens
                if available > 200:
                    compressed_content = self._smart_compress(content, available * 4)
    ...
    def _smart_compress(self, content: str, max_chars: int) -> str:
        """
        A-Team Enhancement: Structure-aware compression instead of arbitrary truncation.
        
        Preserves:
        - JSON/code structure boundaries
        - First and last parts (context + conclusion)
        - Numbered list items
        
        Removes:
        - Middle verbose content
        - Repeated similar lines
        """
        if len(content) <= max_chars:
            return content
        
        # Strategy 1: If JSON/code, try to keep structure
        if content.strip().startswith('{') or content.strip().startswith('['):
            # Keep opening and closing brackets visible
            try:
                # Try to parse and re-serialize compactly
                import json as json_module
                parsed = json_module.loads(content)
                compact = json_module.dumps(parsed, separators=(',', ':'))
        ...
        # Strategy 2: Keep first + last portions (context + conclusion)
        keep_start = max_chars // 3
        keep_end = max_chars // 4
        middle_budget = max_chars - keep_start - keep_end - 50  # 50 for marker
        
        start_part = content[:keep_start]
        end_part = content[-keep_end:] if keep_end > 0 else ""
        
        # For middle, extract key sentences (first sentence of each paragraph)
        middle_content = content[keep_start:-keep_end] if keep_end > 0 else content[keep_start:]
        paragraphs = middle_content.split('\n\n')
        middle_summary = []
        ...
        for para in paragraphs:
            if remaining <= 0:
                break
            first_sentence = para.split('.')[0][:100] if para else ""
            if first_sentence and len(first_sentence) + 5 < remaining:
                middle_summary.append(first_sentence)
                remaining -= len(first_sentence) + 5
        
        middle_part = "; ".join(middle_summary) if middle_summary else ""
        
        return f"{start_part}\n\n[...{len(middle_content)} chars summarized: {middle_part}...]\n\n{end_part}"
    ...
    def explore(
        ...
        try:
            result = self.explorer(
                current_state=todo.get_state_summary()[:1500],
                failed_actions=json.dumps(failed_info, default=str)[:1000],
                available_actions=json.dumps(available_actions[:10], default=str)[:1000],
                goal=goal[:500]
            )
