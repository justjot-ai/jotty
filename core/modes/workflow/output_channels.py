...
class OutputChannelManager:
    ...
    async def send_to_telegram(
        ...
        if "telegram" not in self.skills:
            return ChannelDeliveryResult(
                channel="telegram", success=False, error="telegram-sender skill not available"
            )
        ...
        try:
            ...
            if file_path:
                ...
                if not send_tool:
                    return ChannelDeliveryResult(
                        channel="telegram", success=False, error="send_telegram_file_tool not found"
                    )
            ...
            # Send message if provided
                ...
                params = {"message": message, "parse_mode": parse_mode}
                if chat_id:
                    params["chat_id"] = chat_id
            ...
            if result.get("success"):
                ...
                return ChannelDeliveryResult(
                    channel="telegram", success=False, error=result.get("error", "Unknown error")
                )

        except Exception as e:
            logger.error(f"Telegram delivery failed: {e}")
            return ChannelDeliveryResult(channel="telegram", success=False, error=str(e))
    ...
    async def send_to_whatsapp(
        ...
        if "whatsapp" not in self.skills:
            return ChannelDeliveryResult(
                channel="whatsapp", success=False, error="whatsapp skill not available"
            )
        ...
        try:
            ...
            # Send message if provided
                ...
                result = send_tool({"to": to, "message": message, "provider": provider})
            ...
            if result.get("success"):
                ...
                return ChannelDeliveryResult(
                    ...
                    success=True,
                    message_id=result.get("message_id"),
                    metadata={"provider": result.get("provider"), "to": to},
                )
            else:
                return ChannelDeliveryResult(
                    channel="whatsapp", success=False, error=result.get("error", "Unknown error")
                )

        except Exception as e:
            logger.error(f"WhatsApp delivery failed: {e}")
            return ChannelDeliveryResult(channel="whatsapp", success=False, error=str(e))
            ...
            if channel_lower == "telegram":
                ...
                if not whatsapp_to:
                    results["whatsapp"] = ChannelDeliveryResult(
                        channel="whatsapp", success=False, error="whatsapp_to parameter required"
                    )
                else:
                ...
                results[channel_lower] = ChannelDeliveryResult(
                    channel=channel_lower, success=False, error=f"Channel not supported: {channel}"
                )
    ...
    def get_summary(self, results: Dict[str, ChannelDeliveryResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "errors": {
                ch: res.error for ch, res in results.items() if not res.success and res.error
            },
        }
