...
class IntentClassifier:
    ...
    def classify(
        ...
        try:
            ...
            logger.info(
                f"Intent classified: {analysis.intent.value} (confidence: {analysis.confidence:.2f})"
            )
            return analysis
    ...
    def _build_classification_prompt(
        self, task: str, attachments: Optional[List[str]], context: Optional[Dict[str, Any]]
    ) -> str:
...
Think carefully about the core intent, not just keywords."""
    ...
    def _parse_classification(
        self, response: str, task: str, attachments: Optional[List[str]]
    ) -> IntentAnalysis:
    ...
    def _auto_detect_tools(
        self, task: str, attachments: Optional[List[str]], intent: TaskIntent
    ) -> List[str]:
        ...
        if any(
            word in task_lower
            for word in ["search", "find", "look up", "what is", "who is", "when was"]
        ):
    ...
    def _heuristic_fallback(self, task: str, attachments: Optional[List[str]]) -> IntentAnalysis:
...
def classify_task_intent(
    task: str, attachments: Optional[List[str]] = None, context: Optional[Dict[str, Any]] = None
) -> IntentAnalysis:
    """
