...
class ChatOrchestrator:
    ...
    def __init__(
        self, conductor: Any, agent_id: Optional[str] = None, mode: str = "dynamic"
    ) -> None:
        """
    ...
    def _select_with_q_predictor(
        self, message: str, history: Optional[List[Any]], context: Optional[Dict[str, Any]]
    ) -> Optional[str]:
        ...
        for actor in self.conductor.actors:
            ...
            action = {"agent": agent_name, "type": "chat"}

            try:
                q_value, confidence, _ = self.conductor.q_predictor.predict_q_value(
                    state=state, action=action, goal=message
                )
    ...
    def prepare_agent_context(
        ...
        if history:
            ...
            agent_context["conversation_history"] = self._format_history(
                history
