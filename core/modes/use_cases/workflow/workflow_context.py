...
class WorkflowContext:
    ...
    def add_task(
        ...
        task = WorkflowTask(
            id=task_id, goal=goal, dependencies=dependencies or [], metadata=metadata or {}
        )
        ...
        if len(self.tasks) > self.max_tasks:
            ...
            completed_tasks = [
                (tid, t) for tid, t in self.tasks.items() if t.status == TaskStatus.COMPLETED
            ]
    ...
    def get_ready_tasks(self) -> List[WorkflowTask]:
        ...
        for task in self.tasks.values():
            if task.status == TaskStatus.PENDING:
                ...
                deps_completed = all(
                    self.tasks.get(
                        dep_id, WorkflowTask(id=dep_id, goal="", status=TaskStatus.COMPLETED)
                    ).status
    ...
    def is_complete(self) -> bool:
        ...
        return all(
            task.status in [TaskStatus.COMPLETED, TaskStatus.FAILED] for task in self.tasks.values()
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkflowContext":
        ...
        context = cls(workflow_id=data.get("workflow_id"), max_tasks=data.get("max_tasks", 100))
