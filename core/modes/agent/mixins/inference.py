...
class InferenceMixin:
    ...
    def infer_task_type(self, task: str) -> Tuple:
        ...
        try:
            ...
            task_type_str = (
                str(result.task_type).lower().strip().split()[0] if result.task_type else "unknown"
            )
            ...
            reasoning = (
                str(result.reasoning).strip()
                if result.reasoning
                else f"Inferred as {task_type_str}"
            )
            ...
            # Comparison keywords
                ...
                w in task_lower
                for w in ["compare", "vs", "versus", "comparison", "difference", "better"]
            ):
                return TaskType.COMPARISON, "Keyword fallback: comparison task", 0.6
            # Research keywords
            elif any(
                w in task_lower
                for w in ["research", "find", "search", "discover", "look up", "what is"]
            ):
            ...
            # Automation keywords
                ...
                w in task_lower for w in ["automate", "schedule", "pipeline", "workflow", "cron"]
            ):
                return TaskType.AUTOMATION, "Keyword fallback: automation task", 0.6
            # Communication keywords
            elif any(
                w in task_lower for w in ["send", "email", "notify", "message", "communicate"]
            ):
        ...
        try:
            ...
            try:
                if capabilities_str.startswith("["):
                    ...
                    capabilities = [
                        c.strip().lower()
                        for c in capabilities_str.replace('"', "")
                        .replace("[", "")
                        .replace("]", "")
                        .split(",")
                    ]
    ...
    async def _call_with_compression_retry(
        self, call_fn: Any, conversation: str, instruction: str, max_retries: int = 3
    ) -> Any:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                is_overflow = any(pat in error_str for pat in self._CONTEXT_OVERFLOW_PATTERNS)
                ...
                current_conversation = self._compress_context(current_conversation, target_len)
