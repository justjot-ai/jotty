...
class UniversalRetryHandler:
    ...
    async def execute_with_retry(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if self.analyzer and attempt < self.max_retries - 1:
                    analysis = self._analyze_failure(task_description, attempts, current_input)
        ...
        if specialist_agent:
            ...
            return await self._call_specialist(
                specialist_agent, task_description, initial_input, attempts
            )
    ...
    def _analyze_failure(self, task: str, attempts: List[Dict], context: Dict) -> Dict[str, Any]:
    ...
    async def _call_specialist(
        self, specialist: Any, task: str, original_input: Dict, error_history: List[Dict]
    ) -> AgentResult:
...
class PatternDetectionSignature(dspy.Signature):
    ...
    patterns_to_detect = dspy.InputField(
        desc="List of patterns to look for (semantic, not keywords)"
    )
    ...
    async def detect(self, text: str, patterns: List[str] = None) -> Dict[str, bool]:
        ...
        try:
            result = self.detector(
                text=text, patterns_to_detect=json.dumps(patterns)  # Limit for context
            )
...
class LLMCounterfactualCritic:
    ...
    async def assign_credit(
        self, trajectory: List[Dict], agents: List[str], final_reward: float
    ) -> Dict[str, float]:
        """
...
class SelfRAGMemoryRetriever:
    ...
    async def retrieve(
        self, task: str, context: str, memories: List[Dict], top_k: int = 5
    ) -> List[Dict]:
        """
        ...
        try:
            decision = self.retrieval_decider(
                current_task=task, current_context=context, available_memories=memory_summary
            )
        ...
        for mem in memories:
            try:
                judgment = self.relevance_judge(
                    task=retrieval_query, memory_content=str(mem.get("content", mem))
                )
                ...
                if is_relevant and score > 0.3:
                    relevant.append(
                        {"memory": mem, "score": score, "how_to_use": judgment.how_to_use}
                    )
...
class LLMSurpriseEstimator:
    ...
    async def estimate_surprise(
        self, event: Dict[str, Any], context: Dict[str, Any], history: List[str]
    ) -> Tuple[int, str, str]:
        """
