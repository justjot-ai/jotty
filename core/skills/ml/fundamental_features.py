...
class FundamentalFeaturesSkill(MLSkill):
    ...
    async def execute(self, X: pd.DataFrame = None, y: Optional[pd.Series] = None, **context: Any) -> SkillResult:
        ...
        try:
            fund_features = await self.get_fundamental_features(symbol, df_prices=df_prices, mode=mode)
    ...
    async def get_fundamental_features(self,
                                        symbol: str,
        ...
        fund_df = self._add_quarterly_features(fund_df, df, quarterly_financials, quarterly_balance, info)
    ...
    def _add_quarterly_features(self, fund_df: pd.DataFrame, df: pd.DataFrame,
    ...
    def _add_price_based_features(self, fund_df: pd.DataFrame, df: pd.DataFrame, info: Dict) -> pd.DataFrame:
        """
    ...
    def _add_derived_features(self, fund_df: pd.DataFrame, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'fund_earnings_yield' in fund_df.columns and 'fund_volatility_20d' in fund_df.columns:
            fund_df['fund_yield_per_risk'] = fund_df['fund_earnings_yield'] / (fund_df['fund_volatility_20d'] + 0.01)
    ...
    async def _add_sector_features(self, fund_df: pd.DataFrame, df: pd.DataFrame,
                                    symbol: str, sector: str) -> pd.DataFrame:
        """
    ...
    def _add_live_features(self, fund_df: pd.DataFrame, df: pd.DataFrame, info: Dict) -> pd.DataFrame:
        """
