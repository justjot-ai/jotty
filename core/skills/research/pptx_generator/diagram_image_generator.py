        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=60)) as response:
                    ...
                    logger.info(f"Generated Mermaid diagram: {cache_path} ({len(image_bytes)} bytes)")
                    return str(cache_path)
...
class DiagramImageGenerator:
    ...
    def _create_concept_map_mermaid(self, spec: Dict[str, Any]) -> str:
        ...
        # Styles
        lines.append("")
        lines.append(f'    style CENTER fill:{COLORS["primary"]},color:{COLORS["text"]},stroke-width:3px')
    ...
    async def generate_architecture_diagram(self, spec: Dict[str, Any], paper_title: str = "") -> Optional[str]:
    ...
    async def generate_flow_diagram(self, spec: Dict[str, Any], paper_title: str = "") -> Optional[str]:
    ...
    def _auto_generate_concept_map_spec(self, paper_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _auto_generate_flow_spec(self, paper_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if len(concepts) >= 3:
            return {
                "nodes": [
                    {"label": c.get("name", f"Step {i+1}")[:20]}
    ...
    async def generate_all_diagrams(
        ...
        visualization_specs: Dict[str, Any]
    ) -> Dict[str, str]:
        """
