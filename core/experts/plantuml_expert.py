...
class PlantUMLExpertAgent(ExpertAgent):
    ...
    def _create_plantuml_agent(improvements: List[Dict[str, Any]] = None):
        ...
        class PlantUMLGenerationSignature(dspy.Signature):
            ...
            diagram_type: str = dspy.InputField(desc="Type of diagram: sequence, class, activity, component, state, usecase, deployment")
            learned_improvements: str = dspy.InputField(desc="Previously learned patterns and improvements (optional)", default="")
            
            output: str = dspy.OutputField(desc="Complete PlantUML diagram code. MUST include @startuml at the start and @enduml at the end. No markdown code fences.")
        ...
        if improvements:
            try:
                ...
                signature_class = inject_improvements_into_signature(PlantUMLGenerationSignature, improvements)
            except ImportError:
    ...
    def _create_plantuml_teacher():
        ...
        class PlantUMLTeacherSignature(dspy.Signature):
            ...
            gold_standard: str = dspy.InputField(desc="The EXACT PlantUML code to return. Copy this EXACTLY without modification.")
            student_output: str = dspy.InputField(desc="What the student generated (IGNORE THIS - just return gold_standard)")
            
            output: str = dspy.OutputField(desc="ONLY the PlantUML code from gold_standard. No explanation. No evaluation. Just the code.")
    ...
    async def _evaluate_plantuml(output: Any, gold_standard: str, task: str, context: Dict) -> Dict[str, Any]:
        ...
        output_normalized = output_content.lower().replace(' ', '').replace('\n', '').replace('\t', '')
        ...
        if output_normalized == gold_normalized and not missing_tags:
            ...
            status = "PARTIAL"
        elif is_valid and (output_normalized in gold_normalized or gold_normalized in output_normalized):
            score = 0.7
            status = "PARTIAL"
        elif is_valid:
            score = 0.3
        ...
        return [
            {
                ...
                "context": {"description": "User and System interaction", "diagram_type": "sequence"},
