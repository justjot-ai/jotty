...
def validate_via_node_cli(
    latex_code: str,
    latex_validation_path: Optional[str] = None
) -> Tuple[Optional[bool], str, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True, "", {
                "validation_method": "latex-validation-cli",
                "code_length": len(latex_code)
            }
        else:
...
def validate_latex_with_openreview(
    ...
    return None, "No LaTeX validator available. Options: 1) Start HTTP server: 'node dist/main.js run-server', 2) Install Node.js and build latex-validation", {}
