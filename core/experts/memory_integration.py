...
def retrieve_improvements_from_memory(
    ...
    # Build query
    query_parts = [f"expert agent {expert_name}", f"domain {domain}", "improvements learned patterns"]
    if task:
        ...
        for pattern_type, entries in pattern_groups.items():
            if len(entries) >= 2:  # Consolidate groups with 2+ similar improvements
                ...
                if patterns:
                    ...
                    logger.info(f"Consolidated {len(entries)} improvements of type '{pattern_type}' into SEMANTIC pattern")
        
        logger.info(f"Consolidation complete: {consolidated_count} patterns consolidated, {merged_count} improvements merged")
        return {
