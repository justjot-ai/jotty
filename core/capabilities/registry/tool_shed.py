...
class ToolShedSchema:
    ...
    def to_prompt_string(self) -> str:
        ...
        output_str = (
            str(self.output_type.__name__)
            if self.output_type and hasattr(self.output_type, "__name__")
            else "Any"
        )
...
if DSPY_AVAILABLE:
    ...
    class ToolSelectionSignature(dspy.Signature):
        ...
        required_output = dspy.InputField(
            desc="What type of output is needed (e.g., 'list of tables', 'SQL query', 'DataFrame')"
        )
        ...
        selected_tools = dspy.OutputField(
            desc="Comma-separated list of tool names to use, in order"
        )
...
class CapabilityIndex:
    ...
    def register_tool(self, schema: ToolShedSchema) -> None:
        ...
        logger.debug(
            f" Registered {schema.name}: produces={schema.producer_of}, consumes={schema.consumer_of}"
        )
...
class ToolShed:
    ...
    def to_prompt_string(self) -> str:
        ...
        return "\n\n".join([schema.to_prompt_string() for schema in self.schemas.values()])
