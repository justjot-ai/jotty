...
class MethodChecker(ast.NodeVisitor):
    ...
    DANGEROUS_ATTRIBUTES = {"__builtins__", "__globals__", "__locals__", "__code__", "__dict__"}
    ...
    def visit_Call(self, node: Any) -> None:
        ...
        if isinstance(node.func, ast.Name):
            if node.func.id in self.DANGEROUS_METHODS:
                self.errors.append(f"Dangerous method call: {node.func.id}()")
    ...
    def visit_ImportFrom(self, node: Any) -> None:
        ...
        if node.module not in self.allowed_imports:
            self.errors.append(f"Unauthorized import from: {node.module}")
        self.generic_visit(node)
...
class ToolValidator:
    ...
    def validate_tool(
        self, tool_func: Callable, tool_metadata: Dict[str, Any]
        ...
        return RegistryValidationResult(valid=len(errors) == 0, errors=errors, warnings=warnings)
    ...
    def _validate_signature(
        ...
        try:
            ...
            if param_names != expected_inputs:
                ...
                if missing:
                    errors.append(f"Signature missing parameters: {missing}")
                if extra:
                    warnings.append(f"Signature has extra parameters: {extra}")
    ...
    def _validate_types(self, tool_metadata: Dict[str, Any]) -> RegistryValidationResult:
        ...
        if output_type not in AUTHORIZED_TYPES:
            errors.append(
                f"Output type '{output_type}' not authorized. " f"Allowed: {AUTHORIZED_TYPES}"
            )
        ...
        if isinstance(inputs, dict):
            for input_name, input_spec in inputs.items():
                ...
                if "type" not in input_spec or "description" not in input_spec:
                    errors.append(f"Input '{input_name}' should have 'type' and 'description' keys")
                    continue
...
class ToolGuard:
    ...
    def __init__(
        self, blocked_path_patterns: Optional[Set[str]] = None, max_destructive_per_session: int = 3
    ) -> None:
