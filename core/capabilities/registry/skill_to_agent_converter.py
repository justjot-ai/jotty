...
class SkillToAgentConverter:
    ...
    async def convert_skill_to_agent(
        self, skill: SkillDefinition, agent_name: Optional[str] = None
    ) -> AgentConfig:
        """
    ...
    async def convert_skills_to_agents(
        self, skills: List[SkillDefinition], agent_name_prefix: Optional[str] = None
    ) -> List[AgentConfig]:
        """
    ...
    def _create_signature_class(self, skill_name: str, analysis: Dict[str, Any]) -> type:
        ...
        try:
            if "->" in signature_str:
                ...
                signature_class = type(
                    f"{analysis['agent_name']}Signature", (dspy.Signature,), signature_dict
                )
        ...
        for part in parts:
            if ":" in part:
                ...
                if field_type.lower() in ["str", "string"]:
                    fields[field_name] = (
                        dspy.InputField(desc=f"{field_name}")
                        if "input" in field_name.lower()
                        else dspy.OutputField(desc=f"{field_name}")
                    )
