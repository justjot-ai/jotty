...
class JottyIntegration:
    ...
    def __init__(self, config: SwarmConfig = None) -> None:
        ...
        # Context management
        self.context_guard = GlobalContextGuard(max_tokens=self.config.max_context_tokens)

        # Document processing
        self.doc_processor = ContentGate(max_tokens=self.config.max_context_tokens)
    ...
    def read_document(
        self, path: str, extraction_goal: str = "Extract key information", future_tasks: list = None
    ) -> str:
        """
        ...
        return self.doc_processor.process(
            content, extraction_goal=extraction_goal, future_tasks=future_tasks
        )
    ...
    async def assign_credit(
        ...
        for agent in agents:
            shapley = (
                shapley_credits.get(agent).combined_credit if agent in shapley_credits else 0.0
            )
    ...
    async def should_store_memory(self, event: dict, expected: str, history: list) -> tuple:
        """
        ...
        bits, level, reason = self.surprise_estimator.estimate_information_content(
            event=event, expected=expected, history=history
        )
    ...
    async def retrieve_memories(
        self, task: str, uncertainties: list, candidates: list, top_k: int = 5
    ) -> list:
        """
        Retrieve memories using mutual information maximization.
        """
        selected_ids, reduction, reason = self.mi_retriever.retrieve(
            task=task, uncertainties=uncertainties, candidates=candidates, top_k=top_k
        )

        logger.debug(
            f"Retrieved {len(selected_ids)} memories (uncertainty reduction: {reduction:.2f})"
        )
