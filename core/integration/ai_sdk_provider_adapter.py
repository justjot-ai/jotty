...
class AISDKProviderLM(BaseLM):
    ...
    def _call_ai_sdk(self, messages: List[Dict[str, str]], stream: bool = False) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                url, json=payload, headers=headers, stream=stream, timeout=LLM_TIMEOUT_SECONDS
            )
            ...
            if stream:
                ...
                if self.provider == "opencode":
                    return {"text": data.get("content", ""), "usage": data.get("usage", {})}
                else:
                    return {
    ...
    def _handle_streaming_response(self, response: Any) -> Dict[str, Any]:
            ...
            if line:
                ...
                if line_str.startswith("data:"):
                    ...
                    try:
                        ...
                        if data.get("type") == "text-delta" and data.get("data", {}).get(
                            "textDelta"
                        ):
        ...
        return {"text": "".join(text_parts), "usage": {}}
    ...
    def __call__(self, prompt: Any = None, messages: Any = None, **kwargs: Any) -> Any:
        ...
        # Store in history
        self.history.append(
            {"prompt": prompt or formatted_messages, "response": result["text"], "kwargs": kwargs}
        )
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        return {
            ...
            "usage": result.get(
                "usage", {"total_tokens": len(result["text"].split())}  # Rough estimate
            ),
        }
