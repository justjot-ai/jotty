...
class ReportMixin:
    def init_report(self, config: "ReportConfig" = None) -> None:
        ...
        try:
            ...
            logger.info(
                f"Report generation initialized: output_dir={self._report_config.output_dir}"
            )
    ...
    def add_model_benchmarking(self, model_scores: Dict[str, Dict[str, float]]) -> None:
        ...
        section = {
            ...
            "content": {
                "models": model_scores,
                "best_model": (
                    max(model_scores.items(), key=lambda x: x[1].get("cv_score", 0))[0]
                    if model_scores
                    else None
                ),
            },
    ...
    def add_baseline_comparison(
        self, baseline_score: float, final_score: float, baseline_model: str = "DummyClassifier"
    ) -> Any:
    ...
    def _build_report_story(self) -> List:
        ...
        # Title page
        story.append(
            self._reportlab["Paragraph"]("Machine Learning Analysis Report", styles["Title"])
        )
        story.append(self._reportlab["Spacer"](1, 12))
        story.append(
            self._reportlab["Paragraph"](
                f"Generated by Jotty SwarmML Comprehensive v{self.version}", styles["Normal"]
            )
        )
        story.append(
            self._reportlab["Paragraph"](
        ...
        for fig_path in self._report_data["figures"]:
            if os.path.exists(fig_path):
                try:
                    img = self._reportlab["Image"](
                        fig_path,
                        width=6 * self._reportlab["inch"],
    ...
    def _render_section(self, section: Dict, styles: Any) -> List:
        ...
        # Section title
        elements.append(self._reportlab["Paragraph"](section["title"], styles["Heading1"]))
    ...
    def _render_executive_summary(self, content: Dict, styles: Any) -> List:
        ...
        if content.get("context"):
            elements.append(
                self._reportlab["Paragraph"](
                    f"<b>Context:</b> {content['context']}", styles["Normal"]
                )
            )

        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Final Score:</b> {content.get('final_score', 0):.4f}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Best Model:</b> {content.get('best_model', 'N/A')}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Features Used:</b> {content.get('n_features_used', 0)}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Iterations:</b> {content.get('iterations', 0)}", styles["Normal"]
            )
        )
    ...
    def _render_data_profile(self, content: Dict, styles: Any) -> List:
        ...
        if content.get("missing"):
            ...
            if any(v > 0 for v in missing.values()):
                elements.append(
                    self._reportlab["Paragraph"]("<b>Missing Values:</b>", styles["Normal"])
                )
                for col, count in list(missing.items())[:10]:
                    if count > 0:
                        elements.append(
                            self._reportlab["Paragraph"](f"  • {col}: {count}", styles["Normal"])
                        )

        if content.get("recommendations"):
            elements.append(self._reportlab["Spacer"](1, 6))
            elements.append(
                self._reportlab["Paragraph"]("<b>EDA Recommendations:</b>", styles["Normal"])
            )
    ...
    def _render_feature_importance(self, content: Dict, styles: Any) -> List:
        ...
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Total Features:</b> {content.get('total_features', 0)}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Top {content.get('top_n_shown', 20)} Features:</b>", styles["Normal"]
            )
        )
    ...
    def _render_confusion_matrix(self, content: Dict, styles: Any) -> List:
        ...
        if report:
            elements.append(
                self._reportlab["Paragraph"]("<b>Classification Metrics:</b>", styles["Normal"])
            )
    ...
    def _render_roc_analysis(self, content: Dict, styles: Any) -> List:
        ...
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>AUC Score:</b> {content.get('auc', 0):.4f}", styles["Normal"]
            )
        )
    ...
    def _render_baseline_comparison(self, content: Dict, styles: Any) -> List:
        ...
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Baseline Model:</b> {content.get('baseline_model', 'N/A')}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
                f"<b>Baseline Score:</b> {content.get('baseline_score', 0):.4f}", styles["Normal"]
            )
        )
        elements.append(
            self._reportlab["Paragraph"](
    ...
    def _create_importance_figure(self, sorted_importance: List[Tuple[str, float]]) -> Any:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                delete=False, suffix=".png", dir=self._report_config.output_dir
            ) as f:
