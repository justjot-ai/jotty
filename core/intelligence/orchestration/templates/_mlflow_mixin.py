...
if TYPE_CHECKING:
    from Jotty.core.intelligence.orchestration.templates.swarm_ml_comprehensive import MLflowConfig
...
class MLflowMixin:
    def init_mlflow(self, config: "MLflowConfig" = None) -> None:
        ...
        try:
            ...
            # Start a new run
            run_name = (
                f"{self._mlflow_config.run_name_prefix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            logger.info(
                f"MLflow initialized: experiment={self._mlflow_config.experiment_name}, run={run_name}"
            )
    ...
    def log_model(
        self, model: Any, artifact_path: str = "model", input_example: Any = None
    ) -> None:
