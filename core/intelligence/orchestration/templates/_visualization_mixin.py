...
class VisualizationMixin:
                ...
                for stype, color in type_colors.items():
                    if stype != "unknown":
                        legend_elements.append(
                            Patch(
                                facecolor=color, alpha=0.85, label=stype.replace("_", " ").title()
                            )
                        )
    ...
    def _create_cross_dataset_chart(self, results: Dict, metric_name: str) -> str:
        ...
        try:
            ...
            with self._chart_context(
                "cross_dataset_validation", figsize=(12, 5), nrows=1, ncols=2
            ) as (fig, (ax1, ax2)):
                # Score comparison
                colors = [
                    self.theme["chart_palette"][i % len(self.theme["chart_palette"])]
                ...
                ax2.set_title(
                    "Dataset Sizes", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
    ...
    def _create_distribution_overview(self, df: pd.DataFrame) -> str:
        ...
        try:
            ...
            with self._chart_context(
                "distributions", figsize=(14, 3 * n_rows), nrows=n_rows, ncols=4
            ) as (fig, axes):
    ...
    def _create_outlier_boxplot(self, df: pd.DataFrame, outlier_summary: Dict) -> str:
        ...
        try:
            ...
            outlier_cols = sorted(
                outlier_summary.keys(), key=lambda x: outlier_summary[x]["count"], reverse=True
            )[:10]
            ...
            with self._chart_context("outlier_boxplot", figsize=(12, 6)) as (fig, ax):
                ...
                bp = ax.boxplot(
                    [plot_data[col].dropna() for col in outlier_cols],
                ...
                ax.set_title(
                    "Outlier Distribution (Standardized)",
                    fontsize=14,
                    fontweight="bold",
                    color=self.theme["primary"],
                )
            ...
            with self._chart_context("correlation_matrix", figsize=(12, 10)) as (fig, ax):
                ...
                try:
                    linkage = hierarchy.linkage(
                        squareform(1 - np.abs(corr_matrix)), method="average"
                    )
                    order = hierarchy.leaves_list(
                        hierarchy.optimal_leaf_ordering(
                            linkage, squareform(1 - np.abs(corr_matrix))
                        )
                    )
                    corr_matrix = corr_matrix.iloc[order, order]
                except Exception as e:
                    self._record_internal_warning(
                        "CorrelationClustering", "correlation matrix clustering failed", e
                    )
                    pass
                ...
                sns.heatmap(
                    corr_matrix,
                    mask=mask,
                    cmap=self.theme["chart_diverging_cmap"],
                    center=0,
    ...
    def _create_vif_chart(self, vif_data: Dict) -> str:
        ...
        try:
            ...
            with self._chart_context("vif_analysis", figsize=(10, 6)) as (fig, ax):
                ...
                ax.set_title(
                    "VIF Analysis", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ...
                ax.axvline(
                    x=10, color=self.theme["danger"], linestyle="--", alpha=0.7, label="Severe (10)"
                )
    ...
    def _create_learning_curve_chart(
        self, sizes: Any, train_mean: Any, train_std: Any, val_mean: Any, val_std: Any
    ) -> str:
        ...
        try:
            with self._chart_context("learning_curves", figsize=(10, 6)) as (fig, ax):
                ...
                ax.set_title(
                    "Learning Curves", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ax.legend(loc="lower right")
    ...
    def _create_calibration_chart(self, fraction_pos: Any, mean_pred: Any, y_prob: Any) -> str:
        ...
        try:
            with self._chart_context("calibration", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                # Calibration curve
                ...
                ax1.fill_between(
                    mean_pred, fraction_pos, mean_pred, alpha=0.2, color=self.theme["warning"]
                )

                ax1.set_xlabel("Mean Predicted Probability", fontsize=11)
                ax1.set_ylabel("Fraction of Positives", fontsize=11)
                ax1.set_title(
                    "Calibration Curve", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
    ...
    def _create_confidence_charts(self, y_true: Any, y_prob: Any, ece_data: Dict) -> str:
        ...
        try:
            with self._chart_context("confidence_analysis", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                bins = ece_data["bins"]
                ...
                ax2.set_title(
                    f'Per-Bin Calibration (ECE={ece_data["ece"]:.4f})',
                    fontsize=14,
                    fontweight="bold",
                    color=self.theme["primary"],
                )
    ...
    def _create_lift_gain_chart(
        self, deciles: Any, gains: Any, lift: Any, ks_stat: Any, ks_idx: Any
    ) -> str:
        """Create lift and gain visualization."""
        try:
            with self._chart_context("lift_gain", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                # Cumulative gains
                ...
                ax1.fill_between(
                    deciles * 100, gains * 100, deciles * 100, alpha=0.2, color=self.theme["accent"]
                )

                # KS line
                ax1.axvline(
                    x=deciles[ks_idx] * 100, color=self.theme["danger"], linestyle=":", alpha=0.7
                )
                ax1.annotate(
                    f"KS = {ks_stat:.3f}",
                    xy=(deciles[ks_idx] * 100, (gains[ks_idx] + deciles[ks_idx]) / 2 * 100),
                    fontsize=10,
                    color=self.theme["danger"],
                )
                ...
                ax2.set_title(
                    "Lift Curve", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
    ...
    def _create_cv_chart(self, cv_results: Dict) -> str:
        ...
        try:
            with self._chart_context("cv_analysis", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                ...
                ax1.set_title(
                    "Accuracy by Fold", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ...
                bp = ax2.boxplot(
                    data, labels=["Accuracy", "Precision", "Recall", "F1"], patch_artist=True
                )
    ...
    def _create_shap_summary(self, shap_values: Any, feature_names: Any, X_sample: Any) -> str:
        ...
        try:
            ...
            shap.summary_plot(
                plot_values, X_sample, feature_names=feature_names, show=False, max_display=20
            )
            plt.title(
                "SHAP Summary Plot", fontsize=14, fontweight="bold", color=self.theme["primary"]
            )
            plt.tight_layout()
    ...
    def _create_shap_dependence(
        self, shap_values: Any, feature_names: Any, X_sample: Any, top_n: int
    ) -> str:
        ...
        try:
            ...
            with self._chart_context(
                "shap_dependence", figsize=(5 * top_n, 4), nrows=1, ncols=top_n
            ) as (fig, axes):
                if top_n == 1:
                    axes = [axes]
    ...
    def _create_threshold_chart(
        self, results: List[Dict], best_f1_thresh: float, best_cost_thresh: float
    ) -> str:
        """Create threshold analysis visualization."""
        try:
            with self._chart_context(
                "threshold_optimization", figsize=(12, 5), nrows=1, ncols=2
            ) as (fig, (ax1, ax2)):
                ...
                ax1.plot(
                    thresholds, f1, "^-", color=self.theme["warning"], linewidth=2, label="F1 Score"
                )
                ...
                ax2.set_title(
                    "Cost vs Threshold", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ax2.legend(loc="best")
                ax2.grid(True, alpha=0.3)
    ...
    def _create_hyperparameter_charts(
        self, trials: List[Dict], param_names: List[str], objective_name: str
    ) -> str:
        ...
        try:
            ...
            with self._chart_context(
                ...
                for pname in param_names:
                    ...
                    try:
                        ...
                        if raw_arr.max() != raw_arr.min():
                            param_data[pname] = (raw_arr - raw_arr.min()) / (
                                raw_arr.max() - raw_arr.min()
                            )
                        else:
                        ...
                        val_map = {
                            v: i / max(1, len(unique_vals) - 1) for i, v in enumerate(unique_vals)
                        }
                ...
                ax1.set_xticklabels(
                    [p[:12] for p in param_names], rotation=45, ha="right", fontsize=8
                )
    ...
    def _create_executive_dashboard_chart(self, kpis: Dict[str, float]) -> str:
        ...
        try:
            n_kpis = len(kpis)
            with self._chart_context(
                "executive_dashboard", figsize=(4 * n_kpis, 3.5), nrows=1, ncols=n_kpis
            ) as (fig, axes):
                if n_kpis == 1:
                    axes = [axes]
    ...
    def _create_class_distribution_chart(self, labels: List[str], counts: Any) -> str:
        ...
        try:
            with self._chart_context("class_distribution", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                # Bar chart
                ...
                bars = ax1.bar(
                    labels, counts, color=colors, alpha=0.85, edgecolor="white", linewidth=1.5
                )
                ...
                ax2.set_title(
                    "Class Proportions", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
    ...
    def _create_permutation_importance_chart(
        self, result: Any, feature_names: List[str], top_idx: Any
    ) -> str:
        """Create permutation importance chart with error bars."""
        try:
            n = len(top_idx)
            with self._chart_context("permutation_importance", figsize=(10, max(6, n * 0.4))) as (
                fig,
                ax,
            ):
    ...
    def _create_pdp_chart(
        self, model: Any, X: Any, top_features: List[str], all_feature_names: List[str]
    ) -> str:
        ...
        try:
            ...
            n = len(top_features)
            with self._chart_context("pdp_ice", figsize=(5 * n, 4.5), nrows=1, ncols=n) as (
                fig,
                axes,
            ):
                if n == 1:
                ...
                for i, feat_name in enumerate(top_features):
                    ax = axes[i]
                    feat_idx = (
                        all_feature_names.index(feat_name) if feat_name in all_feature_names else i
                    )

                    # Compute PDP
                    pdp_result = partial_dependence(
                        model, X, features=[feat_idx], kind="both", grid_resolution=50
                    )
    ...
    def _create_bootstrap_auc_chart(
        ...
        try:
            ...
            with self._chart_context(
                "bootstrap_analysis", figsize=(6 * n_plots, 5), nrows=1, ncols=n_plots
            ) as (fig, axes):
                if n_plots == 1:
                    axes = [axes]
                ...
                ax1.axvline(
                    np.mean(boot_accuracies),
                    color=self.theme["primary"],
                    linestyle="-",
                    linewidth=2,
                    label=f"Mean: {np.mean(boot_accuracies):.4f}",
                )
                ax1.axvline(
                    acc_ci_lower,
                    color=self.theme["danger"],
                    linestyle="--",
                    linewidth=1.5,
                    label=f"95% CI: [{acc_ci_lower:.4f}, {acc_ci_upper:.4f}]",
                )
                ...
                if auc_data and n_plots > 1:
                    ...
                    ax2.axvline(
                        ...
                        linestyle="--",
                        linewidth=1.5,
                        label=f'95% CI: [{auc_data["ci_lower"]:.4f}, {auc_data["ci_upper"]:.4f}]',
                    )
                    ax2.axvline(
                        auc_data["ci_upper"],
                        color=self.theme["danger"],
    ...
    def _create_score_distribution_chart(
        self, class_probs: Dict[str, np.ndarray], optimal_threshold: float
    ) -> str:
        """Create score distribution chart with overlap shading."""
        try:
            from scipy.stats import gaussian_kde

            with self._chart_context("score_distribution", figsize=(12, 5), nrows=1, ncols=2) as (
                fig,
                (ax1, ax2),
            ):
                colors = [
                ...
                for (label, probs), color in zip(class_probs.items(), colors):
                    if len(probs) > 2:
                        try:
                            ...
                            self._record_internal_warning(
                                "KDEComputation", "KDE computation failed", e
                            )
                            pass
    ...
    def _create_training_curves(self, history: Dict) -> str:
        ...
        try:
            ...
            with self._chart_context(
                "training_curves", figsize=(fig_width, 5), nrows=1, ncols=n_cols
            ) as (fig, axes):
                ...
                ax1.plot(
                    epochs, loss, color=self.theme["accent"], linewidth=2, label="Training Loss"
                )
                if val_loss:
                    ax1.plot(
                        epochs,
    ...
    def _create_regression_charts(self, y_true: Any, y_pred: Any, residuals: Any) -> str:
        ...
        try:
            ...
            with self._chart_context(
                "regression_diagnostics", figsize=(12, 10), nrows=2, ncols=2
            ) as (fig, axes):
                ...
                ax1.plot(
                    [min_val, max_val], [min_val, max_val], "k--", linewidth=1.5, label="Perfect"
                )
                ...
                ax2.set_title(
                    "Residual Plot", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ...
                standardized_residuals = (
                    sorted_residuals - sorted_residuals.mean()
                ) / sorted_residuals.std()

                ax3.scatter(
                    theoretical_quantiles,
                    standardized_residuals[: len(theoretical_quantiles)],
                    alpha=0.5,
                    s=20,
                    color=self.theme["warning"],
                )
                ...
                ax3.set_title(
                    "Q-Q Plot", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ...
                ax4.plot(
                    x_range, normal_pdf, color=self.theme["danger"], linewidth=2, label="Normal"
                )
...
Plotly.newPlot('{chart_id}', [{{
    ...
    paper_bgcolor: '{t["chart_bg"]}',
                ...
                importance = (
                    data.get("mean_shap") or data.get("importance") or data.get("importance_values")
                )
...
Plotly.newPlot('{chart_id}', [{{
    ...
    plot_bgcolor: 'white',
                ...
                if x_values and y_values:
                    ...
                    if color_values:
                        marker_config = (
                            f"color: {color_values}, colorscale: 'RdYlGn', showscale: true"
                        )
...
Plotly.newPlot('{chart_id}', [{','.join(traces)}], {{
    ...
    def _create_importance_chart(self, sorted_importance: List[Tuple[str, float]]) -> str:
        ...
        try:
            ...
            with self._chart_context("feature_importance", figsize=(10, max(6, n * 0.35))) as (
                fig,
                ax,
            ):
    ...
    def _create_benchmark_chart(self, sorted_models: List[Tuple[str, Dict]]) -> str:
        ...
        try:
            ...
            with self._chart_context("model_benchmark", figsize=(10, 6)) as (fig, ax):
                ...
                ax.set_title(
                    "Model Comparison", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ax.set_xlim(0, 1)
    ...
    def _create_confusion_matrix_chart(self, cm: Any, labels: List[str] = None) -> str:
        ...
        try:
            ...
            with self._chart_context("confusion_matrix", figsize=(8, 6)) as (fig, ax):
                ...
                ax.set_title(
                    "Confusion Matrix", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
    ...
    def _create_roc_chart(self, fpr: Any, tpr: Any, roc_auc: float) -> str:
        ...
        try:
            with self._chart_context("roc_curve", figsize=(8, 6)) as (fig, ax):
                ...
                ax.set_title(
                    "ROC Curve", fontsize=14, fontweight="bold", color=self.theme["primary"]
                )
                ax.legend(loc="lower right")
                ax.grid(True, alpha=0.3)
    ...
    def _create_shap_chart(self, shap_values: Any, feature_names: Any, X_sample: Any) -> str:
        ...
        try:
            ...
            fig, ax = plt.subplots(figsize=(10, 8))
            shap.summary_plot(
                plot_values, X_sample, feature_names=feature_names, show=False, max_display=15
            )
            plt.title(
                "SHAP Feature Impact", fontsize=14, fontweight="bold", color=self.theme["primary"]
            )
            plt.tight_layout()
        ...
        try:
            ...
            with self._chart_context("model_comparison_roc", figsize=(8, 8)) as (fig, ax):
                for i, (model_name, curve) in enumerate(roc_curves.items()):
                    color = palette[i % len(palette)]
                    ax.plot(curve["fpr"], curve["tpr"], color=color, linewidth=2, label=model_name)
    ...
    def _create_metrics_radar_chart(self, comparison: Dict) -> str:
        ...
        try:
            ...
            available_metrics = [
                m for m in metrics if any(v.get(m) is not None for v in comparison.values())
            ]
            ...
            with self._chart_context(
                "model_comparison_radar", figsize=(8, 8), subplot_kw={"projection": "polar"}
            ) as (fig, ax):
                ...
                ax.set_ylim(0, 1.05)
                ax.set_title(
                    "Model Comparison — Metrics Radar",
                    fontsize=14,
                    fontweight="bold",
                    color=self.theme["primary"],
                    pad=20,
