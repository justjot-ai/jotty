...
async def p2p_discovery_phase(
    ...
    async def run_discovery_agent(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate discovery
        start = datetime.now()
        result = agent(task=task, context=context_text)
        ...
        return {"agent": agent_name, "output": output, "time": elapsed}
...
async def sequential_delivery_phase(
    ...
    for i, config in enumerate(agents_config):
        ...
        all_discoveries = "\n\n".join(
            [f"{name}: {text[:500]}" for name, text in discoveries.items()]
        )
...
"""
        ...
        start = datetime.now()
        result = agent(context=context, previous_output=previous_output or "First deliverable")
