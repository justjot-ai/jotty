...
class ProfessionalMLReport(
    ...
    def __init__(
        self,
        output_dir: str = "professional_reports",
    ...
    def _save_figure(self, filename: str) -> str:
        ...
        plt.savefig(full_path, dpi=300, bbox_inches="tight", facecolor=self.theme["chart_bg"])
        plt.close()
        ...
        try:
            ...
            if self.theme_name == "goldman":
                ...
                try:
                    ...
                    if hasattr(self, "_warnings"):
                        self._record_internal_warning(
                            "PlotStyleGoldman", "seaborn style not available", e
                        )
                mpl.rcParams.update(
                    {
                        "font.family": "serif",
                ...
                try:
                    ...
                    if hasattr(self, "_warnings"):
                        self._record_internal_warning(
                            "PlotStyleProfessional", "seaborn style not available", e
                        )
                mpl.rcParams.update(
                    {
    ...
    def _validate_inputs(self, **kwargs: Any) -> Any:
        ...
        if X is not None and y_true is not None:
            ...
            if x_len > 0 and x_len != y_len:
                raise ValueError(f"X rows ({x_len}) != y_true length ({y_len})")
        ...
        if feature_names is not None:
            if not all(isinstance(f, str) for f in feature_names):
                ...
                raise ValueError(f"feature_names must all be strings, found types: {bad_types}")
    ...
    def _record_internal_warning(
        ...
        logger.debug(
            f"Internal warning [{component}]: {message}" + (f" ({error})" if error else "")
        )
    ...
    def _capture_environment(self) -> Dict:
        ...
        for lib_name in ["sklearn", "numpy", "pandas", "matplotlib", "shap"]:
            try:
                ...
                self._record_internal_warning(
                    "EnvironmentCapture", f"Could not get version for {lib_name}", e
                )
    ...
    def _validate_narrative(self, text: str, section_type: str = "general") -> Dict:
        ...
        prob_matches = re.findall(
            r"(?:probability|prob|p-value|confidence)\s*(?:of|=|:)?\s*(\d+\.?\d*)", cleaned.lower()
        )
        ...
        count_matches = re.findall(
            r"(\-\d+)\s*(?:samples|instances|observations|errors|count)", cleaned.lower()
        )
        ...
        for attempt in range(2):
            try:
                ...
                response = self._llm.generate(
                    prompt=prompt, timeout=30, max_tokens=400, fallback=True
                )
    ...
    def _add_executive_summary_impl(
        self, metrics: Any, best_model: Any, n_features: Any, context: Any
    ) -> Any:
        # Compute risk score
        ...
        for assessment in risk_data["metric_assessments"]:
            if assessment["traffic_light"] == "RED":
                key_findings.append(
                    f"- **{assessment['metric']}** ({assessment['value']:.4f}) is below acceptable threshold"
                )
            elif assessment["traffic_light"] == "GREEN":
                key_findings.append(
                    f"- **{assessment['metric']}** ({assessment['value']:.4f}) meets production standards"
                )

        if not key_findings:
            key_findings = [
                f"- Model {best_model} achieves moderate performance across all metrics"
            ]
...
"""
    ...
    def _add_data_profile_impl(
        self, shape: Any, dtypes: Any, missing: Any, recommendations: Any
    ) -> Any:
...
"""
    ...
    def _add_model_comparison_impl(
        self, models: Any, X_test: Any, y_true: Any, class_labels: Any = None
    ) -> Any:
                    ...
                    try:
                        if n_classes <= 2:
                            ...
                            auc_val = roc_auc_score(
                                y_true_arr, y_prob, multi_class="ovr", average="weighted"
                            )
                    except Exception:
                        auc_val = None
        ...
        for metric in metrics_list:
            valid = {m: v[metric] for m, v in comparison.items() if v.get(metric) is not None}
            if valid:
                best_model = max(valid, key=valid.get)
                best_per_metric[metric] = {"model": best_model, "value": valid[best_model]}
        ...
        for model_name, scores in comparison.items():
            ...
            for metric in metrics_list:
                ...
                if val is None:
                    ...
                    is_best = (
                        metric in best_per_metric and best_per_metric[metric]["model"] == model_name
                    )
                    fmt = f"{val:.4f}"
...
"""
    ...
    def _add_confusion_matrix_impl(self, y_true: Any, y_pred: Any, labels: Any) -> Any:
        ...
        report = classification_report(
            preds.y_true, preds.y_pred, target_names=labels, output_dict=True
        )
    ...
    def _add_precision_recall_impl(self, y_true: Any, y_prob: Any, pos_label: Any) -> Any:
        ...
        if is_binary:
            ...
            precision, recall, _ = precision_recall_curve(
                preds.y_true, prob_1d, pos_label=pos_label
            )
...
"""
        self._content.append(content)
        self._store_section_data(
            "precision_recall", "Precision-Recall Analysis", {"avg_precision": avg_precision}
        )

    def _add_baseline_comparison_impl(
        self, baseline_score: Any, final_score: Any, baseline_model: Any
    ) -> Any:
...
"""
        self._content.append(content)
        self._store_section_data(
            "baseline_comparison", "Baseline Comparison", {"improvement_pct": improvement_pct}
        )
...
"""
            for col in (
                missing_counts[missing_counts > 0].sort_values(ascending=False).head(15).index
            ):
...
"""
            for col, stats in sorted(
                outlier_summary.items(), key=lambda x: x[1]["count"], reverse=True
            )[:15]:
                if stats["count"] > 0:
...
"""
        ...
        self._store_section_data(
            ...
            "Correlation Analysis",
            {
                ...
                "high_corr_pairs": [
                    {"f1": p["feature_1"], "f2": p["feature_2"], "corr": p["correlation"]}
                    for p in sorted(
    ...
    def _add_reproducibility_section_impl(
        ...
        for pkg in ["numpy", "pandas", "scikit-learn", "matplotlib", "seaborn", "shap"]:
            try:
                ...
                if hasattr(self, "_warnings"):
                    self._record_internal_warning(
                        "PackageVersion", f"Could not get version for {pkg}", e
                    )
...
"""
    ...
    def _bootstrap_auc_ci(self, y_true: Any, y_prob: Any, n_boot: int = 1000) -> Dict:
        ...
        for _ in range(n_boot):
            ...
            try:
                ...
                if hasattr(self, "_warnings"):
                    self._record_internal_warning(
                        "BootstrapAUC", "single bootstrap iteration failed", e
                    )
    ...
    def _get_nn_architecture_info(self, model: Any) -> Dict:
        ...
        if "torch" in model_type:
            ...
            try:
                ...
                if hasattr(self, "_warnings"):
                    self._record_internal_warning(
                        "PyTorchModelInfo", "failed to extract PyTorch model info", e
                    )
        ...
        # Keras / TensorFlow
            ...
            try:
                ...
                if hasattr(self, "_warnings"):
                    self._record_internal_warning(
                        "KerasModelInfo", "failed to extract Keras model info", e
                    )
    ...
    def _add_model_card_impl(
        self, model: Any, results: Any, intended_use: Any, limitations: Any, ethical: Any
    ) -> Any:
        import platform
        ...
        if score < 0.8:
            auto_limitations.append(
                "Model accuracy below 80% — may not be suitable for high-stakes decisions"
            )
        if score > 0.98:
            auto_limitations.append(
                "Very high accuracy may indicate overfitting — validate on new data"
            )
        ...
        if "class_distribution" in results:
            ...
            if isinstance(dist, dict):
                ...
                if len(counts) >= 2:
                    ...
                    if ratio > 3:
                        auto_limitations.append(
                            f"Training data is imbalanced (ratio {ratio:.1f}:1) — predictions for minority class may be less reliable"
                        )
        ...
        if auto_limitations:
            ...
            limitations_combined = (
                ...
                if limitations
                else f"**Auto-detected Limitations:**\n\n{auto_text}"
            )
