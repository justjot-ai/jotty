...
"""
                ...
                for group, m in sorted(display_groups.items(), key=lambda x: -x[1]["n"]):
                    ...
                    content += (
                        f"| {group} (n={m['n']}) | {m['positive_rate']:.3f} | "
...
"""
            ...
            if len(sensitive_features) >= 2:
                ...
                if intersectional_metrics:
                    ...
                    display_metrics = {
                        k: v for k, v in intersectional_metrics.items() if k != "_aggregate"
                    }
                    content += """
## Intersectional Analysis
...
"""
                    for group, m in sorted(display_metrics.items(), key=lambda x: -x[1]["n"]):
                        content += (
                            f"| {group[:40]} | {m['n']} | {m['positive_rate']:.3f} | "
            ...
            if all_metrics and y_prob_arr is not None:
                ...
                if (
                    isinstance(first_feat_data, str)
                    and hasattr(X, "columns")
                    and first_feat_data in X.columns
                ):
                ...
                if groups_arr is not None:
                    fair_thresholds = self._find_fair_thresholds(
                        y_true_arr, y_prob_arr, groups_arr, fairness_criterion="equalized_odds"
                    )
                    if fair_thresholds:
                        content += f"""
...
"""
            ...
            if all_metrics:
                ...
                chart_metrics = {
                    k: v for k, v in all_metrics[first_feat].items() if k != "_aggregate"
                }
...
"""
            ...
            for k, v in all_metrics.items():
                stored_metrics[k] = {gk: {mk: mv for mk, mv in gv.items()} for gk, gv in v.items()}
        ...
        if len(metrics) >= 2:
            ...
            metrics["_aggregate"] = {
                "equalized_odds_diff": float(
                    max(max(group_tprs) - min(group_tprs), max(group_fprs) - min(group_fprs))
                ),
    ...
    def _compute_intersectional_fairness(
        self, y_true: Any, y_pred: Any, y_prob: Any, sensitive_features: Dict[str, Any], X: Any
    ) -> Dict:
        ...
        intersection_groups = np.array(
            [f"{feat_names[0]}={a} x {feat_names[1]}={b}" for a, b in zip(arr1, arr2)]
        )

        return self._compute_fairness_metrics(y_true, y_pred, y_prob, intersection_groups)

    def _find_fair_thresholds(
        self,
        y_true: Any,
        y_prob: Any,
        groups: Any,
        fairness_criterion: Any = "equalized_odds",
        min_performance: Any = None,
    ) -> Dict:
    ...
    def _create_fairness_radar_chart(self, metrics_per_group: Dict, feature_name: str) -> str:
        ...
        try:
            ...
            with self._chart_context(chart_name, figsize=(8, 8), subplot_kw=dict(polar=True)) as (
                fig,
                ax,
            ):
                ...
                ax.set_ylim(0, 1)
                ax.set_title(
                    f"Fairness Metrics: {feature_name}",
                    fontsize=14,
                    fontweight="bold",
