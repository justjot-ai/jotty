...
class DriftMixin:
    ...
    def _compute_categorical_drift(self, ref_col: Any, cur_col: Any, feature_name: str) -> Dict:
        ...
        try:
            ...
            self._record_internal_warning(
                "CategoricalDrift", "Failed to compute categorical drift metrics", e
            )
            return {"chi2_stat": 0.0, "chi2_pval": 1.0, "cramers_v": 0.0, "status": "OK"}
    ...
    def _compute_mahalanobis_drift(self, X_ref_numeric: Any, X_cur_numeric: Any) -> Dict:
        ...
        try:
            ...
            self._record_internal_warning(
                "MahalanobisDrift", "Failed to compute Mahalanobis distance drift", e
            )
    ...
    def _compute_mmd(self, X_ref: Any, X_cur: Any, n_permutations: Any = None) -> Dict:
        ...
        try:
            ...
            mmd_stat = float(np.mean(K_rr) - 2 * np.mean(K_rc) + np.mean(K_cc))
            ...
            self._record_internal_warning(
                "MMDComputation", "Failed to compute Maximum Mean Discrepancy", e
            )
        ...
        try:
            ...
            with _warnings.catch_warnings():
                ...
                scores = cross_val_score(
                    clf, X_combined, y_combined, cv=min(5, min(n_ref, n_cur)), scoring="roc_auc"
                )
            ...
            feature_importances = {f"feature_{i}": float(importances[i]) for i in top_indices}
            ...
            self._record_internal_warning(
                "ClassifierDrift", "Failed to perform classifier-based drift detection", e
            )
            return {"auc": 0.5, "drift_detected": False, "feature_importances": {}}

    def _compute_ensemble_drift_score(
        self, mahal_result: Any, mmd_result: Any, classifier_result: Any
    ) -> Dict:
    ...
    def add_drift_analysis(
        ...
        try:
            ...
            fig_path = (
                self._create_drift_heatmap(drift_results, psi_warn, psi_alert)
                if drift_results
                else ""
            )
            ...
            for r in sorted(drift_results, key=lambda x: x["psi"], reverse=True)[:20]:
                status_icon = (
                    "ALERT"
                    if r["status"] == "ALERT"
                    else ("WARN" if r["status"] == "WARNING" else "OK")
                )
                table_md += (
                    f"| {r['feature'][:25]} | {r['psi']:.4f} | {r['ks_stat']:.4f} | "
...
"""
                for r in sorted(
                    categorical_results, key=lambda x: x.get("cramers_v", 0), reverse=True
                ):
                    content += (
                        f"| {r['feature'][:25]} | {r['chi2_stat']:.2f} | "
            ...
            if len(numeric_indices) >= 2:
                ...
                if ref_mask.sum() > 10 and cur_mask.sum() > 10:
                    ...
                    mahal_result = self._compute_mahalanobis_drift(X_ref_clean, X_cur_clean)
...
"""
                    ...
                    classifier_result = self._classifier_drift_detection(X_ref_clean, X_cur_clean)
...
"""
                    if classifier_result.get("feature_importances"):
                        ...
                        top_feats = sorted(
                            classifier_result["feature_importances"].items(),
                            key=lambda x: x[1],
                            reverse=True,
                        )[:5]
...
|---------|-----|-----------|---------------|
"""
                for r in sorted(severity_results, key=lambda x: x["severity_score"], reverse=True)[
                    :10
                ]:
                    content += (
                        f"| {r['feature'][:25]} | {r['psi']:.4f} | "
...
"""
                        ...
                        self._record_internal_warning(
                            "ConceptDrift",
                            "Failed to compute concept drift for target distribution",
                            e,
                        )
...
"""
            ...
            self._store_section_data(
                ...
                (
                    [{"type": "heatmap", "path": fig_path}, {"type": "violin"}]
                    if drift_results
                    else []
                ),
            )
    ...
    def _calculate_js_divergence(self, reference: Any, current: Any, n_bins: int = None) -> float:
        ...
        try:
            ...
            self._record_internal_warning(
                "JensenShannonDivergence", "Failed to calculate Jensen-Shannon divergence", e
            )
            return 0.0

    def _create_drift_heatmap(
        self, drift_results: List[Dict], psi_warn: float, psi_alert: float
    ) -> str:
        ...
        try:
            ...
            with self._chart_context(
                ...
                # Heatmap
                colors_cmap = LinearSegmentedColormap.from_list(
                    "drift", [self.theme["success"], self.theme["warning"], self.theme["danger"]]
                )
                ...
                colors = [
                    (
                        self.theme["danger"]
                        if v >= psi_alert
                        else (self.theme["warning"] if v >= psi_warn else self.theme["success"])
                    )
                    for v in psi_values
                ]
                ...
                ax2.set_title(
                    "PSI Values", fontsize=12, fontweight="bold", color=self.theme["primary"]
                )
