...
class SwarmResearcher:
    ...
    async def research(self, query: str, research_type: str = "general") -> ResearchResult:
        """
...
"""
        ...
        try:
            ...
            if hasattr(dspy, "settings") and dspy.settings.lm:
                ...
                try:
                    ...
                    if not findings:
                        findings = self._extract_findings_from_keywords(
                            response, query, research_type
                        )
        ...
        for line in lines:
            ...
            if line.startswith("-") or line.startswith("*") or line.startswith("•"):
                ...
                if current_section == "tools" or "tool" in line_lower or "library" in line_lower:
                    ...
                    if package_match:
                        findings.append(
                            {"type": "tool", "name": package_match.group(1), "text": content}
                        )
                ...
                # Extract API names
                    ...
                    if api_match:
                        findings.append(
                            {"type": "api", "name": api_match.group(1).strip(), "text": content}
                        )
                    else:
                        # Try to extract API name directly
                        api_name = content.split()[0] if content.split() else content
                        findings.append({"type": "api", "name": api_name, "text": content})
                ...
                # Extract URLs
                    ...
                    if url_match:
                        findings.append(
                            {"type": "documentation", "url": url_match.group(0), "text": content}
                        )
            ...
            # Look for inline mentions
                ...
                if pip_match:
                    findings.append({"type": "tool", "name": pip_match.group(1), "text": line})

                # Check for API mentions
                api_match = re.search(r"([A-Z][a-zA-Z\s]+)\s+API", line, re.IGNORECASE)
                if api_match:
                    findings.append(
                        {"type": "api", "name": api_match.group(1).strip(), "text": line}
                    )
                ...
                if url_match:
                    findings.append(
                        {"type": "documentation", "url": url_match.group(0), "text": line}
                    )
    ...
    def _extract_findings_from_keywords(
        self, response: str, query: str, research_type: str
    ) -> List[Dict[str, Any]]:
        ...
        for pattern in api_patterns:
            ...
            for match in matches:
                if isinstance(match, tuple):
                    match = match[0]
                findings.append(
                    {"type": "api", "name": match.strip(), "text": f"Found API: {match}"}
                )
            ...
            for skill_name in skill_names:
                ...
                if skill:
                    ...
                    if hasattr(skill, "tags") and skill.tags:
                        ...
                        if any(
                            query_lower in tag
                            or tag in query_lower
                            or any(w in tag for w in query_words)
                            for tag in tags
                        ):
                            tools_found.append(skill_name)
        except Exception as e:
    ...
    def _rank_providers(
        self, candidates: List[ProviderCandidate], capability: str
    ) -> List[ProviderCandidate]:
        """
