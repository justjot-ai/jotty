...
class PerspectiveLearningSwarm(SwarmTemplate):
    ...
    async def _execute_phases(
        ...
        perspective_results = await executor.run_parallel(
            2,
            f"6 Perspectives in Parallel ({len(perspective_tasks)} agents)",
            perspective_tasks,
        )
        ...
        if non_english:
            ...
            lang_results = await executor.run_parallel(
                3,
                f"Multilingual Content ({len(non_english)} languages)",
                lang_tasks,
            )
        else:
            lang_results = []
        ...
        socratic_questions = (
            narrative_result.get("socratic_questions", [])
            if isinstance(narrative_result, dict)
            else []
        )
        parent_guide = (
            narrative_result.get("parent_guide", "") if isinstance(narrative_result, dict) else ""
        )
        key_takeaways = (
            narrative_result.get("key_takeaways", []) if isinstance(narrative_result, dict) else []
        )

        logger.info(
            f"Phase 5: {len(socratic_questions)} Socratic questions, "
        ...
        logger.info(
            f"  {len(perspective_sections)} perspectives, "
        ...
        output_results = await executor.run_parallel(
            ...
            [
                (
                    ...
                    AgentRole.ACTOR,
                    (
                        self._generate_pdf(topic, student_name, content)
                ...
                (
                    ...
                    AgentRole.ACTOR,
                    (
                        self._generate_html(topic, student_name, content)
        ...
        if should_send and TELEGRAM_AVAILABLE and content:
            logger.info(
                f"Sending to Telegram (token={'set' if os.environ.get('TELEGRAM_TOKEN') else 'MISSING'})..."
            )
...
async def teach_perspectives(
    ...
    Usage:
        from Jotty.core.intelligence.orchestration.swarms.perspective_learning_swarm import teach_perspectives
    ...
    depth_enum = (
        ContentDepth(depth) if depth in [d.value for d in ContentDepth] else ContentDepth.STANDARD
    )
