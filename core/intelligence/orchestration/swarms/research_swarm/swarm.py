...
class ResearchSwarm(SwarmTemplate):
    ...
    async def research(
        ...
        # Validate: ResearchSwarm needs a valid ticker
        if ticker is None:
            logger.warning(f"ResearchSwarm: No valid stock ticker in query: {query[:80]}")
            return ResearchResult(
                ...
                execution_time=0.0,
                ticker="UNKNOWN",
                error=f"No valid stock ticker found in query. ResearchSwarm is for stock/company research. Query: {query[:100]}",
                rating="N/A",
            )
    ...
    async def _execute_phases(
        ...
        if self.config.parallel_fetch:  # type: ignore[attr-defined]
            ...
            financial_data = (
                fetch_results[0]
                if not isinstance(fetch_results[0], dict) or "error" not in fetch_results[0]
                else {}
            )
            web_data = (
                fetch_results[1]
                if not isinstance(fetch_results[1], dict) or "error" not in fetch_results[1]
                else {}
            )
        ...
        sentiment_result = (
            analysis_results[0]
            if not (isinstance(analysis_results[0], dict) and "error" in analysis_results[0])
            else {}
        )
        social_sentiment_result = (
            analysis_results[1]
            if not (isinstance(analysis_results[1], dict) and "error" in analysis_results[1])
            else {}
        )
        peer_result = (
            analysis_results[2]
            if not (isinstance(analysis_results[2], dict) and "error" in analysis_results[2])
            else {}
        )
        llm_result = (
            analysis_results[3]
            if not (isinstance(analysis_results[3], dict) and "error" in analysis_results[3])
            else {}
        )
        ...
        if self.config.include_charts:  # type: ignore[attr-defined]
            ...
            chart_paths = (
                chart_result.get("chart_paths", []) if isinstance(chart_result, dict) else []
            )
            ...
            try:
                ...
                if doc_skill and doc_skill.tools:
                    ...
                    if convert_tool:
                        ...
                        if pdf_result.get("success") and os.path.exists(
                            pdf_result.get("output_path", "")
                        ):
                            ...
                            pdf_path = ""
                            logger.warning(
                                f"Topic research: PDF conversion failed: {pdf_result.get('error', 'unknown')}"
                            )
                    ...
                    if telegram_skill and telegram_skill.tools:
                        ...
                        if send_tool:
                            ...
                            if telegram_sent:
                                ...
                                logger.warning(
                                    f"Topic research: Telegram send failed: {tg_result.get('error')}"
                                )
    ...
    # Words that look like tickers (short ALL-CAPS) but are common English
    _COMMON_ENGLISH_WORDS = {
        ...
        "ZERO",
        "ZONE",
        # Common tech/ML abbreviations that are not stock tickers
        "LSTM",
        "GRU",
        "CNN",
        "RNN",
    ...
    def _extract_ticker(self, query: str) -> Optional[str]:
        ...
        # 1. Explicit patterns: RELIANCE.NS, $AAPL, NSE:INFY, BSE:500325
        explicit_patterns = [
            r"\b([A-Z]{1,15})\.(NS|BO|NSE|BSE)\b",  # RELIANCE.NS
            r"\$([A-Z]{1,10})\b",  # $AAPL
            r"\b(?:NSE|BSE|NYSE|NASDAQ)[:\s]([A-Z]{1,15})\b",  # NSE:INFY
        ]
        ...
        # 2. Find ALL-CAPS words 1-10 chars that aren't common English words
        words = query.split()
        for word in words:
            clean = re.sub(r'[,\-:.\'"!?]', "", word)
            if (
