    ...
    def _get_lm(self) -> Any:
        ...
        if self._lm is None:
            ...
            try:
                ...
                logger.debug(f"UnifiedLMProvider not available: {e}")
...
class CurriculumArchitectAgent(BaseOlympiadAgent):
    ...
    async def design(
        ...
        try:
            ...
            checkpoints = [
                c.strip() for c in str(result.milestone_checkpoints).split("|") if c.strip()
            ]
...
class ConceptDecomposerAgent(BaseOlympiadAgent):
    ...
    async def decompose(
        ...
        try:
            ...
            misconceptions = [
                m.strip() for m in str(result.common_misconceptions).split("|") if m.strip()
            ]
...
class PatternHunterAgent(BaseOlympiadAgent):
    ...
    async def hunt(
        self, topic: str, subject: str, concepts: str, target_level: str
    ) -> Dict[str, Any]:
        """Find patterns in a topic."""
        try:
            ctx = f"\n\n{self.learned_context}" if self.learned_context else ""
            result = self._hunter(
                topic=topic + ctx, subject=subject, concepts=concepts, target_level=target_level
            )
...
class ProblemCrafterAgent(BaseOlympiadAgent):
    ...
    async def craft(
        ...
        try:
            ...
            tier_enum = (
                DifficultyTier(tier)
                if tier in [t.value for t in DifficultyTier]
...
class SolutionStrategistAgent(BaseOlympiadAgent):
    ...
    async def strategize(
        ...
        try:
            ...
            return {"strategies": strategies, "speed_techniques": speed, "stuck_toolkit": stuck}
...
class MistakeAnalyzerAgent(BaseOlympiadAgent):
    ...
    async def analyze(
        ...
        try:
            ...
            return {"mistakes": mistakes, "trap_problems": traps}
...
class ConnectionMapperAgent(BaseOlympiadAgent):
    ...
    async def map_connections(self, topic: str, subject: str, concepts: str) -> Dict[str, Any]:
        """Map connections to other topics."""
        try:
            ctx = f"\n\n{self.learned_context}" if self.learned_context else ""
            result = self._mapper(topic=topic + ctx, subject=subject, concepts=concepts)

            direct = [c.strip() for c in str(result.direct_connections).split("|") if c.strip()]
            surprising = [
                c.strip() for c in str(result.surprising_connections).split("|") if c.strip()
            ]
            powerful = [
                c.strip() for c in str(result.powerful_combinations).split("|") if c.strip()
            ]
...
class NarrativeEditorAgent(BaseOlympiadAgent):
    ...
    async def edit(
        ...
        try:
            ...
            logger.info(
                f"NarrativeEditor: {len(socratic)} Socratic questions, {len(breakthroughs)} breakthroughs"
            )
