...
class WorkspaceManager:
    ...
    async def write_file(self, filename: str, content: str) -> "CommandResult":
        ...
        if not self._ensure_init():
            from Jotty.core.intelligence.orchestration.state.swarm_terminal import CommandResult

            return CommandResult(
                success=False, command="write_file", output="", error="WorkspaceManager unavailable"
            )
        import os
    ...
    async def syntax_check(self, filename: str, language: str = "python") -> "CommandResult":
        ...
        if not self._ensure_init():
            ...
            return CommandResult(
                ...
                output="",
                error="WorkspaceManager unavailable",
            )
        import os
    ...
    async def run_python(self, filename: str, timeout: int = 30) -> "CommandResult":
        ...
        if not self._ensure_init():
            ...
            return CommandResult(
                success=False, command="run_python", output="", error="WorkspaceManager unavailable"
            )
        import os
    ...
    async def run_tests(
        self, test_filename: Optional[str] = None, timeout: int = 60
    ) -> "CommandResult":
        ...
        if not self._ensure_init():
            ...
            return CommandResult(
                success=False, command="run_tests", output="", error="WorkspaceManager unavailable"
            )
        if test_filename:
            import os
