...
class SwarmTemplate:
    ...
    agents: Dict[str, "AgentConfig"] = {}  # Agent name → config
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SwarmTemplate":
        ...
        for name, config in data.get("agents", {}).items():
            template.agents[name] = Any(**config)  # type: ignore[name-defined]
...
class TemplateExecutor:
    ...
    async def _execute_skill(
        self, skill_name: str, inputs: Dict, agent_config: Any  # type: ignore[name-defined]
    ) -> Dict[str, Any]:
        """
        Execute a skill.
