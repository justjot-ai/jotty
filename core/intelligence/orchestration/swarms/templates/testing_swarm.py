...
from .._base.swarm_signatures import TestingSwarmSignature
...
class CodeAnalysisSignature(dspy.Signature):
    ...
    testable_units: str = dspy.OutputField(
        desc="JSON list of testable units with names and descriptions"
    )
    ...
    integration_points: str = dspy.OutputField(
        desc="Integration points requiring tests, separated by |"
    )
...
class CoverageAnalysisSignature(dspy.Signature):
    ...
    recommendations: str = dspy.OutputField(
        desc="Recommendations to improve coverage, separated by |"
    )
...
class CodeAnalyzerAgent(SwarmLearningAgent):
    ...
    async def analyze(self, code: str, language: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._analyzer(code=code, language=language, context=enriched_context)
            ...
            self._broadcast(
                "code_analyzed", {"units": len(testable_units), "dependencies": len(dependencies)}
            )
    ...
    async def generate(
        ...
        try:
            ...
            result = self._generator(
                code=enriched_code, unit=unit, framework=framework, mocks=json.dumps(mocks or [])
            )

            test_cases = _split_field(result.test_cases)
            coverage_areas = _split_field(result.coverage_areas)

            self._broadcast("unit_tests_generated", {"unit": unit, "test_count": len(test_cases)})
...
class IntegrationTestAgent(SwarmLearningAgent):
    ...
    async def generate(
        self, code: str, integration_point: str, framework: str, setup_info: str = ""
    ) -> Dict[str, Any]:
        """Generate integration tests."""
        try:
            ...
            self._broadcast(
                ...
                {"integration_point": integration_point, "scenarios": len(scenarios)},
            )
...
class E2ETestAgent(SwarmLearningAgent):
    ...
    async def generate(
        ...
        try:
            ...
            result = self._generator(
                code=code, user_flow=user_flow, framework=framework, config=enriched_config
            )

            steps = _split_field(result.steps)
            assertions = _split_field(result.assertions)

            self._broadcast("e2e_tests_generated", {"user_flow": user_flow, "steps": len(steps)})

            return {"test_code": str(result.test_code), "steps": steps, "assertions": assertions}
...
class CoverageAgent(SwarmLearningAgent):
    ...
    async def analyze(self, code: str, tests: str, coverage_data: str = "") -> Dict[str, Any]:
        """Analyze coverage gaps."""
        try:
            enriched_coverage_data = (coverage_data or "No coverage data available") + (
                "\n\n" + self.learned_context if self.learned_context else ""
            )
            result = self._analyzer(code=code, tests=tests, coverage_data=enriched_coverage_data)
            ...
            self._broadcast(
                ...
                {
                    "estimated_coverage": (
                        float(result.estimated_coverage) if result.estimated_coverage else 0
                    ),
                    "gaps_found": len(gaps),
                },
            )
            ...
            return {
                "estimated_coverage": (
                    float(result.estimated_coverage) if result.estimated_coverage else 0.0
                ),
                "gaps": gaps,
...
class QualityAgent(SwarmLearningAgent):
    ...
    async def assess(self, tests: str, code: str) -> Dict[str, Any]:
        """Assess test quality."""
        try:
            enriched_code = code + (
                "\n\n# Learned Context:\n" + self.learned_context if self.learned_context else ""
            )
            result = self._assessor(tests=tests, code=enriched_code)
            ...
            return {
                ...
                "completeness_score": (
                    float(result.completeness_score) if result.completeness_score else 0.0
                ),
                "issues": issues,
...
class TestingSwarm(SwarmTemplate):
    ...
    def _build_output_data(self, result: TestingResult) -> Dict[str, Any]:
        tests = result.test_suite.tests if result.test_suite else []
        return {
            "test_count": result.test_count,
            "estimated_coverage": result.estimated_coverage,
            "quality_score": result.quality_score,
            "gaps_found": len(result.gaps_found),
            "unit_tests": len([t for t in tests if t.test_type == TestType.UNIT]),
            "integration_tests": len([t for t in tests if t.test_type == TestType.INTEGRATION]),
            "e2e_tests": len([t for t in tests if t.test_type == TestType.E2E]),
        }
    ...
    async def _execute_phases(
        self, executor: Any, code: str, lang: str, types: List[TestType], fw: TestFramework
    ) -> TestingResult:
        ...
        dependencies = (
            [d.get("name", str(d)) if isinstance(d, dict) else str(d) for d in raw_deps]
            if isinstance(raw_deps, list)
            else []
        )
        raw_points = analysis.get("integration_points", [])
        integration_points = (
            [p.get("name", str(p)) if isinstance(p, dict) else str(p) for p in raw_points]
        ...
        if TestType.UNIT in types and testable_units:
            ...
            for i, result in enumerate(unit_results):
                ...
                if isinstance(result, dict) and "test_code" in result:
                    unit_name = (
                        testable_units[i].get("name", f"unit_{i}")
                        if isinstance(testable_units[i], dict)
                        else str(testable_units[i])
                    )
                    all_tests.append(
                        TestCase(
        ...
        logger.info(
            f"TestingSwarm complete: {len(all_tests)} tests, {estimated_coverage:.1f}% coverage"
        )
