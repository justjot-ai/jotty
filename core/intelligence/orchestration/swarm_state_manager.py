...
class AgentStateTracker:
    ...
    def __init__(self, agent_name: str) -> None:
        ...
        self.tool_usage: Dict[str, Dict[str, int]] = {"successful": {}, "failed": {}}
    ...
    def record_tool_call(self, tool_name: str, success: bool, metadata: Dict = None) -> None:
        ...
        if success:
            self.tool_usage["successful"][tool_name] = (
                self.tool_usage["successful"].get(tool_name, 0) + 1
            )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tool_success_rate": (
                ...
                if self.stats["total_tool_calls"] > 0
                else 0.0
            ),
        }
...
class SwarmStateManager:
    ...
    def __init__(
        ...
        self.swarm_tool_usage: Dict[str, Dict[str, int]] = {"successful": {}, "failed": {}}
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        state = {
            ...
            "task_progress": {
                ...
                "pending": len(
                    [
                        t
                        for t in self.swarm_task_board.subtasks.values()
            ...
            "recent_outcomes": [t.get("success", False) for t in self.swarm_trajectory[-5:]],
        }
        ...
        if self.shared_context:
            # SharedContext has .get() method
            # Tables
            tables = self.shared_context.get("table_names") or self.shared_context.get(
                "relevant_tables"
            )
            if tables:
            ...
            # Filters
            filters = self.shared_context.get("filters") or self.shared_context.get(
                "filter_conditions"
            )
            if filters:
    ...
    def get_available_actions(self) -> List[Dict[str, Any]]:
        ...
        for name, config in self.agents.items():
            actions.append(
                {"agent": name, "action": "execute", "enabled": getattr(config, "enabled", True)}
            )
        return actions
    ...
    def save_state(self, file_path: Path) -> None:
        ...
        state_data = {
            ...
            "agent_states": {
                name: tracker.get_state() for name, tracker in self.agent_trackers.items()
            },
            "timestamp": datetime.now().isoformat(),
        }
