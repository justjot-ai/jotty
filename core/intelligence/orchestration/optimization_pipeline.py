...
class OptimizationConfig:
    ...
    improvements_file: Optional[Path] = (
        None  # Path to save improvements (default: output_path/improvements.json)
    )
    update_dspy_instructions: bool = (
        False  # Update DSPy module instructions (if agent is DSPy module)
    )
    update_jotty_instructions: bool = (
        False  # Update Jotty learned_instructions (if using Orchestrator)
    )
...
class OptimizationPipeline:
    ...
    async def _run_agent_pipeline(
        self, task: str, context: Dict[str, Any], previous_outputs: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if self.conductor:
            ...
            try:
                result = await self.conductor.arun(goal=task, **context)
        ...
        main_pipeline_agents = [
            ...
            if not (
                "teacher" in agent_config.name.lower()
                or (agent_config.metadata and agent_config.metadata.get("is_teacher", False))
                or "kb" in agent_config.name.lower()
                or "knowledge" in agent_config.name.lower()
                or (agent_config.metadata and agent_config.metadata.get("is_kb_updater", False))
            )
        ]

        if not main_pipeline_agents:
            raise ValueError(
                "No agents available for main pipeline (all agents are teachers/KB updaters)"
            )
        ...
        for i, agent_config in enumerate(main_pipeline_agents):
            ...
            try:
                ...
                agent_inputs = self._prepare_agent_inputs(
                    agent_config, current_context, intermediate_outputs
                )
                ...
                if self.config.enable_thinking_log:
                    self._write_thinking_log(
                        f"Extracted output from {agent_name}: {repr(str(output_value)[:100])}"
                    )
        ...
        if main_pipeline_agents:
            ...
            if self.config.enable_thinking_log:
                self._write_thinking_log(
                    f"Final output from {last_agent_name}: {repr(str(final_output)[:100]) if final_output else 'None'}"
                )
    ...
    async def _evaluate_output(
        self, output: Any, gold_standard: Any, task: str, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if self.config.evaluation_function:
            try:
                result = await self.config.evaluation_function(
                    output=output, gold_standard=gold_standard, task=task, context=context
                )
                if isinstance(result, dict):
                    ...
                    return {
                        ...
                        "status": "CORRECT" if result == 1.0 else "INCORRECT",
                    }
            except Exception as e:
    ...
    async def _run_teacher_model(
        ...
        teacher_agent = None
        for agent_config in self.agents:
            is_teacher = "teacher" in agent_config.name.lower() or (
        ...
        try:
            ...
            eval_feedback = evaluation_result.get("feedback", "") or evaluation_result.get(
                "status", "INCORRECT"
            )
            teacher_context = {
                **context,
            ...
            if teacher_output_value:
                ...
                is_evaluation_text = any(
                    word in teacher_output_str.lower()
                    for word in [
                        "evaluation",
                        "analysis",
                        "correct",
                ...
                if is_evaluation_text:
                    logger.warning(
                        "Teacher returned evaluation text instead of diagram code, extracting or using gold_standard"
                    )
                    # Try to extract PlantUML code first
                    plantuml_match = re.search(
                        r"@startuml.*?@enduml", teacher_output_str, re.DOTALL | re.IGNORECASE
                    )
                    if plantuml_match:
                    ...
                    teacher_output_str = re.sub(
                        r"^```\s*$", "", teacher_output_str, flags=re.MULTILINE
                    )
                    teacher_output_value = teacher_output_str.strip()

                    # If still looks like evaluation text, use gold_standard
                    if any(
                        word in teacher_output_value.lower()
                        for word in ["evaluation", "analysis", "assessment"]
                    ):
            ...
            if teacher_output_value and self.config.enable_teacher_quality_check:
                teacher_output_value = self._validate_teacher_output(
                    teacher_output_value, student_output, gold_standard, evaluation_result
                )

            if teacher_output_value:
                self._write_thinking_log(
                    f"Teacher model provided output: {str(teacher_output_value)[:100]}..."
                )
    ...
    async def _update_knowledge_base(
        ...
        try:
            ...
            return {
                ...
                "updates": (
                    kb_result_value
                    if isinstance(kb_result_value, dict)
                    else {"result": kb_result_value}
                ),
            }
    ...
    async def optimize(
        ...
        self._write_thinking_log(
            f"Max iterations: {self.config.max_iterations}, Required passes: {self.config.required_pass_count}"
        )
        ...
        while self.iteration_count < self.config.max_iterations:
            ...
            try:
                ...
                if previous_outputs.get("teacher_output"):
                    ...
                    self._write_thinking_log(
                        f"Passing teacher output to agent: {str(previous_outputs.get('teacher_output'))[:50]}"
                    )

                # Run agent pipeline
                pipeline_result = await self._run_agent_pipeline(
                    task=task, context=context, previous_outputs=previous_outputs
                )
                    ...
                    if self.config.enable_teacher_model:
                        ...
                        if teacher_output:
                            ...
                            try:
                                ...
                                if (
                                    kb_updates
                                    and isinstance(kb_updates, dict)
                                    and kb_updates.get("status") == "completed"
                                ):
                                    self._write_thinking_log(
                                        "Knowledge base updated, context refreshed for next iteration"
                                    )
                                ...
                                if teacher_output and eval_score < 1.0:
                                    ...
                                    if improvement:
                                        self._write_thinking_log(
                                            f"Improvement recorded: {improvement.get('improvement_type', 'unknown')}"
                                        )

                                        # Record credit assignment if enabled
                                        if self.credit_assignment:
                                            teacher_eval = evaluation_result.get(
                                                "teacher_evaluation", {}
                                            )
                                ...
                                self._write_thinking_log(
                                    f"Error processing teacher output: {kb_error}"
                                )
    ...
    def _save_improvements_summary(self, final_result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._write_thinking_log(
                f"Saved {len(self.improvements)} improvements to {summary_file}"
            )
        except Exception as e:
    ...
    def _record_improvement(
        ...
        improvement = {
            ...
            or evaluation_result.get("error_info"),
        ...
        # Method 2: Shared memory from conductor
            ...
            self.conductor
            and hasattr(self.conductor, "shared_memory")
            and self.conductor.shared_memory
        ):
            ...
            try:
                ...
                if hasattr(self, "expert_name") and hasattr(self, "expert_domain"):
                    ...
                    if domain == "general" and hasattr(self, "current_context"):
                        context_domain = (
                            self.current_context.get("domain")
                ...
                logger.info(
                    f"Stored improvement to memory system (domain: {domain}, expert: {expert_name})"
                )
            except Exception as e:
    ...
    def _extract_learned_pattern(self, student_output: Any, teacher_output: Any, task: str) -> str:
        """
    ...
    def _validate_teacher_output(
        ...
        if gold_len > 0:
            ...
            if length_ratio < 0.3 or length_ratio > 3.0:
                logger.warning(
                    f"Teacher output length ratio {length_ratio:.2f} suspicious, rejecting"
                )
                return None
        ...
        teacher_lower = teacher_str.lower()
        if any(
            word in teacher_lower
            for word in ["evaluation", "analysis", "assessment", "correct", "incorrect"]
        ):
            # If it's mostly evaluation text, reject
            if (
                len([w for w in ["evaluation", "analysis", "assessment"] if w in teacher_lower])
                >= 2
            ):
        ...
        try:
            if self.conductor and hasattr(self.conductor, "learned_instructions"):
                ...
                if len(self.conductor.learned_instructions["actor"]) > 10:
                    self.conductor.learned_instructions["actor"] = (
                        self.conductor.learned_instructions["actor"][-10:]
                    )
