...
class WorkflowExecutor:
    ...
    async def execute(
        self, goal: str, context: Optional[Dict[str, Any]] = None, max_iterations: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(self.conductor, "run"):
                result = await self.conductor.run(
                    goal=goal, context=context, max_iterations=max_iterations
                )
            else:
            ...
            # Update task status
            self.context.update_task_status(task_id, TaskStatus.COMPLETED, result=result)
            ...
            # Update task status
            self.context.update_task_status(task_id, TaskStatus.FAILED, error=str(e))
    ...
    async def _execute_sequential(self, max_iterations: int) -> Any:
        ...
        while iteration < max_iterations:
            ...
            for task in ready_tasks:
                ...
                # Prepare context
                agent_context = self.orchestrator.prepare_agent_context(task, self.context, {})

                # Execute agent
                try:
                    result = await self.conductor.run_actor(
                        actor_name=agent_id, goal=task.goal, context=agent_context
                    )

                    # Update task
                    self.context.update_task_status(task.id, TaskStatus.COMPLETED, result=result)

                    results[task.id] = result

                except Exception as e:
                    logger.error(f"Task {task.id} failed: {e}")
                    self.context.update_task_status(task.id, TaskStatus.FAILED, error=str(e))
