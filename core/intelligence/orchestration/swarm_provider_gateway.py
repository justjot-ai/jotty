        ...
        try:
            ...
            if in_async_context:
                    ...
                    if not self._configured_lm:
                        ...
                        for provider_name, env_key in api_providers:
                            if os.getenv(env_key):
                                try:
                                    self._configured_lm = UnifiedLMProvider.create_lm(
                                        provider=provider_name
                                    )
                                    logger.info(
                                        f" SwarmProviderGateway (async): Using {provider_name} API"
                                    )
                                    break
                                except Exception:
                                    continue
                ...
                if self._configured_lm:
                    ...
                    logger.info(
                        f" SwarmProviderGateway configured (async-safe): {provider_name}/{model_name}"
                    )
            else:
                ...
                try:
                    ...
                    with dspy.context():
                        if hasattr(dspy.settings, "lm") and dspy.settings.lm:
                            ...
                            logger.info(
                                " SwarmProviderGateway using existing DSPy LM configuration (async)"
                            )
                except RuntimeError:
