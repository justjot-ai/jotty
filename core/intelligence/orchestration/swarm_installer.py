...
class SwarmInstaller:
    ...
    async def install(self, package: str, package_type: Optional[str] = None) -> InstallationResult:
        """
        ...
        if self._is_builtin_python_module(package):
            ...
            return InstallationResult(
                package=package, success=True, method="builtin", version="built-in"
            )
        ...
        result = InstallationResult(
            package=package, success=False, method="unknown", error=error_msg
        )
    ...
    async def _try_install_skill(self, skill_name: str) -> InstallationResult:
        ...
        return InstallationResult(
            package=skill_name, success=False, method="skill", error="Skill not found in registry"
        )
    ...
    async def _install_with_pip(self, package: str) -> InstallationResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return InstallationResult(
                    package=package, success=True, method="pip", version=version
                )
            else:
                error = result.stderr or result.stdout
                logger.error(f" Pip installation failed: {error}")
                return InstallationResult(package=package, success=False, method="pip", error=error)
        except subprocess.TimeoutExpired:
            return InstallationResult(
                package=package, success=False, method="pip", error="Installation timeout"
            )
        except Exception as e:
            logger.error(f" Pip installation error: {e}")
            return InstallationResult(package=package, success=False, method="pip", error=str(e))
    ...
    async def _install_with_npm(self, package: str) -> InstallationResult:
        ...
        try:
            ...
            result = subprocess.run(
                ["npm", "install", package], capture_output=True, text=True, timeout=300
            )

            if result.returncode == 0:
                logger.info(f" Installed {package} with npm")
                return InstallationResult(package=package, success=True, method="npm")
            else:
                error = result.stderr or result.stdout
                logger.error(f" NPM installation failed: {error}")
                return InstallationResult(package=package, success=False, method="npm", error=error)
        except FileNotFoundError:
            return InstallationResult(
                package=package, success=False, method="npm", error="npm not found"
            )
        except subprocess.TimeoutExpired:
            return InstallationResult(
                package=package, success=False, method="npm", error="Installation timeout"
            )
        except Exception as e:
            logger.error(f" NPM installation error: {e}")
            return InstallationResult(package=package, success=False, method="npm", error=str(e))
    ...
    async def ensure_skill_ready(self, skill_name: str) -> bool:
        ...
        if failed:
            logger.warning(
                f" Some dependencies failed for {skill_name}: {[r.package for r in failed]}"
            )
            return False
