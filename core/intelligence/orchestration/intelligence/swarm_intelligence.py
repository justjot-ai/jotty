...
from .morph_scoring import MorphScorer, MorphScores
from .swarm_data_structures import (  # arXiv swarm enhancements
    AgentProfile,
    AgentSession,
    AgentSpecialization,
    AuctionBid,
...
from ..learning.benchmarking import SwarmBenchmarks, SwarmMetrics
from ..learning.byzantine_verification import ByzantineVerifier, ConsistencyChecker
from ..learning.metrics_collector import MetricsCollector
from ..learning.stigmergy import StigmergyLayer, StigmergySignal
from ._consensus_mixin import ConsensusMixin
from ._morph_mixin import MorphMixin
from .curriculum_generator import CurriculumGenerator, SyntheticTask
...
class SwarmIntelligence:
    ...
    def get_training_task(
        ...
        if tool_aware:
            return self.curriculum_generator.generate_tool_aware_task(
                profiles=self.agent_profiles, target_agent=target_agent, prefer_weak_tools=True
            )
        else:
            return self.curriculum_generator.generate_training_task(
                profiles=self.agent_profiles, target_agent=target_agent
            )
    ...
    def record_task_result(
        ...
        with self._state_lock:
            ...
            self.adaptation_buffer.append(
                {"agent": agent_name, "task_type": task_type, "success": success}
            )
    ...
    def get_specialization_summary(self) -> Dict[str, str]:
        ...
        return {name: profile.specialization.value for name, profile in self.agent_profiles.items()}
    ...
    def get_best_agent_for_task(
        ...
        if use_morph_scoring and task_description and self.morph_scorer:
            best = self.morph_scorer.get_best_agent_by_tras(
                profiles=profiles, task=task_description, task_type=task_type, min_rcs=min_rcs
            )
            if best:
        ...
        if route_signals:
            ...
            if available_signals:
                best_from_stigmergy = max(
                    available_signals.keys(), key=lambda a: available_signals[a]
                )
    ...
    def _perform_online_adaptation(self) -> None:
        ...
        for agent_name, results in recent_by_agent.items():
            ...
            if profile and recent_rate < struggle_threshold and len(results) >= 3:
                ...
                logger.info(
                    f"Online adaptation: {agent_name} struggling ({recent_rate:.0%}), may need different task types"
                )
    ...
    def deposit_success_signal(
        self, agent: str, task_type: str, execution_time: float = 0.0
    ) -> None:
        """
    ...
    def save(self, path: str) -> None:
        ...
        logger.info(
            f"Saved swarm intelligence: {len(self.agent_profiles)} profiles, {len(self.stigmergy.signals)} stigmergy signals, curriculum tasks={self.curriculum_generator.total_generated}"
        )
        ...
        try:
            ...
            for name, p_data in data.get("agent_profiles", {}).items():
                try:
                    ...
                    logger.warning(
                        f"Unknown specialization '{p_data.get('specialization')}' for '{name}', defaulting to GENERALIST"
                    )
                ...
                try:
                    profile = AgentProfile(
                        ...
                        task_success={
                            ...
                            for k, v in p_data.get("task_success", {}).items()
                        },
            ...
            self.collective_memory = deque(
                data.get("collective_memory", []), maxlen=self.collective_memory_limit
            )
            ...
            if "curriculum" in data:
                self.curriculum_generator = CurriculumGenerator.from_dict(
                    data["curriculum"], self.config
                )
            ...
            logger.info(
                f"Loaded swarm intelligence: {len(self.agent_profiles)} profiles, {len(self.stigmergy.signals)} stigmergy signals, curriculum tasks={self.curriculum_generator.total_generated}"
            )
            return True
