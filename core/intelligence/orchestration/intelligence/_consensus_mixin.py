...
class ConsensusMixin:
    ...
    def _tally_weighted(self, votes: List[ConsensusVote]) -> Tuple[str, float]:
        """Original: weight = confidence × trust, with optional Byzantine verification."""
        vote_weights: Dict[str, float] = defaultdict(float)

        # Get Byzantine verifier if available (injected by SwarmLearningPipeline)
        byzantine = getattr(self, "_byzantine_verifier", None)
        ...
        for vote in votes:
            ...
            # Byzantine verification: penalize unverified claims
            if byzantine:
                try:
                    verified = byzantine.verify_claim(
                        agent=vote.agent_name,
                        claimed_success=True,
                        actual_result={"decision": vote.decision, "reasoning": vote.reasoning},
                        task_type="consensus",
                    )
                    if isinstance(verified, dict) and not verified.get("verified", True):
                        confidence *= 0.3  # Penalize unverified claims
                        logger.debug(
                            f"Byzantine: penalized {vote.agent_name}'s consensus vote "
                            f"(confidence {vote.confidence:.2f} -> {confidence:.2f})"
                        )
                except Exception as e:
                    logger.debug(f"Byzantine consensus check skipped for {vote.agent_name}: {e}")

            vote_weights[vote.decision] += confidence * trust
