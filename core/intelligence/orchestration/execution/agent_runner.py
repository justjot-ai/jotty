...
from Jotty.core.intelligence.reasoning.tools.inspector import (  # type: ignore[import]
    MultiRoundValidator,
    ValidatorAgent,
)
...
class AgentRunner:
    ...
    def __init__(
        ...
        if swarm_terminal is None and config.enable_terminal:
            try:
                from ..state.swarm_terminal import SwarmTerminal
        ...
        # Learning components (injected via inject_learning)
        self._stigmergy = None
        self._byzantine_verifier = None
    ...
    def inject_learning(self, components: Dict[str, Any]) -> None:
        """Receive learning subsystems from SwarmLearningPipeline.

        Called by Orchestrator after building runners so that post-execution
        hooks can write to stigmergy, byzantine verifier, MAS learning, and
        metrics collector without the agent_runner importing those modules.
        """
    ...
    async def _execute_agent(self, ctx: ExecutionContext) -> ExecutionContext:
        ...
        _cache_key = None
        if self.swarm_intelligence and hasattr(self.swarm_intelligence, "get_cached"):
            try:
                import hashlib

                _cache_key = hashlib.md5(
                    f"{self.agent_name}:{ctx.enriched_goal[:200]}".encode()
                ).hexdigest()
                cached = self.swarm_intelligence.get_cached(_cache_key)
                if cached is not None:
                    logger.info(f"Cache hit for {self.agent_name}: reusing previous result")
                    ctx.agent_output = cached
                    ctx.inner_success = True
                    ctx.trajectory = [{"step": 1, "action": "cache_hit", "success": True}]
                    ctx.task_progress.complete_step(2)
                    return ctx
        ...
        if (
            ctx.inner_success
            and _cache_key
            and self.swarm_intelligence
            and hasattr(self.swarm_intelligence, "cache_result")
        ):
            try:
                self.swarm_intelligence.cache_result(
                    _cache_key, ctx.agent_output, ttl=1800.0  # 30 min TTL
                )
            except Exception as cache_err:
    ...
    async def _record_and_build_result(self, ctx: ExecutionContext) -> EpisodeResult:
        ...
        pass  # workspace checkpoint removed (dead code)
        ...
        # Post-execution learning: activate ALL learning subsystems
        await self._post_execution_learning(
            agent_name=self.agent_name,
            task_type=ctx.kwargs.get("task_type", "unknown"),
            goal=ctx.goal,
            output=ctx.agent_output,
            success=ctx.success,
            quality=getattr(episode_result, "quality_score", 0.8 if ctx.success else 0.2),
            duration=ctx.duration,
        )
    ...
    async def _post_execution_learning(
        ...
        """Central post-execution hook — activates ALL learning subsystems."""
        try:
            # 1. Stigmergy: Record outcome for emergent routing
            if self._stigmergy:
                self._stigmergy.record_outcome(
                    agent=agent_name,
                    task_type=task_type,
                    success=success,
                    quality=quality,
            ...
            # 2. Byzantine: Verify output quality (catches agents that lie about success)
            if self._byzantine_verifier and success:
                try:
                    verification = self._byzantine_verifier.verify_output_quality(
                        agent=agent_name,
                    ...
                    if hasattr(verification, "passed") and not verification.passed:
                        logger.info(
                            f"Byzantine verification overrode success claim for {agent_name}"
                        )
                        success = False
                except Exception as e:
                    logger.debug(f"Byzantine verification skipped: {e}")

            # 3. MAS Learning: Record session for strategy recommendations
            if self._mas_learning:
                try:
                    self._mas_learning.record_session(
            ...
            # 4. Metrics: Record for observability
            if self._metrics:
                try:
    ...
    async def _handle_execution_error(
        ...
        if isinstance(error, (AgentExecutionError, ToolExecutionError)):
            ...
            # Try auto-recovery from MAS Learning fix database
            if self._mas_learning:
                try:
                    fix = self._mas_learning.find_fix(error_str)
                    if fix and fix.success_rate >= 0.5:
                        logger.info(
                            f"MAS learning found fix: {fix.solution_description[:100]} "
                            f"(success_rate={fix.success_rate:.0%})"
                        )
                        # Record that we're using this fix
                        ctx.kwargs.setdefault("learning_context", "")
                        ctx.kwargs["learning_context"] += (
                            f"\n[MAS Recovery] Previously solved similar error: "
                            f"{fix.solution_description[:200]}"
                        )
                except Exception as mas_err:
                    logger.debug(f"MAS auto-recovery lookup skipped: {mas_err}")
