...
class MorphMixin:
    ...
    def compute_morph_scores(
        ...
        """
        if not self.morph_scorer:  # type: ignore[attr-defined]
            return {}

        scores = self.morph_scorer.compute_all_scores(  # type: ignore[attr-defined]
            profiles=self.agent_profiles, task=task, task_type=task_type  # type: ignore[attr-defined]
        )

        self.morph_score_history.append(  # type: ignore[attr-defined]
            {
                "timestamp": time.time(),
                "scores": {
                    name: {"rcs": s.rcs, "rds": s.rds, "tras": s.tras} for name, s in scores.items()
                },
    ...
    def get_swarm_health(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            "agent_count": len(self.agent_profiles),  # type: ignore[attr-defined]
            "total_tasks": sum(p.total_tasks for p in self.agent_profiles.values()),  # type: ignore[attr-defined]
            "recommendations": [],
        }

        if not self.agent_profiles:  # type: ignore[attr-defined]
            health["recommendations"].append("No agents registered - add agents to swarm")  # type: ignore[attr-defined]
            return health

        if self.morph_scorer:  # type: ignore[attr-defined]
            rcs_scores = []
            for profile in self.agent_profiles.values():  # type: ignore[attr-defined]
                rcs, _ = self.morph_scorer.compute_rcs(profile)  # type: ignore[attr-defined]
                rcs_scores.append(rcs)
            health["avg_rcs"] = sum(rcs_scores) / len(rcs_scores) if rcs_scores else 0.5
            health["rds"] = self.morph_scorer.compute_rds(self.agent_profiles)  # type: ignore[attr-defined]

        trust_scores = [p.trust_score for p in self.agent_profiles.values()]  # type: ignore[attr-defined]
        health["avg_trust"] = sum(trust_scores) / len(trust_scores) if trust_scores else 0.5

        unique_specs = set(p.specialization for p in self.agent_profiles.values())  # type: ignore[attr-defined]
        health["specialization_coverage"] = len(unique_specs) / len(AgentSpecialization)

        if health["avg_rcs"] < 0.4:  # type: ignore[operator]
            health["recommendations"].append(  # type: ignore[attr-defined]
                "Low role clarity - consider warmup training to specialize agents"
            )
        if health["rds"] < 0.4:  # type: ignore[operator]
            health["recommendations"].append(  # type: ignore[attr-defined]
                "Low role differentiation - agents are too similar, consider diversifying"
            )
        if health["avg_trust"] < 0.5:  # type: ignore[operator]
            health["recommendations"].append(  # type: ignore[attr-defined]
                "Low average trust - some agents have inconsistent performance"
            )
        if health["total_tasks"] < 10:  # type: ignore[operator]
            health["recommendations"].append(  # type: ignore[attr-defined]
                "Limited task history - consider warmup() for self-training"
            )
        if not health["recommendations"]:
            health["recommendations"].append("Swarm health is good - no issues detected")  # type: ignore[attr-defined]

        return health

    def optimize_profiles_morph(
        self, num_iterations: int = 5, threshold: float = 0.1
    ) -> Dict[str, Any]:
        """
        ...
        if self.morph_scorer:  # type: ignore[attr-defined]
            results["initial_rds"] = self.morph_scorer.compute_rds(self.agent_profiles)  # type: ignore[attr-defined]
            rcs_scores = [self.morph_scorer.compute_rcs(p)[0] for p in self.agent_profiles.values()]  # type: ignore[attr-defined]
            results["initial_avg_rcs"] = sum(rcs_scores) / len(rcs_scores) if rcs_scores else 0.5

        prev_score = results["initial_rds"] + results["initial_avg_rcs"]  # type: ignore[operator]

        for iteration in range(num_iterations):
            results["iterations"] = iteration + 1

            low_rcs_agents = []
            for name, profile in self.agent_profiles.items():  # type: ignore[attr-defined]
                if self.morph_scorer:  # type: ignore[attr-defined]
                    rcs, components = self.morph_scorer.compute_rcs(profile)  # type: ignore[attr-defined]
                    if rcs < 0.5:
                ...
                if components.get("focus", 1.0) < 0.5:
                    ...
                    if profile.task_success:
                        best_type = max(
                            ...
                            key=lambda t: profile.task_success[t][0]
                            / max(1, profile.task_success[t][1]),
                        )
                        results["improvements"].append(  # type: ignore[attr-defined]
                            f"{agent_name}: Focus training on {best_type} (RCS: {rcs:.2f})"
                        )

            if self.morph_scorer:  # type: ignore[attr-defined]
                new_rds = self.morph_scorer.compute_rds(self.agent_profiles)  # type: ignore[attr-defined]
                new_rcs_scores = [
                    self.morph_scorer.compute_rcs(p)[0] for p in self.agent_profiles.values()  # type: ignore[attr-defined]
                ]
    ...
    def format_morph_report(self) -> str:
        ...
        lines = ["# MorphAgent Scores Report", "=" * 50, ""]
        ...
        if self.morph_scorer:  # type: ignore[attr-defined]
            rds = self.morph_scorer.compute_rds(self.agent_profiles)  # type: ignore[attr-defined]
        ...
        for name, profile in self.agent_profiles.items():  # type: ignore[attr-defined]
            if self.morph_scorer:  # type: ignore[attr-defined]
                ...
                lines.append(
                    f"      Focus: {components.get('focus', 0):.2f}, "
