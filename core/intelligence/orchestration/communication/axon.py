...
from Jotty.core.infrastructure.utils.tokenizer import (
    SmartTokenizer,  # type: ignore[import-not-found]
)
...
class SmartAgentSlack:
    ...
    def register_agent(
        ...
        if capabilities:
            ...
            caps = self._extract_capabilities_from_signature(
                agent_name=agent_name, signature=signature, max_context=max_context
            )
        else:
    ...
    def send(
        ...
        if target_caps.needs_compression(target_context):
            logger.info(
                f" {to_agent}'s context at {target_context}/{target_max} ({int(target_context/target_max*100)}%)"
            )
            if self.compressor:
        ...
        if not target_caps.can_accept_format(current_format):
            ...
            logger.info(
                f" [AUTO] Format mismatch: '{current_format}' not in {target_caps.acceptable_formats}"
            )
            logger.info(
                f" [AUTO] Invoking internal Transformer: {current_format} → {target_format}"
            )
        ...
        if success:
            ...
            self.context_budget_tracker[to_agent] = (
                self.context_budget_tracker.get(to_agent, 0) + data_tokens
            )
    ...
    def broadcast(
        ...
        targets = [name for name in self.agent_capabilities if name not in exclude_set]
        ...
        logger.info(
            f" [AGENT SLACK] {from_agent} broadcast to " f"{delivered}/{len(targets)} agents"
        )
        return results
    ...
    def _compress_agent_context(self, agent_name: str) -> None:
        ...
        try:
            ...
            logger.info(
                f" Compressed {agent_name} context: {original_tokens} → {new_tokens} tokens"
            )
        except Exception as e:
            logger.error(f" Compression failed: {e}")

    def _extract_capabilities_from_signature(
        self, agent_name: str, signature: dspy.Signature, max_context: int
    ) -> AgentCapabilities:
        """
    ...
    def _record_cooperation_event(
        self, event_type: str, from_agent: str, to_agent: str, description: str, impact: float
    ) -> Any:
        """
    ...
    def should_communicate(
        ...
        learned_value = (
            information_value * (0.5 + 0.5 * historical_success) * (0.5 + 0.5 * confidence_gap)
        )
        ...
        if should_send:
            logger.debug(
                f" [NASH] {from_agent} → {to_agent}: SEND (learned_value={learned_value:.2f}, learned_cost={learned_cost:.2f}, history_success={historical_success:.2f})"
            )
        else:
            logger.debug(
                f" [NASH] {from_agent} → {to_agent}: SKIP (learned_value={learned_value:.2f}, learned_cost={learned_cost:.2f}, history_success={historical_success:.2f})"
            )
