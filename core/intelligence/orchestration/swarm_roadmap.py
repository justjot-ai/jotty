    ...
    def to_llm_summary(self) -> str:
        ...
        if self.predicted_outcome:
            summary_parts.append(
                f"Prediction: {self.predicted_outcome} (conf: {self.action_confidence:.2f})"
            )
...
class DecomposedQFunction:
    ...
    def get_action_ranking(
        self, state: AgenticState, actions: List[str]
...
class SubtaskState:
    ...
    def fail(self, error: str) -> None:
        ...
        if self.attempts >= self.max_attempts:
            ...
            logger.error(
                f"Task {self.task_id} FAILED (attempts={self.attempts}, max={self.max_attempts})"
            )
        else:
            self.status = TaskStatus.PENDING  # Retry
            logger.warning(
                f"Task {self.task_id} will RETRY (attempts={self.attempts}, max={self.max_attempts})"
            )
        self.error = error
...
class SwarmTaskBoard:
    ...
    def get_next_task(
        ...
        logger.info(
            f" Task statuses: {', '.join([f'{t.task_id}={t.status.name}' for t in self.subtasks.values()])}"
        )
        ...
        logger.info(
            f" [get_next_task] q_predictor={q_predictor is not None}, state={current_state is not None}, goal={goal is not None}, epsilon={epsilon}"
        )
        ...
        if q_predictor and current_state and goal:
            ...
            if rand_value < epsilon:
                ...
                logger.info(
                    f" [get_next_task] EXPLORE mode (rand={rand_value:.3f} < eps={epsilon:.3f}) → selected {selected_task.actor}"
                )
                return selected_task
            else:
                logger.debug(f"EXPLOIT ({rand_value:.2f} >= {epsilon})")
                logger.info(
                    f" [get_next_task] EXPLOIT mode (rand={rand_value:.3f} >= eps={epsilon:.3f})"
                )
                ...
                for task in available_tasks:
                    ...
                    try:
                        ...
                        q_values_debug.append(
                            f"{task.actor}={q_value:.3f}"
                            if q_value is not None
                            else f"{task.actor}=None"
                        )
                        ...
                        logger.warning(
                            f" [get_next_task] Q-prediction failed for {task.actor}: {e}"
                        )
                        pass

                logger.debug(f"Q-values: {', '.join(q_values_debug)}")
                logger.debug(
                    f"Best task: {best_task.actor if best_task else 'None'} (Q={best_q_value:.3f})"
                )
                logger.info(f" [get_next_task] Q-values: {', '.join(q_values_debug)}")
                logger.info(
                    f" [get_next_task] Best task: {best_task.actor if best_task else 'None'} (Q={best_q_value:.3f})"
                )
    ...
    def checkpoint(self) -> Dict:
        ...
        checkpoint = {
            ...
            "subtask_states": {
                tid: {"status": t.status.value, "progress": t.progress, "attempts": t.attempts}
    ...
    def should_replan(
        ...
        if time_remaining <= 0:
            return (
                True,
                f"DEADLINE_EXCEEDED: elapsed={elapsed_time:.1f}s > deadline={global_deadline:.1f}s",
            )
        ...
        if time_ratio > 0.5 and progress < time_ratio * success_threshold:
            return (
                True,
                f"BEHIND_SCHEDULE: progress={progress:.1%} < expected={time_ratio * success_threshold:.1%}",
            )
    ...
    def replan(self, observation: str = "") -> List[str]:
        ...
        if pending:
            ...
            pending.sort(key=lambda t: t.priority * (1.0 / (1.0 + len(t.depends_on))), reverse=True)
    ...
    def get_state_summary(self) -> str:
        ...
        if pending:
            ...
            sorted_pending = sorted(
                pending, key=lambda t: t.priority * t.estimated_reward, reverse=True
            )
...
class ThoughtLevelCredit:
    ...
    def assign_credit(
        ...
        for step_idx in decision_steps:
            credits[step_idx] = credits.get(step_idx, 0) + outcome * self.decision_weight / max(
...
class StateCheckpointer:
    ...
    ) -> Tuple[AgenticState, DecomposedQFunction, SwarmTaskBoard]:
...
class TrajectoryPredictor:
    ...
    def predict(self, state: AgenticState, available_actions: List[str]) -> Dict:
    ...
    def evaluate_prediction_accuracy(
        self, prediction: Dict, actual_action: str, actual_outcome: str
    ) -> float:
