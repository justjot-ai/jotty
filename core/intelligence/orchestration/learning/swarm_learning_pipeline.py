...
class SwarmLearningPipeline:
    ...
    def _init_components(self) -> Any:
        ...
        from Jotty.core.intelligence.reasoning.tools.axon import (
            SmartAgentSlack,  # type: ignore[import-not-found]
        )
        ...
        # -----------------------------------------------------------------
        from ..intelligence.curriculum_generator import CurriculumGenerator
        ...
        # Metrics collector: observability for swarm performance
        from .metrics_collector import MetricsCollector

        self.metrics = MetricsCollector.get_global()

        # MAS Learning: multi-agent session recording and strategy recommendations
        try:
    ...
    # =========================================================================
    # Learning Components (dependency injection for hooks)
    # =========================================================================

    @property
    def learning_components(self) -> Dict[str, Any]:
        """Expose all learning subsystems for injection into agent_runner/orchestrator."""
        return {
    ...
    def _step_td_lambda(self, ctx: Any) -> Any:
        """TD-Lambda: update grouped value baselines via TD(0), record metrics."""
        self.td_learner.start_episode(ctx["goal"], task_type=ctx["task_type"])
        td_result = self.td_learner.update(
            state={"goal": ctx["goal"]},
        ...
        # Record learning convergence metrics
        if self.metrics:
            try:
                td_error = 0.0
                if isinstance(td_result, dict):
                    td_error = td_result.get("td_error", 0.0)
                elif isinstance(td_result, (int, float)):
                    td_error = float(td_result)
                self.metrics.record_task(
                    swarm="learning_pipeline",
                    agent=ctx["agent_name"],
                    task_type=f"td_update:{ctx['task_type']}",
                    success=ctx["result"].success,
                    duration=0.0,
                )
            except Exception as e:
                logger.debug(f"Metrics recording for TD step failed: {e}")
    ...
    def learn_from_result(
        ...
        """Learn from a successful execution result (records in stigmergy/metrics)."""
        if not result.success:
            return

        # Record in stigmergy if available
        if self.stigmergy:
            task_type = "unknown"
            if goal:
                try:
            ...
            self.stigmergy.record_outcome(
                ...
                task_type=task_type,
                success=True,
                quality=getattr(result, "quality_score", 0.8),
            )
        ...
        if self.metrics:
            self.metrics.record_task(
                swarm="orchestrator",
                agent=agent_config.name,
                task_type=goal[:50] if goal else "unknown",
                success=True,
                duration=getattr(result, "execution_time", 0.0),
            )
