...
class ParadigmExecutor:
    ...
    async def debate(self, goal: str, **kwargs: Any) -> EpisodeResult:
        ...
        # Consistency check via Byzantine verifier (hallucination detection)
        if len(drafts) >= 2:
            try:
                learning = getattr(sm, "learning", None)
                byzantine = getattr(learning, "byzantine_verifier", None) if learning else None
                if byzantine and hasattr(byzantine, "consistency_checker"):
                    outputs_for_check = list(drafts.values())
                    consistency = byzantine.consistency_checker.check_consistency(outputs_for_check)
                    if hasattr(consistency, "outliers") and consistency.outliers:
                        for outlier_idx in consistency.outliers:
                            if outlier_idx < len(list(drafts.keys())):
                                outlier_name = list(drafts.keys())[outlier_idx]
                                if outlier_name in all_results:
                                    logger.info(
                                        f"Byzantine consistency: downweighting outlier "
                                        f"'{outlier_name}' in debate"
                                    )
                                    # Create modified result with reduced confidence
                                    outlier_result = all_results[outlier_name]
                                    if hasattr(outlier_result, "success"):
                                        outlier_result.success = False
            except Exception as e:
    ...
    def aggregate_results(self, results: Dict[str, EpisodeResult], goal: str) -> EpisodeResult:
        ...
        Uses SwarmIntelligence consensus voting (when available) for
        trust-weighted decision making, then falls back to CandidateVerifier.
        """
        sm = self._manager
        if not results:
        ...
        if len(results) >= 2:
            try:
                si = getattr(getattr(sm, "learning", None), "swarm_intelligence", None)
                if si and hasattr(si, "gather_consensus"):
                    # Build options from agent outputs
                    agent_names = list(results.keys())
                    options = agent_names  # Each agent is an "option"

                    def _vote_func(agent_name: str, question: str, options: list) -> tuple:
                        """Vote for self (each agent votes for their own output)."""
                        r = results.get(agent_name)
                        confidence = 0.8 if (r and r.success) else 0.3
                        reasoning = f"Output quality: {'good' if r and r.success else 'poor'}"
                    ...
                    try:
                        ...
                        # Can't await in sync method — use SI's cached profiles
                        # for lightweight scoring instead
                    except RuntimeError:
                        pass

                    # Lightweight trust-weighted selection (no async needed)
                    best_agent = None
                    best_score = -1.0
                    for name, r in results.items():
                        si.register_agent(name)
                        profile = si.agent_profiles.get(name)
                        trust = profile.trust_score if profile else 0.5
                        quality = 0.8 if r.success else 0.2
                        score = trust * quality
                        if score > best_score:
                            best_score = score
                            best_agent = name
                    if best_agent and best_score > 0.3:
                        consensus_output = results[best_agent].output
                        logger.info(
                            f"SI consensus selected '{best_agent}' "
                            f"(trust-weighted score={best_score:.2f})"
                        )
            except Exception as e:
                logger.debug(f"SI consensus selection skipped: {e}")

        verified_output = consensus_output or sm._mas_zero_verify(goal, results)
