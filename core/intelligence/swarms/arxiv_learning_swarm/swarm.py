...
class ArxivLearningSwarm(DomainSwarm):
    ...
    async def learn(
        ...
        logger.info(" ArxivLearningSwarm starting...")
    ...
    def _build_output_data(self, result: ArxivLearningResult) -> Dict[str, Any]:
        content = getattr(result, "content", None)
        return {
            "concepts_count": getattr(result, "concepts_covered", 0),
            "bingo_moments": getattr(result, "bingo_moments", 0),
            "has_hook": bool(content and getattr(content, "hook", "")),
            "has_summary": bool(content and getattr(content, "summary", "")),
            "has_examples": getattr(result, "concepts_covered", 0) > 0,
            "word_count": getattr(content, "total_words", 0) if content else 0,
        }
    ...
    async def _execute_phases(
        ...
        if len(concepts) > max_concepts:
            logger.info(
                f"  Limiting concepts: {len(concepts)} -> {max_concepts} (depth={learning_depth.value})"
            )
        ...
        result = ArxivLearningResult(
            ...
            swarm_name=self.config.name,
            domain=self.config.domain,
            output={"title": paper.title, "arxiv_id": paper.arxiv_id, "content": final_content},
            execution_time=executor.elapsed(),
            paper=paper,
        ...
        try:
            if hasattr(dspy.settings, "lm") and hasattr(dspy.settings.lm, "get_metrics"):
                ...
                logger.info(
                    f" LLM Metrics: {metrics.get('successful_calls', 0)}/{metrics.get('total_calls', 0)} calls succeeded ({metrics.get('success_rate', 'N/A')})"
                )
    ...
    async def _generate_content(
        self, executor: Any, paper: Any, concepts: Any, learning_depth: Any, config: Any
    ) -> Dict[str, Any]:
    ...
    async def _content_parallel_deep(
        self, executor: Any, paper: Any, concepts: Any, config: Any
    ) -> Dict[str, Any]:
        """Parallel deep mode: full quality with parallel per-concept generation."""
        # Check cache first
        content_cache_key = (
            f"parallel_deep_{paper.arxiv_id}_{config.audience.value}_{len(concepts)}"
        )
        unified_result = None
        if config.use_swarm_cache:
    ...
    async def _content_parallel(
        ...
        logger.info(
            f"     {len(intuitions)} intuitions, {len(math_explanations)} math, {len(examples)} examples"
        )

        # Phase 6: Build content directly (SKIP ProgressiveBuilder to avoid 76KB prompt)
        draft_content = self._build_direct_content(
            paper, concepts, intuitions, math_explanations, examples, config
        )
        ...
        key_insights = [
            ...
            for c in concepts[:3]
            if c.name in intuitions and intuitions[c.name].get("aha_moment")
        ]
    ...
    async def _content_sequential(
        ...
        progressive_result = await executor.run_phase(
            ...
            input_data={
                ...
                "has_math": bool(math_explanations),
    ...
    async def _generate_outputs(self, executor: Any, paper: Any, learning_content: Any) -> tuple:
        ...
        html_path = (
            output_results[2]
            if not isinstance(output_results[2], dict) or "error" not in output_results[2]
            else None
        )
    ...
    async def _generate_pdf(self, paper: PaperInfo, content: LearningContent) -> Optional[str]:
        """Generate professional PDF with visualizations. Returns path or None."""
        try:
            from Jotty.core.capabilities.skills.research.learning_pdf_template import (
            ...
            viz_tasks = [
                generate_viz_for_concept(c) for c in content.concepts[:3]
            ]  # Top 3 concepts
            ...
            for s in content.sections:
                ...
                for concept_name, viz_path in visualization_paths.items():
                    if (
                        concept_name not in used_visualizations
                        and concept_name.lower() in s.title.lower()
                    ):
            ...
            # Generate PDF
            output_path = (
                f"/tmp/arxiv_{paper.arxiv_id.replace('.', '_').replace('/', '_')}_learning.pdf"
            )
    ...
    async def _generate_pptx(self, paper: PaperInfo, content: LearningContent) -> tuple:
        ...
        try:
            from Jotty.core.capabilities.skills.research.pptx_generator import (
                convert_pptx_to_pdf,
            ...
            output_path = (
                f"/tmp/arxiv_{paper.arxiv_id.replace('.', '_').replace('/', '_')}_presentation.pptx"
            )
            ...
            if pptx_path:
                ...
                if self.config.convert_pptx_to_pdf:
                    if is_libreoffice_available():
                        ...
                        logger.warning(
                            " PPTX-to-PDF conversion skipped (LibreOffice not installed)"
                        )
    ...
    async def _generate_html(self, paper: PaperInfo, content: LearningContent) -> Optional[str]:
        ...
        try:
            ...
            paper_data = {
                ...
                "learning_time": (
                    "10-15 min"
                    if content.total_words < 1000
                    else "20-30 min" if content.total_words < 2000 else "45-60 min"
                ),
            }

            # Generate HTML slides
            output_path = (
                f"/tmp/arxiv_{paper.arxiv_id.replace('.', '_').replace('/', '_')}_slides.html"
            )
        ...
        try:
            ...
            if pdf_path and Path(pdf_path).exists():
                file_result = await send_telegram_file_tool(
                    {"file_path": pdf_path, "caption": f" {paper.title[:50]} - Learning Guide"}
                )
...
async def learn_paper(
    ...
    return await swarm.learn(
        paper_id=paper_id, topic=topic, depth=depth_enum, send_telegram=send_telegram
    )
...
def learn_paper_sync(
    ...
    return asyncio.run(
        learn_paper(paper_id=paper_id, topic=topic, depth=depth, send_telegram=send_telegram)
    )
