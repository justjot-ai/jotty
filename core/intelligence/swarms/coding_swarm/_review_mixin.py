...
class ReviewMixin:
    ...
    async def _collaborative_design_loop(
        ...
        """
        _progress(
            "Phase 1", "CollaborativeDesign", f"Starting {max_iterations}-iteration design loop..."
        )
        ...
        for iteration in range(1, max_iterations + 1):
            _progress(
                "Phase 1", "CollaborativeDesign", f"=== Iteration {iteration}/{max_iterations} ==="
            )
            ...
            try:
                ...
                if not current_components:
                    current_components = _extract_components_from_text(
                        current_file_structure, current_architecture, current_interfaces
                    )
                ...
                if research_requests:
                    _progress(
                        "Phase 1",
                        "Architect",
                        f"Requesting research on: {research_requests[:100]}...",
                    )
            ...
            if iteration < max_iterations and getattr(config, "enable_research", True):
                ...
                try:
                    ...
                    if web_search_fn and search_queries:
                        for qi, query in enumerate(search_queries[:5]):
                            _progress(
                                "Phase 1.5",
                                "Researcher",
                                f"Searching ({qi+1}/{min(len(search_queries),5)}): {str(query)[:50]}",
                            )
                            try:
                                    ...
                                    for r in result["results"]:
                                        ...
                                        if snippet:
                                            ...
                                            if any(
                                                ...
                                                accumulated_research.best_practices.append(
                                                    f"{title}: {snippet}"
                                                )
                                            elif any(
                                                kw in query_lower
                                                for kw in ["doc", "api", "reference", "example"]
                                            ):
                                                accumulated_research.api_references.append(
                                                    f"{title}: {snippet}"
                                                )
                                            elif any(
                                                kw in query_lower
                                                ...
                                                accumulated_research.warnings.append(
                                                    f"{title}: {snippet}"
                                                )
                                            else:
                                                accumulated_research.library_docs.append(
                                                    f"{title}: {snippet}"
                                                )
                    ...
                    total_findings = (
                        len(accumulated_research.best_practices)
                    ...
                    _progress(
                        "Phase 1.5",
                        "Researcher",
                        f"Done -- {total_findings} total finding(s) accumulated",
                    )
        ...
        _progress(
            "Phase 1",
            "CollaborativeDesign",
            f"Done -- {len(current_components)} component(s), {total_research} research finding(s)",
        )
        ...
        if feedback_parts:
            ...
            try:
                if not hasattr(self, "_consolidation_module") or self._consolidation_module is None:
                    self._consolidation_module = dspy.ChainOfThought(
                        TeamPlanningConsolidationSignature
                    )
                ...
                _progress(
                    "Phase 2", "TeamPlanning", "Consolidation failed, using original architecture"
                )
        else:
            _progress(
                "Phase 2", "TeamPlanning", "No feedback to consolidate, using original architecture"
            )
    ...
    async def _team_review(
        ...
        if func_reviewers:
            ...
            if rejected:
                ...
                _progress(
                    "Phase 6",
                    "Optimizer",
                    f"Reworking code ({len(rejected)} rejection(s) in functional review)...",
                )
        ...
        if quality_reviewers:
            ...
            if rejected:
                ...
                _progress(
                    "Phase 6",
                    "Optimizer",
                    f"Reworking code ({len(rejected)} rejection(s) in quality review)...",
                )
                review_result["rework_attempts"] += 1
        ...
        _progress(
            "Phase 6",
            "TeamReview",
            f"Final verdict: {verdict_str} (rework attempts: {review_result['rework_attempts']})",
        )
