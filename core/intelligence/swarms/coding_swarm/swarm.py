...
class CodingSwarm(CodebaseMixin, EditMixin, ReviewMixin, PersistenceMixin, DomainSwarm):
    ...
    async def _generate_fullstack(
        ...
        lang = config.language.value if hasattr(config.language, "value") else str(config.language)
        files = {}
        ...
        if "error" not in integration_result:
            files["docker-compose.yml"] = _strip_code_fences(
                integration_result.get("docker_compose", "")
            )
            files[".env.example"] = _strip_code_fences(integration_result.get("env_config", ""))
            files["requirements.txt"] = _strip_code_fences(
                integration_result.get("requirements_txt", "")
            )
            files["start.sh"] = _strip_code_fences(integration_result.get("startup_script", ""))
            _progress(
                "Phase 2d",
                "IntegrationEngineer",
                "Done -- docker-compose.yml, .env.example, requirements.txt, start.sh",
            )
        else:
        ...
        # Stream summary
        _progress(
            "FullStack", "Pipeline", f"Generated {len(files)} file(s): {', '.join(files.keys())}"
        )
    ...
    async def generate(
        ...
        if is_trivial:
            ...
            _progress(
                "FastPath",
                "Detector",
                "Trivial task detected - using fast path (1 iteration, no team)",
            )
        ...
        self._run_input = {"requirements": requirements, "language": lang.value}
    ...
    def _build_output_data(self, result: CodingResult) -> Dict[str, Any]:
        return {
            "code": result.code.files if hasattr(result, "code") and result.code else {},
            "tests": result.code.tests if hasattr(result, "code") and result.code else {},
        }
    ...
    async def _execute_phases(
        ...
        arch_result, research_context = await executor.run_phase(
            ...
            self._collaborative_design_loop(
                ...
                constraints=json.dumps(
                    {"frameworks": config.frameworks, "max_file_size": config.max_file_size}
                ),
        ...
        if self._team_config and not skip_planning:
            _progress(
                "Phase 2", "TeamPlanning", f"Team planning session ({self._team_config.name})..."
            )
            ...
            if planning_result and planning_result.get("refined_architecture"):
                ...
                if planning_result.get("team_agreements"):
                    for line in str(planning_result["team_agreements"]).split("\n")[:5]:
                        if line.strip():
                            _progress(
                                "Phase 2", "TeamPlanning", f"  Agreement: {line.strip()[:70]}"
                            )
        ...
        self._trace_phase(
            ...
            success=len(files) > 0,
            phase_start=executor._start_time,
    ...
    async def _phase_single_tier_codegen(
        ...
        async def _generate_component(comp: Any) -> Any:
            ...
            if isinstance(result, dict) and "filename" in result:
                _progress(
                    "Phase 3",
                    "Developer",
                    f"  [{completed_count}/{total_components}] {result['filename']} ready",
                )
            else:
                _progress(
                    "Phase 3",
                    "Developer",
                    f"  [{completed_count}/{total_components}] {comp_name} failed",
                )
            return result

        code_results = await asyncio.gather(
            *[_generate_component(comp) for comp in components], return_exceptions=True
        )
        ...
        _progress(
            "Phase 3", "Developer", f"Done -- {len(files)} file(s): {', '.join(files.keys())}"
        )
    ...
    async def _phase_optimize(
        self, executor: Any, files: Dict[str, str], requirements: str
    ) -> Dict[str, str]:
        """Phase 4: Optimization (parallel, skip config files).
        ...
        if files_to_skip:
            _progress(
                "Phase 4",
                "Optimizer",
                f"Skipping {len(files_to_skip)} config file(s): {', '.join(files_to_skip.keys())}",
            )

        _progress(
            "Phase 4",
            "Optimizer",
            f"Optimizing {len(files_to_optimize)} code file(s) in parallel...",
        )
        ...
        async def _optimize_one(fname: str, code_str: str) -> Any:
            ...
            _progress(
                "Phase 4", "Optimizer", f"  [{optimized_count}/{total_files}] {fname} optimized"
            )
            return (
                fname,
                _strip_code_fences(opt_result.get("optimized_code", code_str)),
                improvements,
            )
        ...
        _progress(
            "Phase 4",
            "Optimizer",
            f"Done -- {len(files_to_optimize)} code file(s) optimized, {len(files_to_skip)} config file(s) kept as-is",
        )
            ...
            for attempt in range(max_fix):
                    ...
                    if run_result.success:
                        ...
                        if any(err in error_text for err in FIXABLE_ERRORS):
                            ...
                            _progress(
                                "Phase 4.5",
                                "Validator",
                                f"Non-fixable runtime condition (skipped): {error_text[:80]}",
                            )
                ...
                if syntax_ok and runtime_ok:
                    ...
                    _progress(
                        "Phase 4.5",
                        "Validator",
                        f"All files validated after {attempt+1} attempt(s)",
                    )
                    break
                elif not syntax_ok:
                    _progress(
                        "Phase 4.5",
                        "Validator",
                        f"Syntax errors fixed, re-validating (attempt {attempt+1}/{max_fix})...",
                    )
        ...
        async def _run_verification() -> Any:
            ...
            if verification_result and not verification_result.get("verified", True):
                ...
                if issues:
                    ...
                    _progress(
                        "Phase 5", "Verifier", f"Found {len(issues)} issue(s), attempting fix..."
                    )
                    for iss in issues[:5]:
                        if isinstance(iss, dict):
                            _progress(
                                "Phase 5",
                                "Verifier",
                                f"  [{iss.get('severity','?')}] {iss.get('description','')[:80]}",
                            )
    ...
    async def _phase_simplicity_judge(
        self, executor: Any, files: Dict[str, str], main_file: Optional[str], requirements: str
    ) -> Dict[str, Any]:
        """Phase 5.5: Simplicity judge (anti-over-engineering gate).
    ...
    async def _phase_documentation(
        self, executor: Any, files: Dict[str, str], arch_result: Dict[str, Any], gen_docs: bool
    ) -> str:
    ...
    def _build_coding_result(
        ...
        simplicity_score = (
            simplicity_result.get("simplicity_score", 1.0) if simplicity_result else 1.0
        )
        complexity_score = (
            1.0 - simplicity_score
        )  # 0=appropriately simple, 1=severely over-engineered
        ...
        result = CodingResult(
            ...
            output={
                ...
                "architecture": (
                    ...
                    else str(arch_result) if arch_result else ""
                ),
            },
            ...
            quality_score=(
                verification_result.get("coverage_score", 0.8) if verification_result else 0.8
            ),
        ...
        simplicity_verdict = (
            simplicity_result.get("verdict", "ACCEPT") if simplicity_result else "ACCEPT"
        )
        logger.info("=" * 56)
        logger.info(
            "DONE | %d LOC | %d file(s) | %d test(s) | %s",
            loc,
            len(files),
            len(tests),
            validated_str,
        ...
        logger.info(
            "Time: %.1fs | Quality: %.2f | Simplicity: %s",
            exec_time,
