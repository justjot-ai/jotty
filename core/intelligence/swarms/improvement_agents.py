...
class ExpertAgent(MetaAgent):
    ...
    async def evaluate(
        self, gold_standard_id: str, actual_output: Dict[str, Any], context: str = ""
    ) -> Evaluation:
        """
        ...
        result = await self.evaluate_against_gold(
            gold_id=gold_standard_id, output=actual_output, context=context
        )
...
class ReviewerAgent(MetaAgent):
    ...
    async def analyze_and_suggest(
        self, evaluations: List[Evaluation], agent_configs: Dict[AgentRole, SwarmAgentConfig]
                ...
                suggestions.append(
                    ImprovementSuggestion(
                        ...
                        improvement_type=ImprovementType(
                            s.get("improvement_type", "prompt_refinement")
                        ),
        ...
        try:
            ...
            result = await self.execute(
                ...
                improvement_suggestions=json.dumps(
                    [asdict(s) for s in (relevant_improvements or [])], default=str
                ),
            ...
            return {"plan": plan, "rationale": str(output.get("rationale", "")), "risks": risks}
...
class AuditorAgent(MetaAgent):
    ...
    async def audit_evaluation(
        self, evaluation: Dict[str, Any], output_data: Dict[str, Any], context: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"passed": True, "reasoning": f"Audit skipped (non-blocking): {str(e)}"}
...
class LearnerAgent(MetaAgent):
    ...
    async def extract_learnings_from_execution(
        ...
        return await self.extract_learnings(
            input_data=input_data, output_data=output_data, evaluation=evaluation, domain=domain
        )
...
class CollapsedEvaluator(MetaAgent):
    ...
    def __init__(
        self, config: SwarmAgentConfig, gold_db: GoldStandardDB, history: ImprovementHistory
    ) -> None:
        meta_config = MetaAgentConfig(
...
class CollapsedExecutor(MetaAgent):
    ...
    async def execute_and_reflect(
        ...
        if suggestions:
            combined_learnings.extend([f"[Improvement] {s.description}" for s in suggestions[:5]])

        # Get historical successful improvements
        successful = self.history.get_successful_improvements(AgentRole.ACTOR)
        combined_learnings.extend([s["suggestion"]["description"] for s in successful[:3]])
    ...
    def _extract_learnings_from_output(
        self, output: Dict[str, Any], confidence: float, task: str
    ) -> List[str]:
        """
        ...
        if confidence >= 0.8:
            learnings.append(
                f"High-confidence execution of '{task[:50]}': " f"approach produced reliable output"
            )
        elif confidence < 0.4:
            learnings.append(
                f"Low-confidence execution of '{task[:50]}': " f"consider alternative approaches"
            )

        if "error" in output:
            learnings.append(f"Avoid: {str(output['error'])[:100]}")
