...
class DataFetcherAgent(SwarmLearningAgent):
...
class WebSearchAgent(SwarmLearningAgent):
    ...
    async def search(self, ticker: str, max_results: int = 25) -> Dict[str, Any]:
        ...
        try:
            ...
            for query in queries:
                try:
                    if inspect.iscoroutinefunction(self._search_tool):
                        result = await self._search_tool(
            ...
            # Build news text
            news_text = "\n".join(
                [f"• {r.get('title', '')}: {r.get('snippet', '')[:200]}" for r in unique[:20]]
            )

            self._broadcast("web_search_complete", {"ticker": ticker, "count": len(unique)})

            return {"news_text": news_text, "news_items": unique[:20], "news_count": len(unique)}
        except Exception as e:
            logger.error(f"WebSearchAgent error: {e}")
            return {"news_text": "", "news_count": 0}

    async def search_topic(
        self, topic: str, sub_queries: Optional[List[str]] = None, max_results: int = 25
    ) -> Dict[str, Any]:
...
class SentimentAgent(SwarmLearningAgent):
    ...
    def __init__(
        self, memory: Any = None, context: Any = None, bus: Any = None, llm_module: Any = None
    ) -> None:
        super().__init__(memory, context, bus)
...
class PeerComparisonAgent(SwarmLearningAgent):
    ...
    async def compare(
        self, ticker: str, sector: str, industry: str, exchange: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            self._broadcast(
                "peer_comparison_complete", {"ticker": ticker, "peers": list(peer_data.keys())}
            )

            return {"peers": list(peer_data.keys()), "comparison": peer_data}
        except Exception as e:
    ...
    async def generate(self, ticker: str, data: Dict[str, Any], output_dir: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if volumes and len(volumes) == len(prices):
                ...
                colors = [
                    "#4CAF50" if i > 0 and prices[i] >= prices[i - 1] else "#F44336"
        ...
        if self._llm and result.get("timeframes"):
            try:
                indicator_summary = json.dumps(
                    {tf: data.get("indicators", {}) for tf, data in result["timeframes"].items()},
                    indent=2,
                )[:2000]
                ...
                result["summary"] = {
                    ...
                    "signal_strength": (
                        float(llm_result.signal_strength) if llm_result.signal_strength else 0.5
                    ),
                    "key_observations": [
                        obs.strip() for obs in str(llm_result.key_observations).split("|")
                    ],
                }
            except Exception as e:
...
class EnhancedChartGeneratorAgent(SwarmLearningAgent):
            ...
            for timeframe, tf_data in technical_data.get("timeframes", {}).items():
                try:
                    chart_path = await self._generate_multipanel_chart(
                        ticker, timeframe, output_path, timestamp, include_heiken_ashi
                    )
                    if chart_path:
    ...
    async def _generate_multipanel_chart(
        ...
        if "wma_9" in df.columns:
            apds.append(
                mpf.make_addplot(df["wma_9"], color="green", panel=current_panel, width=0.8)
            )
        if "wma_21" in df.columns:
            apds.append(
                mpf.make_addplot(df["wma_21"], color="orange", panel=current_panel, width=0.8)
            )
        ...
        apds.append(
            mpf.make_addplot(
                [ltp] * len(df), color="blue", panel=current_panel, alpha=0.3, width=0.5
            )
        )
        ...
        if "rsi" in df.columns:
            apds.append(
                mpf.make_addplot(
                    [45] * len(df), color="red", panel=current_panel, alpha=0.25, ylim=(0, 100)
                )
            )
            apds.append(
                mpf.make_addplot(
                    [55] * len(df), color="green", panel=current_panel, alpha=0.25, ylim=(0, 100)
                )
            )
            apds.append(
                mpf.make_addplot(df["rsi"], color="purple", panel=current_panel, ylim=(0, 100))
            )
            current_panel += 1
        ...
        if "macd" in df.columns and "macd_signal" in df.columns:
            ...
            if "macd_hist" in df.columns:
                ...
                apds.append(
                    mpf.make_addplot(
                        df["macd_hist"], type="bar", color=colors, panel=current_panel, alpha=0.5
                    )
                )
        ...
        try:
            last_date = df.index[-1].strftime(
                "%Y-%m-%d %H:%M" if "minute" in timeframe.lower() else "%Y-%m-%d"
            )
    ...
    def _add_chart_indicators_ta(self, df: "pd.DataFrame") -> "pd.DataFrame":
        ...
        try:
            ...
            supertrend[0] = (
                upper_band.iloc[0] if not np.isnan(upper_band.iloc[0]) else df["Close"].iloc[0]
            )
            ...
            for i in range(1, len(df)):
                if np.isnan(upper_band.iloc[i]) or np.isnan(lower_band.iloc[i]):
                    ...
                    supertrend[i] = (
                        ...
                        if supertrend[i - 1] > lower_band.iloc[i - 1]
...
class SocialSentimentAgent(SwarmLearningAgent):
    ...
    async def analyze(self, company: str, news_text: str, forum_text: str = "") -> Dict[str, Any]:
        ...
        result = {
            ...
            "key_themes": [],
            "sentiment_drivers": {"positive": [], "negative": []},
        }
        ...
        try:
            if self._llm:
                ...
                if llm_result.key_themes:
                    result["key_themes"] = [
                        t.strip() for t in str(llm_result.key_themes).split("|")
                    ][:5]
                ...
                if llm_result.sentiment_drivers:
                    ...
                    if "||" in drivers_str:
                        ...
                        if len(parts) >= 2:
                            result["sentiment_drivers"]["positive"] = [
                                d.strip() for d in parts[0].split("|") if d.strip()
                            ][:5]
                            result["sentiment_drivers"]["negative"] = [
                                d.strip() for d in parts[1].split("|") if d.strip()
                            ][:5]
            else:
