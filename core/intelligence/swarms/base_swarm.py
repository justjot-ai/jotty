...
class BaseSwarm(SwarmLearningMixin, ABC):
    ...
    def _init_shared_resources(self) -> None:
        ...
        try:
            ...
            from Jotty.core.modes.agent.base.base_agent import _dspy_lm_lock
    ...
    def connect_swarm_intelligence(
        self, swarm_intelligence: Any = None, enable_training: bool = False
    ) -> None:
        """
    ...
    def get_training_task(self, tool_aware: bool = True) -> None:
        ...
        return self._swarm_intelligence.get_training_task(
            target_agent=swarm_name, tool_aware=tool_aware
        )
    ...
    def _manage_tools(self) -> Dict:
        ...
        if replacements or removals:
            ...
            if add_tools or remove_tools:
                si.tool_manager.update_assignments(swarm_name, add=add_tools, remove=remove_tools)
    ...
    def _curate_gold_standard(
        self, task_type: Any, input_data: Any, output_data: Any, evaluation: Any
    ) -> None:
        ...
        logger.info(
            f"Auto-curated gold standard {gs_id} for {task_type} "
            f"(score={evaluation.overall_score:.2f})"
        )

    async def _evaluate_output(
        self, output: Dict[str, Any], task_type: str, input_data: Dict[str, Any]
    ) -> Optional[Evaluation]:
        """
    ...
    def _rule_based_evaluate(
        self, gold_standard: Any, output: Dict[str, Any], task_type: str
    ) -> Optional[Evaluation]:
        """
        ...
        try:
            ...
            expected = (
                gold_standard.expected_output if hasattr(gold_standard, "expected_output") else {}
            )
            ...
            has_error = any(
                k in output_str.lower() for k in ["error", "traceback", "exception", "failed"]
            )
            if has_error:
    ...
    def _form_coalition(
        ...
        if coalition:
            logger.info(
                f"Coalition formed: {coalition.coalition_id} with {len(coalition.members)} agents"
            )
    ...
    def _gossip_broadcast(self, message_type: str, content: Dict[str, Any]) -> None:
        ...
        return self._swarm_intelligence.gossip_broadcast(
            origin_agent=swarm_name, message_type=message_type, content=content
        )
    ...
    def _enqueue_task(
        ...
        if self._swarm_intelligence:
            self._swarm_intelligence.enqueue_task(
                task_id=task_id, task_type=task_type, priority=priority, context=context
            )
    ...
    def _decompose_task(
        ...
        return self._swarm_intelligence.decompose_task(
            task_id=task_id, task_type=task_type, subtasks=subtasks, parallel=parallel
        )
    ...
    def _aggregate_results(self, parent_task_id: str, results: Dict) -> Dict:
        ...
        return self._swarm_intelligence.aggregate_subtask_results(
            parent_task_id=parent_task_id, results=results
        )
    ...
    def _byzantine_vote(self, question: str, options: List[str], voters: List[str] = None) -> Dict:
        ...
        return self._swarm_intelligence.byzantine_vote(
            question=question, options=options, voters=voters
        )
        ...
        if not self._swarm_intelligence:
            ...
            for task in tasks:
                try:
                    ...
                    results.append(
                        {"task_id": task.get("task_id"), "success": True, "result": result}
                    )
                except Exception as e:
                    results.append(
                        {"task_id": task.get("task_id"), "success": False, "error": str(e)}
                    )
            return results
    ...
    async def _process_in_chunks(
        self, items: List, chunk_size: int, process_func: Any, delay: float = 0.1
    ) -> List:
