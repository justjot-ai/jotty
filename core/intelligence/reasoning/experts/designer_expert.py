...
class DesignerExpertAgent(BaseExpert):
    ...
    async def _evaluate_domain(
        ...
        has_wireframes = (
            "wireframe" in output.lower()
            or "mermaid" in output.lower()
            or "layout" in output.lower()
        )
        has_components = "component" in output.lower() or "element" in output.lower()
        has_hierarchy = (
            "hierarchy" in output.lower()
            or "structure" in output.lower()
            or "navigation" in output.lower()
        )
        has_accessibility = (
            "accessibility" in output.lower()
            or "a11y" in output.lower()
            or "aria" in output.lower()
        )
        has_responsive = (
            "responsive" in output.lower()
            or "mobile" in output.lower()
            or "breakpoint" in output.lower()
        )
        ...
        if score < 0.9:
            suggestions = (
                "Enhance: " + ", ".join(issues[:3]) if issues else "Add more design detail"
            )
