...
class DomainValidator(ABC):
    ...
    @abstractmethod
    def validate(self, output: str, expected_type: str, context: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        """
...
class MermaidValidator(DomainValidator):
    ...
    def validate(self, output: str, expected_type: str, context: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if required_elements:
            ...
            metadata["element_coverage"] = len(found_elements) / len(required_elements) if required_elements else 0.0
        ...
        if abs(open_brackets - close_brackets) > 5:  # Allow some imbalance for complex diagrams
            errors.append(f"Significant bracket imbalance: {open_brackets} open, {close_brackets} close")
...
class MathLaTeXValidator(DomainValidator):
    ...
    def detect_type(self, output: str) -> str:
        ...
        if '$$' in output or '\\[' in output or '\\begin{equation}' in output_lower or '\\begin{align}' in output_lower:
            return "display"
        elif '$' in output:
            return "inline"
