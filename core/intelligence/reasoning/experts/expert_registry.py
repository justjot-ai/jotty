...
def get_pipeline_expert(
    output_format: str = "mermaid", auto_train: bool = False
) -> PipelineExpertAgent:
...
async def get_pipeline_expert_async(
    output_format: str = "mermaid", auto_train: bool = True
) -> PipelineExpertAgent:
    """Convenience function to get Pipeline expert (async with auto-train)."""
    return await _registry.get_pipeline_expert_async(
        output_format=output_format, auto_train=auto_train
    )
