...
class TrainingDataLoader:
    ...
    def load_from_github_repo(
        self, repo_url: str, path: str = "", file_pattern: str = "*.puml", max_files: int = 50
    ) -> List[Dict[str, Any]]:
        """
                ...
                try:
                    ...
                    with urllib.request.urlopen(dir_req, timeout=10) as dir_response:
                        ...
                        for item in dir_items:
                            ...
                            if item_type == "file" and self._matches_pattern(
                                item_name, file_pattern
                            ):
                                ...
                                if item_name.lower() not in [
                                    ...
                                    subdir_path = (
                                        f"{dir_path}/{item_name}" if dir_path else item_name
                                    )
                    ...
                    if e.code == 403:
                        logger.warning(
                            "GitHub API rate limit exceeded - consider using GitHub token"
                        )
    ...
    def convert_to_gold_standards(self, examples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _matches_pattern(self, filename: str, pattern: str) -> bool:
        ...
        if normalized_pattern == "*.puml":
            ...
            return (
                filename.lower().endswith(".puml")
                or filename.lower().endswith(".plantuml")
                or filename.lower().endswith(".pu")
            )
