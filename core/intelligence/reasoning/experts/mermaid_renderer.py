    ...
    if len(mermaid_code) > MAX_URL_SAFE_SIZE:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=timeout) as response:
                ...
                if status_code == 200 and ('image' in content_type.lower() or 'png' in content_type.lower()):
            ...
            if e.code == 414 or e.code >= 400:
                ...
                has_valid_structure = (
                    diagram_type != "unknown" and
                    metadata['lines'] > 0 and
                    (diagram_type in ['graph', 'flowchart'] or 
                     '-->' in mermaid_code or 
                     '->>' in mermaid_code or
                     'alt' in mermaid_code.lower())
                )
...
def validate_mermaid_syntax(mermaid_code: str, use_renderer: bool = True) -> Tuple[bool, str, Dict]:
    ...
    if use_renderer:
        ...
        # Basic checks
        first_line = mermaid_code.split('\n')[0].strip().lower()
        has_valid_type = any(keyword in first_line for keyword in [
            'graph', 'flowchart', 'sequencediagram', 'sequence', 'statediagram',
            'gantt', 'erdiagram', 'gitgraph', 'journey', 'classdiagram'
        ])
