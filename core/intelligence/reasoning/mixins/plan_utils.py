    ...
    def _build_skill_params(
        ...
        try:
            ...
            if skill and hasattr(skill, "tools") and skill.tools:
                ...
                tool_func = (
                    skill.tools.get(tool_name) if tool_name else list(skill.tools.values())[0]
                )

                if tool_func:
                    # Extract parameter schema from docstring
                    schema = self._extract_tool_schema(
                        tool_func, tool_name or list(skill.tools.keys())[0]
                    )
                    ...
                    for param in tool_params:
                        ...
                        if param_name in ["query", "topic", "search_query", "q"]:
                            ...
                            params[param_name] = (
                                math_match.group(1).strip() if math_match else clean_task
                            )
                            ...
                            if any(
                                word in param_desc
                                for word in ["input", "content", "data", "result"]
                            ):
                                params[param_name] = prev_ref
                            else:
                                params[param_name] = clean_task
                    ...
                    if params:
                        logger.debug(
                            f"Built params from schema for {skill_name}: {list(params.keys())}"
                        )
                        return params
...
{lang_hint} code:"""
    ...
    def _infer_filename_from_task(self, task: str) -> Optional[str]:
        ...
        if _is_report_or_synthesis and not any(
            w in task_lower for w in ["script", ".py", "python code"]
        ):
            file_ext = ".md"
        elif _is_python_backend:
            file_ext = ".py"  # Explicit Python — don't let 'web' override
        elif any(
            w in task_lower for w in ["html page", "webpage", "web page", "html file", ".html"]
        ):
            file_ext = ".html"
    ...
    def _create_fallback_plan(
        self, task: str, task_type: Any, skills: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _convert_skills_to_agents(self, skills: List[Dict[str, Any]]) -> Any:
        ...
        try:
            from Jotty.core.capabilities.registry.skill_to_agent_converter import (
    ...
    def _create_task_graph_from_string(self, task_string: str, task_type: Any) -> Optional[Any]:
        ...
        return TaskGraph(
            task_type=task_type, workflow=None, metadata={"original_request": task_string}
        )
        ...
        if skills:
            try:
                ...
                if lm:
                    ...
                    response_text = (
                        response[0] if isinstance(response, list) else str(response)
                    ).strip()
    ...
    def _parse_plan_to_steps(
        ...
        plan_data = self._normalize_raw_plan(
            raw_plan, skills=skills, task=task, task_type=task_type
        )
        ...
        for i, step_data in enumerate(plan_data[:max_steps]):
            try:
                ...
                tool_name = (
                    get_val(step_data, "tool_name")
                    or get_val(step_data, "tool", "")
                    or get_val(step_data, "action", "")
                )
                ...
                if not skill_name and len(available_skill_names) == 1:
                    ...
                    logger.info(
                        f"Auto-inferred skill_name='{skill_name}' (only one skill available)"
                    )
                    ...
                    for candidate in available_skill_names:
                        if candidate.replace("-", " ") in desc or any(
                            w in desc for w in candidate.split("-")
                        ):
                            skill_name = candidate
                            logger.info(
                                f"Inferred skill_name='{skill_name}' from description match"
                            )
                            break
                ...
                if not skill_name or skill_name not in available_skill_names:
                    ...
                    if any(
                        ...
                        w in desc_lower
                        for w in ["create", "write", "generate", "save", "file", "report"]
                    ):
                    ...
                    if inferred_skill and inferred_skill in available_skill_names:
                        logger.info(
                            f"Step {i+1}: Inferred skill '{inferred_skill}' from description"
                        )
                        ...
                        logger.warning(
                            f"Skipping step {i+1}: '{str(description)[:50]}' - no skills available"
                        )
                        continue
                ...
                if not tool_name:
                    for s in skills:
                        if s.get("name") == skill_name:
                            ...
                            if skill_tools:
                                tool_names_list = [
                                    t.get("name") if isinstance(t, dict) else t for t in skill_tools
                                ]
                                ...
                                if skill_name == "file-operations":
                                    if any(
                                        w in desc_lower for w in ["directory", "folder", "mkdir"]
                                    ):
                                        if "create_directory_tool" in tool_names_list:
                                            tool_name = "create_directory_tool"
                                    elif any(
                                        w in task_lower or w in desc_lower
                                        for w in ["create", "write", "generate", "make"]
                                    ):
                                        if any(
                                            ext in desc_lower
                                            for ext in [
                                ...
                                logger.debug(
                                    f"Inferred tool_name='{tool_name}' from skill '{skill_name}'"
                                )
                            break

                # Handle params
                step_params = (
                    get_val(step_data, "params")
                    or get_val(step_data, "parameters")
                    or get_val(step_data, "tool_parameters")
                    or get_val(step_data, "inputs")
                    or get_val(step_data, "tool_input")
                    or {}
                )
                ...
                fallback_params = self._build_skill_params(
                    skill_name, param_source, prev_output, tool_name
                )
        ...
        for i, step in enumerate(steps):
            ...
            if schema and schema.outputs:
                ...
                if not step.outputs_produced:
                    ...
                    logger.debug(
                        f"Step {i} ({output_key}): auto-set outputs_produced={declared_names}"
                    )
        ...
        for i, step in enumerate(steps):
            ...
            for param_name, param_value in step.params.items():
                ...
                for match in template_re.finditer(param_value):
                    ...
                    best_field = self._match_output_field(param_name, schema.output_field_names)
                    if best_field:
    ...
    def _maybe_decompose_plan(
        self, steps: list, skills: List[Dict[str, Any]], task: str, task_type: Any
    ) -> Optional[list]:
        """
        ...
        clean_task = (
            self._clean_task_for_query(task) if hasattr(self, "_clean_task_for_query") else task
        )
        ...
        if len(entities) >= 2:
            ...
            if len(longest.split()) > len(shortest.split()):
                ...
                domain_words = [
                    w
                    for w in longest.split()
                    if w.lower() not in shortest_words
        ...
        research_refs = " ".join(f"${{research_{i}.results}}" for i in range(len(entities[:4])))
        ...
        if task_wants_pdf or task_wants_telegram:
            pdf_skill = (
                ...
                if "simple-pdf-generator" in available
                else "document-converter"
            )
            pdf_tool = (
                "generate_pdf_tool"
                if pdf_skill == "simple-pdf-generator"
                else "convert_to_pdf_tool"
            )
            content_ref = (
                "${synthesis.text}" if synth_skill == "claude-cli-llm" else "${synthesis.summary}"
            )
            decomposed.append(
                ExecutionStep(
        ...
        if task_wants_telegram and "telegram-sender" in available:
            decomposed.append(
                ExecutionStep(
                    ...
                    description="Send comparison report via Telegram",
        ...
        if task_wants_slack and "slack" in available:
            decomposed.append(
                ExecutionStep(
                    ...
                    description="Send comparison report via Slack",
        ...
        logger.info(
            f" Decomposed {len(steps)}-step composite plan → {len(decomposed)} granular steps"
        )
    ...
    def _extract_comparison_entities(self, task: str) -> List[str]:
        ...
        if len(vs_match) >= 2:
            ...
            for part in vs_match:
                part = re.sub(
                    r"^\s*(compare|research|analyze|research on|create|generate|send|make|build)\s+",
                    "",
                    part,
                    flags=re.IGNORECASE,
                )
                part = re.sub(
                    r"\s*(comparison|compare|report|pdf|document|analysis|review|overview)\b.*$",
                    "",
                    part,
                    flags=re.IGNORECASE,
                )
                part = re.sub(
                    r"\s*(via telegram|via slack|and send|and create|,.*$)\s*$",
                    "",
                    part,
                    flags=re.IGNORECASE,
                )
