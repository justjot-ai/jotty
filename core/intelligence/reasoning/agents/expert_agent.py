...
Migration Guide:
    ...
    New (recommended):
        from Jotty.core.intelligence.reasoning.experts.expert_templates import create_mermaid_expert
...
from Jotty.core.intelligence.memory.cortex import SwarmMemory  # type: ignore[import-not-found]
...
class ExpertAgent:
        ...
        if config.use_memory_storage and not memory:
            try:
                ...
                self.memory = SwarmMemory(
                    agent_name=f"expert_{config.domain}", config=memory_config
                )

                # Enable persistence
                persistence_dir = self.data_dir / "memory"
                self.memory_persistence = enable_memory_persistence(
                    memory=self.memory, persistence_dir=persistence_dir
                )

                self.use_memory_storage = True
                logger.info(
                    f"Created SwarmMemory for expert {config.name} with persistence at {persistence_dir}"
                )
            ...
            try:
                if use_synthesis:
                    ...
                    synthesized = retrieve_synthesized_improvements(
                        memory=self.memory, expert_name=self.config.name, domain=self.config.domain
                    )
                    ...
                    if synthesized:
                        ...
                        logger.info(
                            f"Loaded synthesized improvements from memory (length: {len(synthesized)} chars)"
                        )
                else:
    ...
    async def train(
        ...
        if self.trained and not force_retrain:
            logger.info(
                f"Expert {self.config.name} already trained. Use force_retrain=True to retrain."
            )
        ...
        if enable_pre_training and training_mode in ["pattern_extraction", "both"]:
            ...
            logger.info(
                f"Pre-training complete: {pre_training_results.get('patterns_learned', 0)} patterns learned"
            )
        ...
        if training_mode in ["iterative", "both"]:
            ...
            for i, case in enumerate(gold_standards, 1):
                logger.info(
                    f"Training case {i}/{len(gold_standards)}: {case.get('task', 'Unknown')}"
                )
        ...
        if self.trained:
            logger.info(
                f"Expert {self.config.name} marked as trained (passed: {has_passed_cases}, improvements: {has_improvements})"
            )
        ...
        if self.use_memory_storage and self.memory and len(self.improvements) >= 3:
            try:
                ...
                if consolidation_result.get("consolidated", 0) > 0:
                    logger.info(
                        f"Consolidated {consolidation_result['consolidated']} patterns to SEMANTIC level"
                    )
        ...
        logger.info(
            f"Training complete: {training_results['passed_cases']}/{training_results['total_cases']} cases passed"
        )
    ...
    async def validate(
        ...
        for i, case in enumerate(validation_cases, 1):
            logger.info(
                f"Validation case {i}/{len(validation_cases)}: {case.get('task', 'Unknown')}"
            )

            output = await self.generate(task=case.get("task", ""), context=case.get("context", {}))
            ...
            if self.config.evaluation_function:
                ...
                score = (
                    1.0
                    if str(output).strip() == str(case.get("gold_standard", "")).strip()
                    else 0.0
                )
    ...
    async def _pre_train_from_gold_standards(
        self, gold_standards: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    async def _pre_train(self, examples: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    if validator:
                        is_valid, error, metadata = validator.validate(
                            output=code, expected_type=diagram_type, context={"task": description}
                        )
        ...
        logger.info(
            f"Pre-training extracted {patterns_learned} patterns from {len(validated_examples)} examples"
        )
    ...
    def _extract_pattern_from_example(
        self, code: str, description: str, diagram_type: str
    ) -> Optional[str]:
        """
    ...
    async def generate(self, task: str, context: Optional[Dict[str, Any]] = None) -> Any:
        """
        ...
        if (
            improvements_to_use
            and hasattr(self, "_optimization_pipeline")
            and self._optimization_pipeline
        ):
            if (
                hasattr(self._optimization_pipeline, "credit_assignment")
                and self._optimization_pipeline.credit_assignment
            ):
                try:
                    improvements_to_use = (
                        self._optimization_pipeline.credit_assignment.prioritize_improvements(
                    ...
                    logger.debug(
                        f"Using {len(improvements_to_use)} prioritized improvements (from {len(self.improvements)} total)"
                    )
                except Exception as e:
        ...
        result = self._call_dspy_agent(
            agent, task=task, learned_improvements=improvements_str, **context
        )
    ...
    def _apply_domain_specific_improvements(
        self, improvements: List[Dict[str, Any]], context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _apply_domain_specific_post_processing(self, output: Any, context: Dict[str, Any]) -> Any:
        """
    ...
    def _create_agents(self) -> List[AgentConfig]:
        ...
        if self.config.agent_module:
            ...
            agents.append(
                AgentConfig(name=f"{self.config.domain}_generator", agent=agent, outputs=["output"])
            )
        else:
        ...
        if self.config.enable_teacher_model:
            ...
            agents.append(
                AgentConfig(name="teacher", agent=teacher_agent, metadata={"is_teacher": True})
            )
    ...
    def _extract_dspy_output(self, result: Any) -> Any:
        ...
        try:
            ...
            if isinstance(result, dspy.Prediction) or (
                hasattr(result, "output") and not isinstance(result, dict)
            ):
