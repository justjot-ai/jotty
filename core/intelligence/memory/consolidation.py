...
class ConsolidationValidator:
    ...
    def validate_pattern(
        self, pattern: str, source_memories: List[Any], pattern_type: str = "SEMANTIC"
    ...
    def _llm_validate(
        ...
        try:
            ...
            result = self.validator(
                pattern=pattern, source_memories=json.dumps(source_texts), pattern_type=pattern_type
            )
    ...
    def quarantine_suspicious(
        ...
        logger.warning(f"Quarantined pattern: {pattern[:100]}... " f"Reason: {reason}")
    ...
    def classify(self, experience: str, context: Dict[str, Any]) -> Tuple[MemoryLevel, float, bool]:
        ...
        try:
            result = self.classifier(
                experience=experience, context=json.dumps(context)  # NO LIMIT - FULL content
            )
