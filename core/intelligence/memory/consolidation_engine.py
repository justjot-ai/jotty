...
class SharpWaveRippleConsolidator:
    ...
    async def consolidate(
        ...
        logger.info(
            f" Starting consolidation #{self.consolidation_count} "
        ...
        logger.info(
            f" Consolidation complete: {result.patterns_extracted} patterns, "
    ...
    async def _rapid_replay_pattern_extraction(self, episodes: List[Dict[str, Any]]) -> List[str]:
        """
    ...
    def _extract_common_pattern(
        self, episodes: List[Dict[str, Any]], outcome_type: str
    ) -> Optional[str]:
    ...
    def _extract_agent_pattern(self, agent: str, episodes: List[Dict[str, Any]]) -> Optional[str]:
    ...
    def _extract_causal_links(self, episodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.consolidator.consolidate(
                    self.recent_episodes, episodic_memories or [], semantic_memories or []
                ),
...
class AgentRole:
    ...
    def to_summary(self) -> str:
        ...
        return (
            f"{self.role_name}: {len(self.agents)} agents, "
...
class AgentAbstractor:
    ...
    def update_agent(self, agent: str, success: bool, task_type: str = "") -> None:
        ...
        if agent not in self.agent_stats:
            self.agent_stats[agent] = {"successes": 0, "failures": 0, "task_types": set()}
    ...
    def _get_detailed_view(self, agents: List[str]) -> Dict[str, Any]:
        """Full detail for small swarms."""
        view = {"abstraction_level": "detailed", "agent_count": len(agents), "agents": {}}
        ...
        for agent in agents:
            ...
            if role_name not in self.role_definitions:
                self.role_definitions[role_name] = AgentRole(
                    role_name=role_name, agents=[], capabilities=set()
                )
            ...
            if total > 0:
                ...
                # Running average
                role.avg_success_rate = (
                    role.avg_success_rate * role.total_tasks + success_rate
                ) / (role.total_tasks + 1)
    ...
    def _infer_role(self, agent: str, stats: Dict[str, Any]) -> str:
        ...
        role_scores = {
            "processor": task_str.count("process")
            + task_str.count("transform")
            + task_str.count("convert"),
            "validator": task_str.count("valid")
            + task_str.count("check")
            + task_str.count("verify"),
            "generator": task_str.count("generate")
            + task_str.count("create")
            + task_str.count("produce"),
            "analyzer": task_str.count("analyze")
            + task_str.count("extract")
            + task_str.count("parse"),
        }
    ...
    def get_context_summary(self) -> str:
        ...
        if view["abstraction_level"] == "detailed":
            ...
            for agent, info in view["agents"].items():
                lines.append(
                    f"  - {agent}: {info['success_rate']:.0%} success, {info['total_tasks']} tasks"
                )
        else:
