...
class RecencyValueRanker:
    ...
    def prerank(
        self, memories: List[MemoryEntry], goal: str, max_candidates: int = 100
        ...
        for memory in memories:
            ...
            combined = (
                0.4 * value_score + 0.25 * level_score + 0.2 * recency_score + 0.15 * access_score
            )
...
class EmbeddingPreFilter:
    ...
    def prefilter(
        self, query: str, memories: List["MemoryEntry"], goal: str = ""
    ) -> List["MemoryEntry"]:
        """
    ...
    if _RelevanceSignature is None:
        ...
        class RelevanceSignature(dspy.Signature):
            ...
            query: str = dspy.InputField(
                desc="The current query/goal - can be ANY language or format"
            )
            ...
            reasoning: str = dspy.OutputField(
                desc="Step-by-step semantic analysis of each memory's relevance"
            )
            scores: str = dspy.OutputField(
                desc="JSON dict mapping memory_key to relevance score 0.0-1.0"
            )
    ...
    if _MemorySynthesisSignature is None:
        ...
        class MemorySynthesisSignature(dspy.Signature):
            ...
            memories: str = dspy.InputField(
                desc="All relevant memories (JSON list with level, content, value)"
            )
            context_hints: str = dspy.InputField(desc="Additional context about task domain")
            analysis: str = dspy.OutputField(
                desc="Deep analysis: patterns across memories, contradictions, causal relationships"
            )
            core_principles: str = dspy.OutputField(
                desc="Key principles synthesized from experiences (what we learned)"
            )
            anti_patterns: str = dspy.OutputField(
                desc="What to avoid based on failures and mistakes"
            )
            recommendations: str = dspy.OutputField(
                desc="Specific actionable recommendations for current task"
            )
            wisdom: str = dspy.OutputField(
                desc="Integrated wisdom - coherent schema combining all insights"
            )
...
class LLMRelevanceScorer:
    ...
    def score_batch(
        self, query: str, memories: List[MemoryEntry], context_hints: str = ""
    ) -> Dict[str, float]:
        """
...
class MemorySynthesizer:
    ...
    def synthesize_wisdom(
        self, query: str, goal: str, memories: List[MemoryEntry], context_hints: str = ""
    ) -> str:
        """
        ...
        try:
            result = self.synthesizer(
                query=query, goal=goal, memories=memories_json, context_hints=context_hints
            )
            ...
            logger.debug(
                f" Synthesized {len(memories)} memories into {len(synthesized)} char wisdom"
            )
    ...
    if _DeduplicationSignature is None:
        ...
        class DeduplicationSignature(dspy.Signature):
            ...
            merged_content: str = dspy.OutputField(
                desc="If duplicate, the merged content preserving all info"
            )
...
class DeduplicationEngine:
    ...
    def __init__(self, config: Any) -> None:
        ...
        self.checker = (
            _get_dspy().ChainOfThought(_get_dedup_signature()) if not self.SKIP_LLM_DEDUP else None
        )
    ...
    def check_duplicate(self, mem_a: MemoryEntry, mem_b: MemoryEntry) -> Tuple[bool, float, str]:
        ...
        len_ratio = min(len(mem_a.content), len(mem_b.content)) / max(
            len(mem_a.content), len(mem_b.content)
        )
        if len_ratio < 0.3:
    ...
    def deduplicate_batch(self, memories: List[MemoryEntry]) -> List[MemoryEntry]:
        ...
        for mem in memories:
            if len(mem.content) > MAX_MEMORY_SIZE:
                ...
                logger.info(
                    f" Memory too large ({len(mem.content)} chars), skipping dedup for: {mem.key[:50]}..."
                )
            else:
...
def _get_causal_signature() -> Any:
    ...
    if _CausalExtractionSignature is None:
        ...
        class CausalExtractionSignature(dspy.Signature):
            ...
            causal_links: str = dspy.OutputField(
                desc="JSON list of {cause, effect, conditions, confidence}"
            )
...
class CausalExtractor:
    ...
    def extract_from_episodes(
        self, success_episodes: List[Dict], failure_episodes: List[Dict], domain: str = "sql"
    ) -> List[Dict]:
        """
...
class LLMRAGRetriever:
    ...
    def retrieve(
        ...
        candidates = self.preranker.prerank(
            memories=memories, goal=goal, max_candidates=self.config.rag_max_candidates
        )
        ...
        if (
            self.embedding_prefilter
            and self.embedding_prefilter.available
        ...
        if self.config.enable_deduplication:
            unique_memories = self.deduplicator.deduplicate_batch([m for m, _ in scored_memories])
        else:
    ...
    def retrieve_and_synthesize(
        ...
        synthesized_wisdom = self.synthesizer.synthesize_wisdom(
            query=query, goal=goal, memories=candidate_memories, context_hints=context_hints
        )

        return synthesized_wisdom

    def retrieve_with_chunks(
        self, query: str, goal: str, large_content: str, source_key: str, budget_tokens: int
    ) -> List[ContentChunk]:
        """
        ...
        chunk_scores = [
            (chunk, scores.get(f"{source_key}_chunk_{chunk.chunk_index}", 0.5)) for chunk in chunks
        ]
