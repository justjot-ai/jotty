...
class SimpleBrain:
    ...
    def _should_remember(self, exp: Experience) -> bool:
        ...
        """
        from Jotty.core.infrastructure.foundation.robust_parsing import AdaptiveThreshold, EpsilonGreedy, safe_hash
...
class BrainInspiredMemoryManager:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_hippo_strength': sum(m.strength for m in self.hippocampus) / len(self.hippocampus) if self.hippocampus else 0,
            'avg_neo_strength': sum(p.strength for p in self.neocortex) / len(self.neocortex) if self.neocortex else 0,
    ...
    def _synaptic_pruning(self) -> tuple[int, int]:
        ...
        self.hippocampus = [
            mem for mem in self.hippocampus
        ...
        self.neocortex = [
            pattern for pattern in self.neocortex
            if pattern.strength > 0.5  # Only remove very weak
            or (current_time - pattern.last_reinforced) < 86400 * 7  # Or keep if reinforced in last 7 days
        ]
