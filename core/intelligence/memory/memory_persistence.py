...
from Jotty.core.infrastructure.foundation.data_structures import (  # type: ignore[import]
    MemoryEntry,
    MemoryLevel,
)
            ...
            for level, file_path in self.level_files.items():
                ...
                for entry in memories.values():
                    entry_dict = {
                        ...
                        "goal_values": {
                            goal: {"value": gv.value, "access_count": gv.access_count}
...
def enable_memory_persistence(
    memory: SwarmMemory, persistence_dir: Optional[Path] = None
) -> MemoryPersistence:
    """
