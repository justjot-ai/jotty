...
class JustJotMemory:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "JustJotMemory":
        ...
        return cls(
            ...
            importance_factors=(
                importance.get("factors", {}) if isinstance(importance, dict) else {}
            ),
...
class JustJotMemorySyncAdapter:
    ...
    Usage:
        ...
        from Jotty.core.intelligence.memory.justjot_sync_adapter import JustJotMemorySyncAdapter
    ...
    def goal_values_to_importance(
        self, goal_values: Dict[str, Any]
    ...
    def importance_to_goal_values(
        self, importance_score: float, importance_factors: Dict[str, float], goal: str = "default"
    ) -> Dict[str, Any]:
        """
    ...
    def sync_to_justjot(self, memory_entry: Any) -> JustJotMemory:
        ...
        memory_id = (
            ...
            else hashlib.md5(memory_entry.content.encode()).hexdigest()
        )

        # Get entity_id (agent name)
        entity_id = (
            getattr(memory_entry, "source_agent", None)
            or getattr(self.cortex, "agent_name", "unknown")
            if self.cortex
            else "unknown"
        )
    ...
    def sync_from_justjot(self, justjot_memory: JustJotMemory) -> Any:
        ...
        goal_values = self.importance_to_goal_values(
            justjot_memory.importance_score, justjot_memory.importance_factors, goal
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        return [
                            self.sync_from_justjot(JustJotMemory.from_dict(m)) for m in memories
                        ]
                    else:
                        error = await response.text()
    ...
    def _pull_sync(self, entity_id: str, limit: int) -> List:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return [self.sync_from_justjot(JustJotMemory.from_dict(m)) for m in memories]
            else:
    ...
    async def sync_all(self, direction: str = "both") -> Dict[str, Any]:
        ...
        Returns:
            Sync statistics
        """
        stats = {"pushed": 0, "pulled": 0, "errors": [], "timestamp": datetime.now().isoformat()}
