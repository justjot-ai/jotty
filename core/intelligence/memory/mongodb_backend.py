...
**Usage**:
```python
from Jotty.core.intelligence.memory import SwarmMemory
from Jotty.core.intelligence.memory.mongodb_backend import MongoDBMemoryBackend
...
from Jotty.core.infrastructure.foundation.data_structures import GoalValue, MemoryEntry, MemoryLevel
...
class MongoDBMemoryBackend:
    ...
    def __init__(
        self, memory: SwarmMemory, mongo_uri: Optional[str] = None, agent_name: Optional[str] = None
    ) -> None:
        """
    ...
    def _ensure_indexes(self) -> Any:
        ...
        try:
            ...
            self.memories_collection.create_index(
                [("agent_name", ASCENDING), ("level", ASCENDING), ("timestamp", DESCENDING)],
                name="agent_level_time",
            )

            # Index for goal-based retrieval
            self.memories_collection.create_index(
                [("agent_name", ASCENDING), ("goal", ASCENDING)], name="agent_goal"
            )

            # Text index for content search
            self.memories_collection.create_index([("content", TEXT)], name="content_text")
        ...
        try:
            ...
            for level in MemoryLevel:
                ...
                cursor = self.memories_collection.find(
                    {"agent_name": self.agent_name, "level": level.value.upper()}
                )
    ...
    def _entry_to_document(self, entry: MemoryEntry, level: MemoryLevel) -> Dict[str, Any]:
        ...
        for goal, gv in entry.goal_values.items():
            goal_values_dict[goal] = {"value": gv.value, "access_count": gv.access_count}
    ...
    def _document_to_entry(self, doc: Dict[str, Any], level: MemoryLevel) -> MemoryEntry:
        ...
        for goal, gv_data in doc.get("goal_values", {}).items():
            goal_value = GoalValue(
                value=gv_data.get("value", 0.5), access_count=gv_data.get("access_count", 0)
            )
        ...
        if doc.get("last_accessed"):
            entry.last_accessed = (
                doc["last_accessed"]
                if isinstance(doc["last_accessed"], datetime)
                else datetime.fromisoformat(str(doc["last_accessed"]))
            )
    ...
    def retrieve_by_goal(
        self, goal: str, levels: Optional[List[MemoryLevel]] = None, max_memories: int = 10
    ) -> List[MemoryEntry]:
        """
        ...
        for level in levels:
            cursor = (
                self.memories_collection.find(
                    {"agent_name": self.agent_name, "level": level.value.upper(), "goal": goal}
                )
    ...
    def store_task_outcome(self, task_id: str, task_spec: str, outcome: Dict[str, Any]) -> bool:
        """
    ...
    def retrieve_planning_context(
        self, task_description: str, category: str = "general", max_memories: int = 10
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_failure_pattern(
        self, task_spec: str, category: str, errors: List[str], task_id: str
    ) -> None:
...
def enable_mongodb_memory(
    memory: SwarmMemory, mongo_uri: Optional[str] = None
