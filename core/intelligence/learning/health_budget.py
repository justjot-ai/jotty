...
class LearningHealthMonitor:
    ...
    def record_episode(
        ...
        value_updates: List[Tuple[str, float, float]],
    ) -> List[str]:
        """
        ...
        if self._detect_reward_hacking():
            alerts.append(
                f"ALERT[{AlertType.REWARD_HACKING.value}]: Success rate suspiciously high (>{self.config.suspicion_threshold:.0%})"
            )

        # Check for conservative collapse
        approval_rate = (
            sum(architect_decisions) / len(architect_decisions) if architect_decisions else 0.5
        )
        if self._detect_conservative_collapse(approval_rate):
            alerts.append(
                f"ALERT[{AlertType.CONSERVATIVE_COLLAPSE.value}]: Rejection rate too high, may be over-conservative"
            )

        # Check for learning stall
        if self._detect_learning_stall():
            alerts.append(
                f"ALERT[{AlertType.LEARNING_STALL.value}]: Learning appears stalled, values not changing"
            )
        ...
        if drift:
            alerts.append(
                f"ALERT[{AlertType.GOAL_DRIFT.value}]: Goal distribution shifting: {drift}"
            )
...
class DynamicBudgetManager:
    ...
    def compute_allocation(
        ...
        # Enforce bounds
        memory_budget = max(
            self.config.min_memory_budget, min(self.config.max_memory_budget, memory_budget)
        )
    ...
    def select_within_budget(
        self, items: List[MemoryEntry], budget: int, goal: str, max_items: int = 50
    ) -> List[MemoryEntry]:
        """
    ...
    def get_learned_context(self, memories: Dict[str, MemoryEntry], goal: str = None) -> str:
        ...
        if improved_memories:
            ...
            for memory, value, improvement in sorted(
                improved_memories, key=lambda x: abs(x[2]), reverse=True
            )[:3]:
