...
class LLMQPredictorSignature(dspy.Signature):
    ...
    state_description = dspy.InputField(
        desc="Current state of the swarm (Task List state, actor states, memory summary)"
    )
...
class LLMQPredictor:
    ...
    def __init__(self, config: Any) -> None:
        ...
        self.q_prune_percentage = getattr(
            config, "q_prune_percentage", 0.2
        )  # Remove 20% when limit hit
    ...
    def _state_to_natural_language(self, state: Dict) -> str:
        ...
        if "tool_calls" in state:
            ...
            if isinstance(tools, list) and tools:
                tool_names = [
                    t.get("tool", t) if isinstance(t, dict) else str(t) for t in tools[:5]
                ]
            ...
            if isinstance(schemas, dict):
                ...
                for tool_name, schema in list(schemas.items())[:3]:
                    if isinstance(schema, dict):
                        ...
                        schema_parts.append(
                            f"{tool_name}({','.join(input_keys[:2])})->{','.join(output_keys[:2])}"
                        )
                    else:
    ...
    def _extract_lesson(
        self, state_desc: str, action_desc: str, reward: float, td_error: float
    ) -> Optional[str]:
        """
        ...
        insight_str = (
            "; ".join(insights) if insights else f"{state_desc[:60]}... → {action_desc[:40]}..."
        )
    ...
    def _get_similar_experiences(self, state: Dict, action: Dict) -> List[Dict]:
        ...
        for exp in self.experience_buffer:
            ...
            if self._are_similar(state_desc, exp_state_desc) or self._are_similar(
                action_desc, exp_action_desc
            ):
                similar.append(exp)
    ...
    def predict_q_value(
        ...
        try:
            ...
            alt = (
                result.alternative_suggestion if hasattr(result, "alternative_suggestion") else None
            )
    ...
    def get_learned_context(self, state: Dict, action: Dict = None) -> str:
        ...
        current_task_type = (
            state.get("task_type", "").lower().strip() if isinstance(state, dict) else ""
        )
    ...
    def get_best_action(
        self, state: Dict, available_actions: List[Dict]
    ...
    def get_q_table_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_lessons": sum(
                len(q_data.get("learned_lessons", [])) for q_data in self.Q.values()
            ),
            # NeuroChunk stats
        ...
        if self.tier1_working:
            ...
            for i, key in enumerate(self.tier1_working[:20], 1):  # Top 20
                ...
                context_parts.append(
                    f"{i}. [{state_short}] → [{action_short}] | Q={q_val:.2f} (n={visits})"
                )
    ...
    def save_state(self, path: str) -> None:
        ...
        state = {
            ...
            "tier1_working": [
                f"{k[0]}|||{k[1]}" if isinstance(k, tuple) else str(k) for k in self.tier1_working
            ],
            "tier2_clusters": {
                cid: {
                    "centroid": c["centroid"],
                    "members": [
                        f"{k[0]}|||{k[1]}" if isinstance(k, tuple) else str(k) for k in c["members"]
                    ],
                }
                for cid, c in self.tier2_clusters.items()
            },
            "tier3_archive": [
                f"{k[0]}|||{k[1]}" if isinstance(k, tuple) else str(k) for k in self.tier3_archive
            ],
            "tier1_threshold": self.tier1_threshold,
        ...
        logger.info(
            f" Q-learning state saved: {len(self.Q)} Q-entries, {len(self.experience_buffer)} experiences"
        )
        ...
        try:
            ...
            for cid, c in state.get("tier2_clusters", {}).items():
                self.tier2_clusters[cid] = {
                    "centroid": c["centroid"],
                    "members": [
                        tuple(k.split("|||", 1)) if "|||" in k else (k, "") for k in c["members"]
                    ],
                }
            ...
            logger.info(
                f" Q-learning state loaded: {len(self.Q)} Q-entries, {len(self.experience_buffer)} experiences"
            )
            return True
