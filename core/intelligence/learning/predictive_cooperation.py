...
class NashBargainingSignature(dspy.Signature):
    ...
    system_reward = dspy.InputField(desc="Total system reward achieved")
    fallback_rewards = dspy.InputField(
        desc="What each agent would get if they did nothing (baseline)"
    )
    cooperation_context = dspy.InputField(desc="How agents cooperated (who helped whom, synergies)")

    reasoning = dspy.OutputField(desc="Step-by-step Nash Bargaining reasoning")
    reward_distribution = dspy.OutputField(
        desc="JSON dict of {agent_name: reward}, must sum to system_reward"
    )
    fairness_score = dspy.OutputField(desc="How fair is this distribution (0.0-1.0)")
...
class NashBargainingSolver:
    ...
    def distribute_rewards(
        ...
        """
        logger.info(
            f" Nash Bargaining: Distributing {system_reward:.2f} among {len(agents)} agents"
        )
        ...
        try:
            ...
            if abs(total - system_reward) > 0.01:
                logger.warning(
                    f" Distribution sum ({total:.2f}) != system_reward ({system_reward:.2f})"
                )
                # Normalize
...
class CooperationReasoningSignature(dspy.Signature):
    ...
    cooperation_principles = dspy.InputField(desc="Cooperation principles to follow")
    past_interactions = dspy.InputField(desc="Memory of past interactions with other agents")
    swarm_goal = dspy.InputField(desc="Overall swarm goal")

    reasoning = dspy.OutputField(desc="Step-by-step reasoning about cooperation and system reward")
    best_action = dspy.OutputField(desc="Action that maximizes SYSTEM reward (not just my reward)")
    communication_plan = dspy.OutputField(desc="What to share with whom (agent_name: message)")
    confidence = dspy.OutputField(desc="Confidence in this decision (0.0-1.0)")
    expected_system_reward = dspy.OutputField(desc="Expected system reward if I take this action")
...
class CooperationReasoner:
    ...
    def decide_cooperative_action(
        ...
        decision = {
            ...
            "expected_system_reward": (
                float(result.expected_system_reward) if result.expected_system_reward else 0.5
            ),
        }
...
class PredictiveCooperativeAgent:
    ...
    def __init__(
        self, agent_name: str, dqn_predictor: Any = None, shapley_estimator: Any = None
    ) -> None:
            ...
            for other_agent in other_agents:
                if other_agent != self.agent_name:
                    pred = self.dqn.predict_next(
                        agent=other_agent, current_state=state, n=3  # Top 3 predictions
                    )
    ...
    def record_cooperation_event(
        self, event_type: str, other_agent: str, description: str, impact: float
    ) -> Any:
    ...
    def distribute_system_reward(
        self, all_agents: List[str], contributions: Dict[str, Dict], system_reward: float
    ) -> Dict[str, float]:
        """
