...
class RoleProfile:
    ...
    success_by_task_type: Dict[str, Tuple[int, int]] = field(
...
class SemanticEmbedder:
    ...
    def find_similar(
        self, query: str, candidates: List[str], threshold: float = 0.7, top_k: int = 5
...
class TransferableLearningStore:
    ...
    def _update_error_pattern(
        self, error: str, recovery_action: str, success: bool, reward: float
    ) -> Any:
        ...
        if success:
            ...
            pattern.contexts.append(
                {"recovery_action": recovery_action, "success": True, "timestamp": time.time()}
            )
        else:
            pattern.failure_count += 1
        pattern.total_reward += reward
    ...
    def record_session(
        ...
        session = {
            "session_id": hashlib.md5(f"{task_description}:{time.time()}".encode()).hexdigest()[
                :12
            ],
            "timestamp": time.time(),
    ...
    def get_execution_strategy(self, task_description: str, available_agents: List[str]) -> Dict:
        ...
        for session in relevant:
            for agent in session.get("agents_used", []):
                ...
                agent_success[agent]["time"] += session.get("total_time", 0) / max(
                    len(session.get("agents_used", [])), 1
                )
        ...
        for agent in available_agents:
            if agent in agent_success and agent_success[agent]["total"] >= 2:
                agent_scores[agent] = (
                    agent_success[agent]["success"] / agent_success[agent]["total"]
                )
            else:
    ...
    def save(self, path: str) -> None:
        ...
        logger.info(
            f"Saved transferable learnings: {len(self.task_patterns)} task patterns, "
    ...
    def load(self, path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Loaded transferable learnings: {len(self.task_patterns)} task patterns, "
