...
class CounterfactualLearner:
    ...
    async def analyze_episode(
        self, episode: StoredEpisode, agent_memories: Dict[str, SwarmMemory]
    ) -> Dict[str, Any]:
        """
        ...
        for result in all_results:
            ...
            if result.agent_name in agent_memories:
                ...
                relevant = mem.retrieve(query=episode.goal, goal=episode.goal, budget_tokens=3000)
            ...
            try:
                ...
                results["agent_analyses"][result.agent_name] = {"error": str(e)}

        return results

    def compute_counterfactual_credit(
        self, counterfactual_analysis: Dict, original_contributions: Dict[str, AgentContribution]
    ) -> Dict[str, float]:
        """
...
class PatternDiscovery:
    ...
    async def discover_patterns(
        self, successes: List[StoredEpisode], failures: List[StoredEpisode], domain: str = "general"
    ) -> Dict[str, Any]:
        """
...
class OfflineLearner:
    ...
    def __init__(self, config: Any) -> None:
        ...
        # Episode buffer
        self.buffer = PrioritizedEpisodeBuffer(capacity=config.episode_buffer_size)
    ...
    async def batch_update(
        self, agent_memories: Dict[str, SwarmMemory], current_episode: int
    ) -> Dict[str, Any]:
        """
    ...
    async def _experience_replay(self, agent_memories: Dict[str, SwarmMemory]) -> Dict[str, Any]:
    ...
    async def _counterfactual_analysis(
        self, agent_memories: Dict[str, SwarmMemory]
    ) -> Dict[str, Any]:
    ...
    async def _store_discovered_patterns(
        self, agent_memories: Dict[str, SwarmMemory], pattern_results: Dict[str, Any]
    ) -> Any:
...
""".strip()
        ...
        for memory in agent_memories.values():
            memory.store(
                ...
                context={"source": "offline_learning", "update": self.update_count},
