...
class TrajectoryPredictionSignature(dspy.Signature):
    ...
    acting_agent = dspy.InputField(desc="Which agent is about to act")
    proposed_action = dspy.InputField(desc="The action the agent is considering")
    other_agents = dspy.InputField(desc="List of other agents and their recent behaviors")
    agent_models = dspy.InputField(
        desc="Models of other agents' typical behaviors and cooperation styles"
    )
    horizon = dspy.InputField(desc="How many steps ahead to predict")
    goal = dspy.InputField(desc="The final goal we're trying to achieve")
    ...
    predicted_final_reward = dspy.OutputField(desc="Predicted reward at the end (0.0 to 1.0)")
...
class LLMTrajectoryPredictor:
    ...
    def __init__(self, config: Any, horizon: int = 5) -> None:
        ...
        self.predictor = (
            dspy.ChainOfThought(TrajectoryPredictionSignature) if DSPY_AVAILABLE else None
        )
    ...
    def predict(
        ...
        try:
            ...
            return PredictedTrajectory(
                horizon=self.horizon, steps=steps, predicted_reward=reward, confidence=confidence
            )
    ...
    def compute_divergence(
        self, predicted: PredictedTrajectory, actual: ActualTrajectory
    ) -> Divergence:
        """
        ...
        action_div = self._compute_action_divergence(predicted.steps, actual.steps)

        # State divergence: How different were the states?
        state_div = self._compute_state_divergence(predicted.steps, actual.steps)
    ...
    def _compute_action_divergence(
        self, predicted_steps: List[Dict], actual_steps: List[Dict]
    ) -> float:
    ...
    def _extract_learning(
        self, predicted: PredictedTrajectory, actual: ActualTrajectory, divergence: float
    ) -> str:
        ...
        for i, (pred, act) in enumerate(zip(predicted.steps, actual.steps)):
            ...
            if pred_agent and act_agent and pred_agent != act_agent:
                differences.append(f"Step {i}: Expected {pred_agent} but {act_agent} acted")
    ...
    def _update_agent_models(self, divergence: Divergence) -> Any:
        ...
        for step_pred, step_actual in zip(divergence.predicted.steps, divergence.actual.steps):
    ...
    def _fallback_prediction(self) -> PredictedTrajectory:
        ...
        return PredictedTrajectory(
            horizon=self.horizon, steps=[], predicted_reward=0.5, confidence=0.3
        )
...
class DivergenceMemory:
    ...
    def get_relevant(self, state: Dict[str, Any], k: int = 5) -> List[Divergence]:
        ...
        sorted_by_info = sorted(self.memories, key=lambda d: d.information_content, reverse=True)
...
class CooperativeCreditAssigner:
    ...
    def assign_credit(
        ...
        for step in trajectory.steps:
            ...
            coop_bonus = self._compute_cooperation_bonus(agent, step, trajectory.steps)

            # Prediction accuracy bonus: Was this agent predictable?
            pred_bonus = self._compute_prediction_bonus(agent, step, predictions.get(agent))

            # A-Team v8.0: Use adaptive weights
            credits[agent] = (
                task_success * (1 + coop_weight * coop_bonus) * (1 + pred_weight * pred_bonus)
            )
        ...
        if final_reward > 0.5:
            ...
            avg_coop = sum(
                self._compute_cooperation_bonus(a, s, trajectory.steps)
    ...
    def _compute_cooperation_bonus(self, agent: str, step: Dict, all_steps: List[Dict]) -> float:
        """Bonus for helping other agents succeed."""
        # Find steps after this agent
        agent_idx = next((i for i, s in enumerate(all_steps) if s.get("agent") == agent), -1)
        if agent_idx < 0:
            return 0.0
    ...
    def _compute_prediction_bonus(
        self, agent: str, actual_step: Dict, prediction: Optional[PredictedTrajectory]
    ) -> float:
    ...
    def save_state(self, path: str) -> None:
        ...
        logger.info(
            f"Saved CooperativeCreditAssigner state: coop={self.coop_weight_tracker.mean:.3f}, pred={self.pred_weight_tracker.mean:.3f}"
        )
    ...
    def load_state(self, path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Loaded CooperativeCreditAssigner state: coop={self.coop_weight_tracker.mean:.3f}, pred={self.pred_weight_tracker.mean:.3f}"
            )
            return True
