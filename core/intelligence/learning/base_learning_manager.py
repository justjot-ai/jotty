...
from Jotty.core.infrastructure.foundation.data_structures import SwarmConfig  # type: ignore[import]
...
class BaseLearningManager(ABC):
    ...
    @abstractmethod
    def end_episode(
        self, final_reward: float, success: bool = True, **kwargs: Any
    ) -> Dict[str, Any]:
        """
    ...
    @abstractmethod
    def get_value(
        self, state: Dict[str, Any], action: Optional[Any] = None, **kwargs: Any
    ) -> float:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        return {"learner_type": self.__class__.__name__, "config": self.config.to_flat_dict()}
...
class ValueBasedLearningManager(BaseLearningManager):
    ...
    @abstractmethod
    def get_best_action(
        self, state: Dict[str, Any], available_actions: List[Any]
    ) -> Tuple[Any, float, str]:
        """
...
class RewardShapingManager(BaseLearningManager):
    ...
    @abstractmethod
    def check_rewards(
        self, state: Dict[str, Any], action: Any, trajectory: Optional[List[Dict]] = None
    ) -> float:
        """
...
class MultiAgentLearningManager(BaseLearningManager):
    ...
    @abstractmethod
    def predict_agent_action(self, agent_name: str, state: Dict[str, Any]) -> Any:
        """
