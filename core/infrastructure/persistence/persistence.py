...
class Vault:
    ...
    def save_markovian_todo(self, todo: "SwarmTaskBoard") -> None:
        ...
        logger.info(
            f" Saved Markovian Task List: {len(todo.subtasks)} tasks, "
    ...
    def _serialize_todo(self, todo: Any) -> Dict:
        ...
        return {
            ...
            "subtasks": {
                task_id: self._serialize_task(task) for task_id, task in todo.subtasks.items()
            },
    ...
    def _format_todo_markdown(self, todo: Any) -> str:
        ...
        completed_tasks = [
            todo.subtasks[tid] for tid in todo.completed_tasks if tid in todo.subtasks
        ]
        if completed_tasks:
            # Sort by completion time (most recent first)
            completed_tasks.sort(
                key=lambda t: t.completed_at if t.completed_at else datetime.min, reverse=True
            )
        ...
        failed_tasks = [todo.subtasks[tid] for tid in todo.failed_tasks if tid in todo.subtasks]
        if failed_tasks:
        ...
        if completed > 0:
            ...
            if completed_with_time:
                avg_duration = sum(
                    (t.completed_at - t.started_at).total_seconds() for t in completed_with_time
    ...
    def save_memory(
        ...
        logger.info(
            f" Saved memory '{name}': {total_memories} memories across "
    ...
    def save_brain_state(self, brain: "SimpleBrain") -> None:
        ...
        if hasattr(brain, "preset"):
            brain_data["preset"] = str(
                brain.preset.value if hasattr(brain.preset, "value") else brain.preset
            )
