...
class TaskQueueManager:
    ...
    async def start(self) -> Any:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                self.running_tasks = {
                    tid: t for tid, t in self.running_tasks.items() if not t.done()
                }
    ...
    async def stop(self) -> Any:
        ...
        if self.running_tasks:
            ...
            await asyncio.wait(
                list(self.running_tasks.values()), timeout=60.0, return_when=asyncio.ALL_COMPLETED
            )
    ...
    async def _process_task(self, task: Task) -> Any:
        ...
        try:
            ...
            result = await self.conductor.run(goal=goal, max_iterations=100, **context)
            ...
            if task.retry_count < task.max_retries:
                ...
                logger.info(
                    f" Task will be retried: {task_id} (attempt {task.retry_count + 1}/{task.max_retries})"
                )
            else:
                await self.task_queue.update_status(
                    task_id,
