...
class ToolInterceptor:
    ...
    def _create_wrapper(self, tool_name: str, tool_func: Callable) -> Callable:
        ...
        def wrapped_tool(**kwargs: Any) -> Any:
            ...
            logger.info(
                f" [INTERCEPTOR] Tool call #{attempt_num}: {tool_name}({list(kwargs.keys())})"
            )
            ...
            call_record = ToolCall(
                ...
                metadata={"actor": self.actor_name},
            )
...
class ToolCallRegistry:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            actor_summaries = {
                name: interceptor.summary() for name, interceptor in self._interceptors.items()
            }
