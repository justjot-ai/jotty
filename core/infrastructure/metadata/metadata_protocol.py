    ...
    async def __call__(self, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if self.meta.is_async:
                result = await asyncio.wait_for(method(**kwargs), timeout=self.meta.timeout)
            else:
                # Run sync method in executor to avoid blocking
                loop = asyncio.get_running_loop()
                result = await asyncio.wait_for(
                    loop.run_in_executor(None, lambda: method(**kwargs)), timeout=self.meta.timeout
                )
...
class MetadataValidator:
    ...
    def check(self, metadata_obj: Any) -> bool:
        ...
        validate_method = getattr(metadata_obj, "__jotty_validate__", None) or getattr(
            metadata_obj, "__reval_validate__", None
        )
        if validate_method:
