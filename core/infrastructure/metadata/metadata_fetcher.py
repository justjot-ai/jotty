...
class MetaDataFetcherSignature(dspy.Signature):
    ...
    query: str = dspy.InputField(desc="User's natural language query that needs metadata")
...
class MetaDataFetcher:
    ...
    def _fetch_with_retry(
        self, query: str, previous_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _execute_fetch(
        ...
        if self._react_agent is None:
            self._react_agent = dspy.ReAct(
                signature=MetaDataFetcherSignature, tools=self.tools, max_iters=self.react_max_iters
            )
        ...
        if (
            "filter_conditions" not in fetched_data
            and "get_all_filter_definitions" not in fetched_data
        ):
            ...
            try:
                ...
                if hasattr(self.metadata_provider, "get_all_filter_definitions"):
                    ...
                    if filter_text:
                        ...
                        logger.info(
                            f" Manually fetched filter conditions ({len(filter_text)} chars)"
                        )
            except Exception as e:
    ...
    def _generate_tool_catalog(self) -> str:
        ...
        for tool_name, tool_meta in self.tool_metadata.items():
            ...
            if tool_meta.parameters:
                params_str = ", ".join([f"{k}: {v}" for k, v in tool_meta.parameters.items()])
    ...
    def _parse_react_output(self, result: Any) -> Dict[str, Any]:
        ...
        if hasattr(result, "fetched_metadata") and result.fetched_metadata:
            try:
                ...
                logger.debug(
                    " fetched_metadata is not valid JSON, relying on trajectory extraction"
                )
...
Return ONLY the JSON, no other text."""
    ...
    def _add_to_cache(self, query: str, data: Dict[str, Any]) -> Any:
        ...
        entry = CacheEntry(
            data=data, timestamp=datetime.now(), ttl=self.default_cache_ttl, query_hash=query_hash
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        avg_fetch_time = (
            self.total_fetch_time / self.total_fetches if self.total_fetches > 0 else 0.0
        )
        ...
        return {
            ...
            "tool_stats": {
                name: {
                    ...
                    "success_rate": (
                        f"{(meta.success_count/meta.call_count*100):.0f}%"
                        if meta.call_count > 0
                        else "N/A"
                    ),
                    "avg_time": (
                        f"{(meta.total_time/meta.call_count):.2f}s"
                        if meta.call_count > 0
                        else "N/A"
                    ),
                }
