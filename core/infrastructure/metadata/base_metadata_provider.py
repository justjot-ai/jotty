...
class BaseMetadataProvider:
    ...
    def __init__(
        self, token_budget: int = 50000, enable_caching: bool = True, **kwargs: Any
    ) -> None:
        """
    ...
    def _auto_register_methods(self) -> Any:
        ...
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            ...
            self._method_descriptions[name] = (
                inspect.getdoc(method) or f"Call {name} on metadata provider"
            )
    ...
    def get_tool_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_pct": (
                (self._tokens_used / self._token_budget) * 100 if self._token_budget > 0 else 0
            ),
        ...
        def safeguarded_tool(*args: Any, **kwargs: Any) -> Any:
            ...
            if self._enable_caching:
                ...
                if cache_key in self._tool_cache:
                    ...
                    logger.info(
                        f" metadata_{name}(*{args[:2] if args else []}, **{list(kwargs.keys())[:2]}) → CACHE HIT"
                    )
            ...
            logger.info(
                f" metadata_{name}(*{args[:2] if args else []}, **{list(kwargs.keys())[:2]}) [call #{self._tool_call_count}]"
            )
            ...
            try:
                ...
                logger.info(
                    f" Budget: {self._tokens_used:,} / {self._token_budget:,} ({usage_pct:.1f}% used)"
                )
