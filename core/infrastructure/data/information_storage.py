...
class SurpriseEstimator:
    ...
    async def estimate_surprise(
        self, event: Dict[str, Any], context: Dict[str, Any], historical_patterns: List[str]
    ) -> Tuple[float, str]:
        """
...
class InformationTheoreticStorage:
    ...
    def __init__(
        self, alpha: float = 0.5, min_info_threshold: float = 0.5, max_info_threshold: float = 3.0
    ) -> None:
        self.alpha = alpha
    ...
    async def store(
        self, event: Dict[str, Any], context: Dict[str, Any], raw_content: str
        ...
        info_content, detail_level, freq_est, surprise = await self.compute_information_content(
            event, context
        )
    ...
    def _summarize_event(self, event: Dict[str, Any]) -> str:
        ...
        return (
            f"{event.get('agent', '?')}: {event.get('action', '?')} -> {event.get('outcome', '?')}"
        )
...
══════════════════════════════════════════════════════════════"""
    ...
    def _high_detail(self, event: Dict[str, Any], context: Dict[str, Any], raw_content: str) -> str:
...
Key Content: {raw_content}"""
    ...
    def _normal_detail(self, event: Dict[str, Any], raw_content: str) -> str:
        """Normal detail for typical events."""
        return (
            f"Event: {event.get('type', event.get('action', 'unknown'))} | "
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_info_content": sum(m.information_content for m in self.memories.values())
...
__all__ = ["InformationWeightedMemory", "SurpriseEstimator", "InformationTheoreticStorage"]
