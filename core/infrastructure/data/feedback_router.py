...
from ..agent.feedback_channel import FeedbackMessage, FeedbackType
...
class AgenticFeedbackSignature(dspy.Signature):
    ...
    feedback_type = dspy.OutputField(
        desc="Type of feedback: 'error_correction', 'refinement', 'consultation'"
    )
...
class AgenticFeedbackRouter:
    ...
    def _parse_target_agents_json(self, target_agents_json: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(
                " AgenticFeedbackRouter: target_agents was not valid JSON; treating as no targets."
            )
            return []
