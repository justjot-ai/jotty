...
class ActorOutput:
    ...
    def get_exploratory(self) -> List[Any]:
        ...
        return [
            a for a in self.tagged_attempts if hasattr(a, "is_exploratory") and a.is_exploratory()
        ]
...
class IOManager:
    ...
    def register_output(
        ...
        success: bool = True,
        error: Optional[str] = None,
        tagged_attempts: Optional[List[Any]] = None,
    ) -> Any:
        """
        ...
        logger.info(
            f" [IOManager RECV] tagged_attempts: {len(tagged_attempts) if tagged_attempts else 0}"
        )
        if output and hasattr(output, "_store"):
            logger.info(
                f" [IOManager RECV] output has _store with keys: {list(output._store.keys())}"
            )
        if output and hasattr(output, "__dict__"):
            logger.info(
                f" [IOManager RECV] output.__dict__ keys: {list(output.__dict__.keys())[:10]}"
            )
        ...
        logger.info(
            f" [IOManager EXTRACT] Extracted {len(output_fields)} fields: {list(output_fields.keys())}"
        )
        if output_fields:
            for key, value in list(output_fields.items())[:5]:  # Show first 5
                value_preview = str(value)[:100] if value is not None else "None"
                logger.info(
                    f" [IOManager EXTRACT] - {key}: {type(value).__name__} = {value_preview}"
                )
    ...
    def _extract_output_fields(
        self, output: Any, signature: Optional[Type[dspy.Signature]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if TAGGED_OUTPUT_AVAILABLE and isinstance(output, TaggedOutput):
            ...
            if best and best.tag == "correct":
                ...
                logger.warning(
                    f" [TAGGED EXTRACTION] No correct attempts found (total: {len(output.all_attempts)})"
                )
                logger.info(
                    f" [FALLBACK] Checking TaggedOutput direct fields: final_output={output.final_output is not None}"
                )
                ...
                output_fields = {
                    ...
                    "execution_result": (
                        output.final_result
                        if hasattr(output, "final_result")
                        else (best.error_message if best else "No attempts")
                    ),
                    ...
                    "sql_query": (
                        output.final_output if isinstance(output.final_output, str) else ""
                    ),
                    "query_explanation": output.explanation,
                }
        ...
        if hasattr(output, "__dict__"):
            for key, value in output.__dict__.items():
                if not key.startswith(
                    "_"
                ):  # Skip private attributes (except _store which we handled)
    ...
    def to_dict(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {actor_name: output.output_fields for actor_name, output in self.outputs.items()}
    ...
    def __repr__(self) -> str:
        return (
            f"IOManager(" f"actors={len(self.outputs)}, " f"execution_order={self.execution_order})"
        )
