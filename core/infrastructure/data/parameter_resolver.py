...
class ParameterMatchingSignature(dspy.Signature):
    ...
    best_match_key: str = dspy.OutputField(
        desc="The key of the best matching data, or 'NO_MATCH' if no good match"
    )
    confidence: float = dspy.OutputField(
        desc="Confidence in match (0.0-1.0). Use NO_MATCH if confidence < 0.7"
    )
    semantic_explanation: str = dspy.OutputField(
        desc="Why this data semantically matches the parameter need"
    )
...
class AgenticParameterResolver(dspy.Module):
    ...
    def resolve_parameter(
        ...
        type_name = (
            parameter_type.__name__ if hasattr(parameter_type, "__name__") else str(parameter_type)
        )
        ...
        try:
            ...
            logger.info(
                f" [AGENTIC RESOLVER] Best match: '{best_match}' (confidence: {confidence:.2f})"
            )
            ...
            if best_match == "NO_MATCH" or confidence < min_confidence:
                logger.warning(
                    f" [AGENTIC RESOLVER] No confident match found (confidence: {confidence:.2f})"
                )
