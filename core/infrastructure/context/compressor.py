...
class CompressionSignature(dspy.Signature):
    ...
    high_impact_items = dspy.InputField(
        desc="Items with HIGH Shapley credit (preserve these first!)"
    )
    low_impact_items = dspy.InputField(
        desc="Items with LOW Shapley credit (can be compressed/removed)"
    )
...
class AgenticCompressor:
    ...
    async def compress(
        ...
        logger.info(
            f" Compressed: {current_tokens} → {compressed_tokens} tokens ({result.compression_ratio})"
        )
    ...
    async def compress_simple(
        self, data: str, target_ratio: float = 0.5, preserve_critical: bool = False
    ) -> str:
        """
        ...
        """
        logger.info(
            f" [AgentSlack API] Compressing: {len(data)} chars, target_ratio={target_ratio}"
        )
        ...
        if preserve_critical:
            ...
            lines = data.split("\n")
            critical_lines = [
                l for l in lines if "CRITICAL" in l.upper() or "IMPORTANT" in l.upper()
            ]
            ...
            logger.info(
                f" Compressed with CRITICAL preservation: {len(data)} → {len(result)} chars"
            )
            return result
        else:
            # Simple truncation
    ...
    def get_stats(self) -> Dict:
        ...
        avg_quality = (
            sum(float(s.get("quality", 0)) for s in self.compression_stats) / total_compressions
        )
