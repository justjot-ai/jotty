...
class MemoryGradientSignature(dspy.Signature):
    ...
    experience_summary = dspy.InputField(
        desc="Summary of what happened (state, action, outcome)"
    )
    ...
    key_lesson = dspy.OutputField(
        desc="The key lesson to remember (actionable insight)"
    )
    lesson_confidence = dspy.OutputField(
        desc="Confidence in this lesson (0.0-1.0)"
    )
    when_to_apply = dspy.OutputField(
        desc="When should this lesson be applied? (conditions)"
    )
    related_lessons = dspy.OutputField(
        desc="How does this relate to past lessons? (connections)"
    )
...
class CooperationGradientSignature(dspy.Signature):
    ...
    cooperation_event = dspy.InputField(
        desc="Description of cooperation (or lack thereof)"
    )
    ...
    recommendation = dspy.OutputField(
        desc="How should agents cooperate differently next time?"
    )
