...
class UnifiedLMProvider:
    ...
    @staticmethod
    def create_lm(
        provider: str, model: Optional[str] = None, inject_context: bool = True, **kwargs: Any
    ) -> BaseLM:
        """
    ...
    def _create_direct_lm(provider: str, model: Optional[str] = None, **kwargs: Any) -> BaseLM:
        ...
        from Jotty.core.infrastructure.foundation.config_defaults import DEFAULTS, MODEL_ALIASES
    ...
    def _create_zen_lm(model: Optional[str] = None, **kwargs: Any) -> BaseLM:
        ...
        if model not in UnifiedLMProvider.ZEN_MODELS:
            ...
            free_matches = [m for m in UnifiedLMProvider.ZEN_FREE_MODELS if model in m]
            all_matches = [m for m in UnifiedLMProvider.ZEN_MODELS if model in m]
