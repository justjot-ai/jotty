    ...
    _FIELDS = frozenset(
        {
            ...
            "max_q_table_size",
            "q_prune_percentage",
            "enable_domain_transfer",
            "enable_rl",
            "rl_verbosity",
            "gamma",
            "lambda_trace",
            "alpha",
            "enable_adaptive_alpha",
            "alpha_min",
            "alpha_max",
            "alpha_adaptation_rate",
            "q_value_mode",
            # Intermediate rewards
            "enable_intermediate_rewards",
            "architect_proceed_reward",
            "tool_success_reward",
            # Cooperative rewards
            "base_reward_weight",
            "cooperation_bonus",
            ...
            "instability_threshold_multiplier",
            "slow_learning_threshold",
            "goal_transfer_discount",
            # Exploration
            "epsilon_start",
            "epsilon_end",
            "epsilon_decay_episodes",
            "ucb_coefficient",
            "enable_adaptive_exploration",
            "exploration_boost_on_stall",
            "max_exploration_iterations",
            "policy_update_threshold",
            # Credit assignment
            "credit_decay",
            "min_contribution",
            "enable_reasoning_credit",
            "reasoning_weight",
            "evidence_weight",
            # Consolidation
            ...
            "learning_components",
            # Goal hierarchy
            "enable_goal_hierarchy",
            "goal_transfer_weight",
            # Causal learning
            "enable_causal_learning",
            "causal_confidence_threshold",
            "causal_min_support",
            # Protection
            "protected_memory_threshold",
            "suspicion_threshold",
            "min_rejection_rate",
        }
    )
    ...
    _FIELDS = frozenset(
        {
            ...
            "enable_validation",
            "validation_mode",
            "require_all_architect",
            "require_all_auditor",
            "enable_per_actor_swarm_auditor",
            "enable_final_swarm_auditor",
            "swarm_validation_confidence_threshold",
            "min_confidence",
            "default_confidence_on_error",
            "default_confidence_no_validation",
            "default_confidence_insight_share",
            "default_estimated_reward",
            "enable_multi_round",
            "refinement_on_low_confidence",
            "refinement_on_disagreement",
...
class SwarmIntelligenceView(_ConfigView):
    ...
    _FIELDS = frozenset(
        {
            "trust_decrease_on_struggle",
            "trust_increase_on_excel",
            "trust_min",
            "adaptation_interval",
            "adaptation_struggle_threshold",
            "adaptation_excel_threshold",
            "stigmergy_routing_threshold",
            "morph_min_rcs",
...
class SwarmLearningConfig:
    ...
    max_eval_iters: int = (
        1  # Architect/Auditor ReAct iterations (1=minimal, 2-3=balanced, 5-10=thorough)
    )
    ...
    token_model_name: Optional[str] = (
        None  # Override model name for token counting (e.g., 'gpt-4o')
    )
    ...
    consolidation_interval: int = (
        3  # A-TEAM: Consolidate every 3 episodes (prevent memory buildup!)
    )
    ...
    enable_per_actor_swarm_auditor: bool = (
        False  # If True, run swarm Auditor after EACH actor (slow!)
    )
    ...
    def _validate_via_focused_configs(self) -> None:
        ...
        for method_name in [
            "to_memory_config",
            "to_learning_config",
            "to_context_budget_config",
            "to_execution_config",
            "to_persistence_config",
            "to_validation_config",
            "to_monitoring_config",
            "to_intelligence_config",
        ]:
            try:
    ...
    def to_intelligence_config(self) -> "IntelligenceConfig":
        ...
        return IntelligenceConfig(**{f: getattr(self, f) for f in SwarmIntelligenceView._FIELDS})
