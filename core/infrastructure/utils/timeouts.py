    ...
    def record_failure(self, error: Exception) -> None:
        ...
        if self.state == CircuitState.CLOSED:
            if self.failure_count >= self.config.failure_threshold:
                ...
                logger.error(
                    f" [{self.config.name}] Circuit OPEN! {self.failure_count} consecutive failures"
                )
...
from Jotty.core.infrastructure.foundation.exceptions import TimeoutError  # noqa: F811
...
class DeadLetterQueue:
        ...
        while self.queue:
            ...
            try:
                ...
                if failed_op.retry_count < 3:
                    self.queue.append(failed_op)
                    logger.warning(
                        f" DLQ: Retry failed for '{failed_op.operation_name}' (attempt {failed_op.retry_count}/3)"
                    )
                else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.total_successful_retries / self.total_retried
...
LLM_CIRCUIT_BREAKER = CircuitBreaker(
    CircuitBreakerConfig(name="llm_calls", failure_threshold=5, timeout=60.0)
)

TOOL_CIRCUIT_BREAKER = CircuitBreaker(
    CircuitBreakerConfig(name="tool_calls", failure_threshold=3, timeout=30.0)
)
...
ADAPTIVE_TIMEOUT = AdaptiveTimeout(initial=30.0, percentile=95.0)
