...
def load_ohlcv(
    ticker: str, timeframe: str, data_path: str = DEFAULT_DATA_PATH
        ...
        for i in range(1, len(df)):
            if df["close"].iloc[i] > supertrend[i - 1]:
                supertrend[i] = (
                    ...
                    or supertrend[i - 1] < lower_band.iloc[i]
                ...
                supertrend[i] = (
                    ...
                    or supertrend[i - 1] > upper_band.iloc[i]
...
def get_latest_indicators(df: "pd.DataFrame") -> Dict[str, Any]:
    ...
    if "MACD_12_26_9" in df.columns:
        indicators["macd"] = (
            round(float(latest["MACD_12_26_9"]), 2) if not pd.isna(latest["MACD_12_26_9"]) else None
        )
        indicators["macd_signal"] = (
            round(float(latest.get("MACDs_12_26_9", 0)), 2)
            if not pd.isna(latest.get("MACDs_12_26_9"))
            else None
        )

    if "ADX_14" in df.columns:
        indicators["adx"] = (
            round(float(latest["ADX_14"]), 2) if not pd.isna(latest["ADX_14"]) else None
        )

    if "BBL_20_2.0" in df.columns:
        indicators["bb_lower"] = (
            round(float(latest["BBL_20_2.0"]), 2) if not pd.isna(latest["BBL_20_2.0"]) else None
        )
        indicators["bb_upper"] = (
            round(float(latest["BBU_20_2.0"]), 2) if not pd.isna(latest["BBU_20_2.0"]) else None
        )

    if "STOCHk_14_3_3" in df.columns:
        indicators["stoch_k"] = (
            round(float(latest["STOCHk_14_3_3"]), 2)
            if not pd.isna(latest["STOCHk_14_3_3"])
            else None
        )
        indicators["stoch_d"] = (
            round(float(latest.get("STOCHd_14_3_3", 0)), 2)
            if not pd.isna(latest.get("STOCHd_14_3_3"))
            else None
        )
        ...
        try:
            ...
            if "pivot" in df.columns:
                for key, target in [
                    ("s1", "support_levels"),
                    ("s2", "support_levels"),
                    ("r1", "resistance_levels"),
                    ("r2", "resistance_levels"),
                ]:
