...
class VisualizationSpecSignature(dspy.Signature):
    ...
    concept_description: str = dspy.InputField(
        desc="Description of the concept including why it matters and how it works"
    )

    chart_type: str = dspy.OutputField(
        desc="One of: bar_chart, line_chart, heatmap, scatter, pie, none"
    )
    ...
    data_json: str = dspy.OutputField(
        desc='JSON string with chart data. For bar_chart: {"labels": [...], "values": [...]}. For line_chart: {"labels": [...], "series": [{"name": "...", "values": [...]}]}. For heatmap: {"x_labels": [...], "y_labels": [...], "values": [[...]]}. For scatter: {"x": [...], "y": [...], "labels": [...]}. For pie: {"labels": [...], "values": [...]}'
    )
    annotation: str = dspy.OutputField(
        desc="Key insight to annotate on the chart - what should the reader notice?"
    )
...
def _apply_syntax_highlighting(code: str) -> str:
    ...
    for line in lines:
        ...
        for kw, marker in keywords:
            line = re.sub(rf"(?<![a-zA-Z_])({kw})(?![a-zA-Z_0-9])", f"__X{marker}X__", line)
...
class VisualizationRenderer:
    ...
    def _auto_format_labels(cls, labels: list) -> dict:
        ...
        all_numeric = all(
            ...
            or (isinstance(l, str) and l.replace(".", "", 1).replace("-", "", 1).isdigit())
            for l in labels
        )
        if all_numeric:
            return {"fontsize": 8, "rotation": 0, "ha": "center", "labels": labels}
            ...
            if chart_type in ("bar_chart", "pie"):
                ...
                if values:
                    ...
                    if x_labels and len(x_labels) != max_cols:
                        data["x_labels"] = (
                            x_labels[:max_cols]
                            if len(x_labels) > max_cols
                            else x_labels + [f"C{i}" for i in range(len(x_labels), max_cols)]
                        )
                    if y_labels and len(y_labels) != len(rect_values):
                        data["y_labels"] = (
                            y_labels[: len(rect_values)]
                            if len(y_labels) > len(rect_values)
                            else y_labels
                            + [f"R{i}" for i in range(len(y_labels), len(rect_values))]
                        )
    ...
    def render(
        ...
        try:
            ...
            fig.add_artist(
                plt.Line2D(
                    [0.12, 0.88],
                    [title_bottom - 0.01, title_bottom - 0.01],
    ...
    def _render_bar_chart(cls, ax: Any, data: dict, x_label: str, y_label: str) -> None:
        ...
        ax.set_xticklabels(
            display_labels, fontsize=fmt["fontsize"], rotation=fmt["rotation"], ha=fmt["ha"]
        )
        ...
        for i, series in enumerate(series_list):
            ...
            try:
                ...
                if len(plot_x) >= 4:
                    ...
                    # Data point markers
                    ax.scatter(
                        plot_x, vals, color=color, s=30, zorder=4, edgecolors="white", linewidth=1.2
                    )
                else:
    ...
    def _render_heatmap(cls, ax: Any, data: dict, x_label: str, y_label: str) -> None:
        ...
        ax.set_xticklabels(
            x_fmt["labels"], fontsize=x_fmt["fontsize"], rotation=x_fmt["rotation"], ha=x_fmt["ha"]
        )
    ...
    def _render_scatter(cls, ax: Any, data: dict, x_label: str, y_label: str) -> None:
        ...
        ax.scatter(
            x, y, c=colors[: len(x)], s=s, edgecolors="white", linewidth=1.0, zorder=3, alpha=0.85
        )
        ax.set_xlabel(x_label, fontsize=9, color="#374151", labelpad=8)
        ax.set_ylabel(y_label, fontsize=9, color="#374151")
        ax.spines["top"].set_visible(False)
        ...
        if point_labels:
            ...
            for i, label in enumerate(point_labels[: len(x)]):
                ...
                ax.annotate(
                    ...
                    fontweight="medium",
                    arrowprops=(
                        dict(arrowstyle="-", color="#d1d5db", lw=0.5)
                        if len(point_labels) <= 12
                        else None
                    ),
                )
    ...
    def _render_pie(cls, ax: Any, data: dict, x_label: str, y_label: str) -> None:
        ...
        wedges, texts, autotexts = ax.pie(
            ...
            colors=colors[: len(display_labels)],
            autopct="%1.0f%%",
            pctdistance=0.78,
...
async def generate_concept_visualization(
    concept_name: str, concept_description: str, output_dir: str = "/tmp", **kwargs: Any
) -> Optional[str]:
    """
    ...
    try:
        ...
        spec = spec_generator(concept_name=concept_name, concept_description=concept_description)
...
async def render_latex_to_image(latex_expr: str, output_dir: str = "/tmp") -> Optional[str]:
    """
...
def _convert_markdown_content(text: str) -> str:
    ...
    lang_ids = (
        r"python|javascript|js|typescript|ts|java|cpp|c\+\+|ruby|go|rust|bash|sh|sql|html|css"
    )
    text = re.sub(rf"\n\s*({lang_ids})\s*\n", "\n", text)
    ...
    text = re.sub(r"(?:^    .+$\n?){2,}", detect_indented_block, text, flags=re.MULTILINE)
    ...
    # e.g., "π", "Σ", "∇G"
    text = re.sub(rf"({math_symbols_pattern}[A-Za-z_]*)", wrap_math_expr, text)

    # PATTERN 4: Subscript/superscript notation
    # e.g., "x_i", "a_1", "π_new"
    text = re.sub(
        r"(?<![a-zA-Z])([a-zA-Z]+_(?:new|old|[a-z0-9]+))(?![a-zA-Z_])", wrap_math_expr, text
    )

    # PATTERN 5: Common math expressions without Greek letters
    # e.g., "R(s,a)", function notation
    text = re.sub(r"(?<![a-zA-Z])([A-Z]\([a-z],[a-z]\))(?![a-zA-Z])", wrap_math_expr, text)
