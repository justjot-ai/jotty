...
from .content_analysis import (
    ...
    MetaSchemaRegistry,
    PatternCrystallizer,
    ResearchTemplateStructure,
)
from .slide_selection import (
    LLMSlideSelector,
    SlideTypeSelector,
    VisualizationGoal,
    VisualizationGoalGenerator,
)
...
class HTMLSlideGenerator(SlideAssetsMixin, SlideRenderersMixin):
    ...
    def add_slide(
        ...
        self.slides.append(
            SlideConfig(
                slide_type=slide_type, data=data, animation=animation, delay_start=delay_start
            )
        )
    ...
    def _format_text(self, text: str) -> str:
        ...
        text = re.sub(
            r"\*\*(.+?)\*\*",
            r'<strong class="text-white font-semibold">\1</strong>',
            text,
            flags=re.DOTALL,
        )
    ...
    def _format_content(self, text: str) -> str:
        ...
        text = re.sub(
            r"^####\s+(.+)$",
            r'<h5 class="text-lg font-semibold text-white mt-4 mb-2">\1</h5>',
            text,
            flags=re.MULTILINE,
        )
        text = re.sub(
            r"^###\s+(.+)$",
            r'<h4 class="text-xl font-semibold text-white mt-4 mb-2">\1</h4>',
            text,
            flags=re.MULTILINE,
        )
        ...
        text = re.sub(
            r"^-\s+(.+)$",
            r'<div class="pl-3 border-l-2 border-orange-400/50 my-2">\1</div>',
            text,
            flags=re.MULTILINE,
        )
        text = re.sub(
            r"^(\d+)\.\s+(?!<strong)(.+)$",
            r'<div class="pl-3 border-l-2 border-blue-400/50 my-2"><span class="text-blue-400 font-semibold mr-2">\1.</span>\2</div>',
            text,
            flags=re.MULTILINE,
        )
...
class LearningSlideBuilder:
    ...
    def _select_slide_type_for_content(
        ...
        selected_type, analysis = self.type_selector.select_slide_type(
            content=content, title=title, level=level, force_variety=True, preferred_types=preferred
        )
    ...
    def _extract_comparison_from_content(self, content: str) -> Tuple[Dict, Dict]:
        ...
        parts = re.split(
            r"\b(?:however|but|in contrast|whereas|while)\b", content, flags=re.IGNORECASE
        )
        if len(parts) >= 2:
    ...
    def _create_timeline_from_content(self, content: str) -> List[Dict]:
        ...
        if not events:
            phase_matches = re.findall(
                r"(?:phase|stage|step)\s*(\d+)[:\s]+([^.]+\.)", content, re.IGNORECASE
            )
    ...
    def build_from_paper_data(self, paper_data: Dict) -> str:
        ...
        logger.info(
            f"🎨 Building intelligent presentation: {len(concepts)} concepts, {len(sections)} sections"
        )
        ...
        # ==========================================
        self.generator.add_slide(
            SlideType.STATS_GRID,
            {
                "label": "Paper Overview",
                "title": "What You'll Learn",
                "stats": [
                    {
                        "value": str(len(concepts)) if concepts else "3+",
                        "label": "Key Concepts",
                        "color": "blue",
                    },
                    {
                        "value": str(len(sections)) if sections else "5+",
                        "label": "Deep Dives",
                        "color": "purple",
                    },
                    {"value": paper_data.get("year", "2025"), "label": "Year", "color": "orange"},
                    {
                        "value": paper_data.get("learning_time", "20-30 min"),
                        "label": "Learning Time",
                        "color": "green",
                    },
                ],
            ...
            if slide_type == SlideType.FORMULA and metadata.get("has_math", False):
                ...
                self.generator.add_slide(
                    SlideType.TWO_COLUMN,
                    {
                        "label": f"Concept {i+1}",
                        "title": name,
                        "left": {"title": "What It Is", "content": description, "icon": icon},
                        "right": {
                            "title": "Why It Matters",
                            "content": (
                                why_matters
                                if why_matters
                                else "Understanding this concept is key to grasping the paper's contribution."
                            ),
                        },
                    },
                )
        ...
        for i, section in enumerate(sections):
            ...
            if level == 3 or "math" in title.lower() or self.llm_selector._has_latex(content):
                ...
                for word in ["versus", "vs", "compared to", "unlike", "whereas"]
            ):
            ...
            logger.debug(
                f"📊 Section '{title[:30]}...': type={content_type}, selected={selected_type_str}"
            )
            ...
            # ===== MATH SECTIONS =====
            elif content_type == "math" and metadata.get(
                "has_math", self.llm_selector._has_latex(content)
            ):
                ...
                if main_formula:
                    ...
                    left_content = (
                        ". ".join(sentences[:mid]) + "."
                        if mid > 0
                        else content[: len(content) // 2]
                    )
                    right_content = (
                        ". ".join(sentences[mid:]) if mid > 0 else content[len(content) // 2 :]
                    )
                    self.generator.add_slide(
                        SlideType.TWO_COLUMN,
                        {
                ...
                self.generator.add_slide(
                    SlideType.BEFORE_AFTER,
                    {
                        ...
                        "before": (
                            before
                            if before["items"]
                            else {"title": "Challenge", "items": [content[:100]]}
                        ),
                        "after": (
                            after
                            if after["items"]
                            else {"title": "Solution", "items": ["Addressed by this approach"]}
                        ),
                    },
                )
            ...
            # ===== GENERAL SECTIONS - Use TWO_COLUMN for consistency =====
                ...
                self.generator.add_slide(
                    SlideType.TWO_COLUMN,
                    {
                        ...
                        "title": title,
                        "left": {
                            "title": level_icons.get(level, "📖") + " Understanding",
        ...
        logger.info(
            f"🎨 Slide variety score: {variety:.1%} ({len(set(self.type_selector.used_types))} unique types)"
        )
        ...
        if methodology:
            self.generator.add_slide(
                ...
                {"label": "Methodology", "title": "How It Works", "steps": methodology[:6]},
            )
        ...
        if key_insights:
            self.generator.add_slide(
                SlideType.BULLET_POINTS,
                {
                    ...
                    "points": [
                        {
                            "title": ins if isinstance(ins, str) else ins.get("title", ""),
                            "description": (
                                "" if isinstance(ins, str) else ins.get("description", "")
                            ),
                        }
        ...
        if paper_data.get("key_quote"):
            self.generator.add_slide(
                SlideType.QUOTE,
                {
                    ...
                    "author": (
                        paper_data.get("authors", [""])[0] + " et al."
        ...
        self.generator.add_slide(
            SlideType.QA,
            {
                ...
                "cta_secondary": {
                    "label": "Read Paper",
                    "icon": "📄",
                    "url": f"https://arxiv.org/abs/{paper_data.get('arxiv_id', '')}",
                },
            },
        )
...
if __name__ == "__main__":
    ...
    sample_paper = {
        ...
        "concepts": [
            ...
            },
            {
                ...
                "description": "Parallel attention layers for diverse representations",
                "icon": "⚡",
                "formula": "h=8",
            },
            {
                "name": "Positional Encoding",
                "description": "Inject sequence order with sinusoidal functions",
                "icon": "📍",
                "formula": "sin(pos/10000^(2i/d))",
            },
        ],
        "methodology_steps": [
        ...
        "comparison": {
            ...
            "after": {
                "title": "Transformer",
                "items": ["Parallel processing", "Full parallelization", "Stable gradients"],
            },
        },
        "timeline": [
            {
                "year": "2017",
                "title": "Transformer",
                "description": "Original paper introduces attention-only architecture",
                "highlight": True,
            },
            {
                "year": "2018",
                "title": "BERT & GPT",
                "description": "Pre-training revolution begins",
            },
            {"year": "2020", "title": "GPT-3", "description": "175B parameters, few-shot learning"},
            {
                "year": "2023+",
                "title": "GPT-4, Claude",
                "description": "Multimodal, reasoning, agents",
            },
        ],
        ...
        "takeaways": [
            {
                ...
                "description": "No more sequential processing bottleneck",
            },
            {"title": "Multi-head provides diversity", "description": "8 parallel attention heads"},
            {
                "title": "Positional encoding preserves order",
                "description": "Sinusoidal functions inject position",
            },
            {
                "title": "Residual connections enable depth",
                "description": "Skip connections and layer norm",
            },
        ],
    }
    ...
    # Save test output
    output_path = (
        "/var/www/sites/personal/stock_market/Jotty/learning-slides/public/generated-test.html"
    )
