...
async def comprehensive_stock_research_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        title = params.get(
            "title", f"{company_name} ({ticker}){location_suffix} - Comprehensive Research Report"
        )
        ...
        if not search_tool:
            return {"success": False, "error": "search_web_tool not found in web-search skill"}
        ...
        search_tasks = [
            run_search(query, aspect_name) for aspect_name, query in search_queries.items()
        ]
        ...
        logger.info(
            f"📊 Total research data: {total_results} results, sending top {total_snippets} snippets to Claude"
        )
...
**START WRITING THE COMPLETE REPORT NOW. BEGIN WITH THE HEADER AND EXECUTIVE SUMMARY, THEN CONTINUE WITH ALL SECTIONS IN FULL DETAIL. DO NOT STOP UNTIL YOU HAVE WRITTEN ALL {target_pages} PAGES OF CONTENT:**"""
        ...
        logger.info(
            f"📝 Generating comprehensive {target_pages}-page report (target: 5,000-8,000 words)"
        )
...
"""
        ...
        if not write_tool:
            return {"success": False, "error": "write_file_tool not found in file-operations skill"}
