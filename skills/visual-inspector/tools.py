...
class VLMClient:
    ...
    def analyze_image(self, image_path: str, prompt: str,
                      detail: str = "high", max_tokens: int = 2000) -> Dict[str, Any]:
    ...
    def analyze_batch(self, image_paths: List[str], prompt: str,
                      detail: str = "high", max_tokens: int = 2000) -> List[Dict[str, Any]]:
        """Analyze multiple images in parallel."""
        with ThreadPoolExecutor(max_workers=min(len(image_paths), 5)) as executor:
            futures = [executor.submit(self.analyze_image, img, prompt, detail, max_tokens)
...
class FileConverter:
    ...
    TEXT_EXTS = {
        ...
        '.h', '.hpp', '.rb', '.php', '.swift', '.kt', '.sql', '.r',
        '.lua', '.vim', '.conf', '.ini', '.env', '.dockerfile',
        '.gitignore', '.csv', '.log',
    }
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(
                    ['pygmentize', '-f', 'png', '-l', ext or 'text',
                     '-O', 'font_size=14,line_numbers=True,style=monokai',
                     '-o', save_path, file_path],
    ...
    def _convert_svg(svg_path: str, save_path: str) -> Dict[str, Any]:
        try:
            result = subprocess.run(['convert', svg_path, save_path],
                                    capture_output=True, timeout=10)
            if result.returncode == 0 and os.path.exists(save_path):
    ...
    def pdf_pages_to_images(pdf_path: str, output_dir: str) -> List[str]:
        ...
        for cmd, pattern in [
            (['pdftoppm', '-png', '-r', '200', pdf_path, os.path.join(output_dir, 'page')], 'page-*.png'),
            (['convert', '-density', '200', pdf_path, os.path.join(output_dir, 'page_%03d.png')], 'page_*.png'),
        ]:
            try:
    ...
    def _convert_pdf_first_page(pdf_path: str, save_path: str) -> Dict[str, Any]:
        ...
        if images:
            ...
            return {"status": "success", "path": save_path, "file_type": "pdf_page",
                    "total_pages": len(images) + 1}
        return {"status": "error", "error": "PDF conversion failed. Install poppler or ImageMagick."}
...
def visual_inspect_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = _get_vlm().analyze_image(image_path, "\n".join(prompt_parts), detail="high", max_tokens=2500)
    if result["status"] == "success":
        return tool_response(visual_state=result["analysis"], model=result.get("model", ""),
...
def inspect_file_visually_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    question = params.get('question', 'Analyze this file content, formatting, and identify any issues.')
    task_context = params.get('task_context', '')
    ...
    if file_type == "text_with_line_numbers":
        ...
        if result["status"] == "success":
            return tool_response(visual_state=result["analysis"], file_path=file_path,
                                 file_type=file_type, model=result.get("model", ""))
        return tool_error(result.get("error", "VLM analysis failed"))
...
def inspect_pptx_slides_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=min(len(slide_images), 5)) as executor:
        futures = [
            executor.submit(_get_vlm().analyze_image, img,
                            slide_prompt.replace("{num}", str(i + 1)), "high", 1500)
    ...
    summary_parts = [f"Analyzed {len(successful)}/{len(slide_images)} slides from {Path(pptx_path).name}:"]
    for a in successful:
        summary_parts.append(f"\n--- Slide {a['slide']} ---\n{a['analysis'][:500]}")

    return tool_response(summary="\n".join(summary_parts), slide_analyses=analyses,
                         total_slides=len(slide_images), pptx_path=pptx_path)
...
def inspect_pdf_pages_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    question = params.get('question', 'Analyze the content, layout, and visual quality of this page.')
    ...
    with ThreadPoolExecutor(max_workers=min(len(page_images), 5)) as executor:
        futures = [
            executor.submit(_get_vlm().analyze_image, img,
                            page_prompt.replace("{num}", str(i + 1)), "high", 1500)
    ...
    successful = [a for a in analyses if a["status"] == "success"]
    summary_parts = [f"Analyzed {len(successful)}/{len(page_images)} pages from {Path(pdf_path).name}:"]
    for a in successful:
        summary_parts.append(f"\n--- Page {a['page']} ---\n{a['analysis'][:500]}")

    return tool_response(summary="\n".join(summary_parts), page_analyses=analyses,
                         total_pages=len(page_images), pdf_path=pdf_path)
...
def inspect_code_for_errors_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if result["status"] == "success":
        return tool_response(issues=result["analysis"], file_path=file_path,
                             lines_analyzed=min(len(lines), 300), total_lines=len(lines),
...
def inspect_browser_state_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    inspect_result = visual_inspect_tool({
        'image_path': save_path, 'question': question,
        'task_context': task_context, 'goal_context': goal_context})
