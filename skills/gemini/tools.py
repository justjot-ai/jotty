...
def generate_text_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not 0.0 <= temperature <= 1.0:
            return {"success": False, "error": "temperature must be between 0.0 and 1.0"}
        ...
        if not text:
            return {"success": False, "error": "No text generated", "raw_response": response}
...
def generate_with_image_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "text": text, "model": model, "image_path": image_path}

    except ValueError as e:
        return {"success": False, "error": str(e)}
    except Exception as e:
        logger.error(f"Gemini vision generation error: {e}", exc_info=True)
        return {"success": False, "error": f"Vision generation failed: {str(e)}"}
...
def chat_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not messages:
            return {"success": False, "error": "messages parameter is required"}

        if not isinstance(messages, list):
            return {"success": False, "error": "messages must be a list of message dicts"}

        # Validate message format
        for i, msg in enumerate(messages):
            if not isinstance(msg, dict):
                return {"success": False, "error": f"Message at index {i} must be a dictionary"}
        ...
        if not text:
            return {"success": False, "error": "No response generated", "raw_response": response}

        return {"success": True, "text": text, "model": model, "message_count": len(messages)}
...
def list_models_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"success": True, "models": models, "default_model": DEFAULT_MODEL}
