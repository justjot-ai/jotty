...
async def notion_knowledge_pipeline_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if workflow_type in ["capture", "full"] and params.get(
            "capture_knowledge", workflow_type == "full"
        ):
                ...
                if capture_tool:
                    ...
                    if not knowledge_content:
                        ...
                        if capture_result.get("success"):
                            ...
                            logger.warning(
                                f"⚠️  Knowledge capture failed: {capture_result.get('error')}"
                            )

        # Step 2: Research and document (Processor)
        if workflow_type in ["research", "full"] and params.get(
            "research_and_document", workflow_type == "full"
        ):
                ...
                if research_tool:
                    ...
                    if not research_topic:
                        ...
                        if research_result.get("success"):
                            ...
                            logger.warning(
                                f"⚠️  Research documentation failed: {research_result.get('error')}"
                            )

        # Step 3: Create implementation plan (Sink)
        if workflow_type in ["implement", "full"] and params.get(
            "create_implementation_plan", workflow_type == "full"
        ):
                ...
                if implement_tool:
                    ...
                    if not spec_page_id:
                        ...
                        if implement_result.get("success"):
                            ...
                            logger.warning(
                                f"⚠️  Implementation plan creation failed: {implement_result.get('error')}"
                            )
