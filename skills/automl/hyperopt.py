...
class HyperoptSkill(MLSkill):
    ...
    async def execute(
        ...
        optimized_model = self._build_optimized_model(best_model_name, best_params, problem_type)
    ...
    def _get_objective(
        self, model_name: str, X_scaled: Any, y: Any, cv: Any, scoring: str, problem_type: str
    ) -> Any:
    ...
    def _build_optimized_model(self, model_name: str, params: Dict, problem_type: str) -> Any:
        ...
        if model_name in ["lightgbm", "lgb"]:
            ...
            try:
                ...
                return (
                    CatBoostClassifier(**final_params)
                    if problem_type == "classification"
                    else CatBoostRegressor(**final_params)
                )
            except ImportError:
            ...
            return (
                ...
                if problem_type == "classification"
                else HistGradientBoostingRegressor(**final_params)
            )

        elif model_name in ["extra_trees", "et"]:
            final_params = {**params, "random_state": 42, "n_jobs": -1}
            return (
                ExtraTreesClassifier(**final_params)
                if problem_type == "classification"
                else ExtraTreesRegressor(**final_params)
            )
            ...
            return (
                ...
                if problem_type == "classification"
                else GradientBoostingRegressor(**final_params)
            )
