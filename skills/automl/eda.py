...
class EDASkill(MLSkill):
        ...
        try:
            ...
            return self._create_result(
                ...
                metrics={
                    ...
                    "n_skewed": len(
                        [c for c, d in insights["distributions"].items() if d.get("is_skewed")]
                    ),
                    "n_high_corr_pairs": len(
                        insights["correlations"].get("high_correlation_pairs", [])
                    ),
                },
                metadata={
    ...
    def _analyze_distributions(self, X: pd.DataFrame) -> Dict:
        ...
        for col in numeric_cols:
            ...
            median = data.median()
            below_median_mean = (
                data[data < median].mean() if len(data[data < median]) > 0 else median
            )
            above_median_mean = (
                data[data >= median].mean() if len(data[data >= median]) > 0 else median
            )
    ...
    def _analyze_correlations(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        corr_info = {"with_target": {}, "feature_pairs": [], "high_correlation_pairs": []}
                ...
                for col2 in numeric_cols[i + 1 :]:
                    ...
                    if not np.isnan(corr) and abs(corr) > 0.5:
                        corr_info["high_correlation_pairs"].append(
                            {"feature1": col1, "feature2": col2, "correlation": float(corr)}
                        )
        ...
        for col in cat_cols:
            ...
            if y is not None:
                try:
                    ...
                    cat_info[col]["target_rate_range"] = float(
                        target_rates.max() - target_rates.min()
                    )
    ...
    def _analyze_interactions(self, X: pd.DataFrame, y: pd.Series) -> Dict:
        ...
        base_corrs = {
            col: abs(X[col].corr(y)) for col in numeric_cols if not np.isnan(X[col].corr(y))
        }
    ...
    def _generate_recommendations(
        self, insights: Dict, X: pd.DataFrame, y: pd.Series
    ) -> List[Dict]:
    ...
    def _format_for_llm(self, insights: Dict) -> str:
        ...
        bimodal = [
            col for col, d in insights["distributions"].items() if d.get("potential_bimodal")
        ]
        if bimodal:
        ...
        strong_corr = [
            col
            for col, c in insights["correlations"].get("with_target", {}).items()
            if c.get("is_strong")
        ]
        if strong_corr:
        ...
        predictive_cats = [
            col for col, c in insights["categorical"].items() if c.get("is_predictive")
        ]
        if predictive_cats:
        ...
        if interactions:
            int_str = [
                (
                    f"{i['features'][0]}*{i['features'][1]}"
                    if i["type"] == "product"
                    else f"{i['features'][0]}/{i['features'][1]}"
                )
        ...
        for rec in insights["recommendations"][:10]:
            lines.append(
                f"- {rec['action']} on {rec.get('feature', rec.get('features'))}: {rec['reason']}"
            )
    ...
    def analyze(
        self, X: pd.DataFrame, y: pd.Series = None, problem_type: str = "classification"
    ) -> Dict:
        """
