...
class PresentonClient:
    ...
    def _make_request(self, method: str, endpoint: str, json_data: Optional[Dict] = None, timeout: int = 300) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': f'Cannot connect to Presenton at {url}. Is the container running?'}
    ...
    def generate_presentation(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self._make_request('POST', '/api/v1/ppt/presentation/generate', json_data=params, timeout=params.get('timeout', 300))
...
def start_presenton_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if run_cmd.returncode != 0:
            return {
                'success': False,
                'error': f'Failed to start container: {run_cmd.stderr}'
            }
...
def stop_presenton_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if stop_cmd.returncode != 0:
            return {
                'success': False,
                'error': f'Failed to stop container: {stop_cmd.stderr}'
            }
...
def generate_presentation_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'download_url': f"{base_url}/api/v1/ppt/presentation/{presentation_id}/download" if presentation_id else '',
        'content': content,
...
async def generate_presentation_from_research_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not web_search_skill:
            ...
            return generate_presentation_tool({
                **params,
                'content': topic,
                'web_search': True
            })
        ...
        search_result = search_tool({
            'query': f"{topic} latest trends statistics facts",
        ...
        if presentation_result.get('success'):
            presentation_result['research_summary'] = f"Researched {len(search_result.get('results', []))} sources"
...
def download_presentation_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not presentation_id:
        return {
            'success': False,
            'error': 'presentation_id parameter is required'
        }
