...
def get_stream_chunk_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return tool_response(chunk=base64.b64encode(chunk).decode("utf-8"), done=False)
...
def voice_to_text(audio_path: str, provider: str = "auto", language: str = None) -> str:
    ...
    result = voice_to_text_tool(
        {"audio_path": audio_path, "provider": provider, "language": language}
    )
...
def text_to_voice(text: str, provider: str = "auto", voice_id: str = None) -> bytes:
    ...
    result = text_to_voice_tool({"text": text, "provider": provider, "voice_id": voice_id})
    if not result.get("success"):
        raise RuntimeError(result.get("error", "Unknown error"))
...
OPENAI_VOICE_DESC = {
    ...
    "nova": "Friendly, expressive (recommended)",
...
class OpenAIAudioClient:
    ...
    def translate(self, audio_path: str, prompt: str = None, response_format: str = "json") -> dict:
    ...
    def generate_speech(
        ...
        if voice not in OPENAI_TTS_VOICES:
            return {
                "success": False,
                "error": f"Invalid voice: {voice}. Available: {OPENAI_TTS_VOICES}",
            }
...
def translate_audio_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return OpenAIAudioClient.get_instance().translate(params["audio_path"], params.get("prompt"))
