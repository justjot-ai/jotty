...
async def product_launch_pipeline_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not product_description:
            return {"success": False, "error": "product_description is required"}
                ...
                if ads_skill:
                    ...
                    if ads_tool:
                        ...
                        for competitor_name in competitor_names:
                            ads_params = {"competitor_name": competitor_name, "max_ads": max_ads}
                            ...
                            if ads_result.get("success"):
                                ...
                                logger.info(
                                    f"✅ Analyzed {competitor_name}: {len(ads_result.get('ads', []))} ads"
                                )
                            else:
                                logger.warning(
                                    f"⚠️  Competitor analysis failed for {competitor_name}: {ads_result.get('error')}"
                                )
            else:
                ...
                if content_tool:
                    ...
                    if competitor_analysis:
                        ...
                        for competitor, data in competitor_analysis.items():
                            research_context += (
                                f"- {competitor}: {len(data.get('ads', []))} ads analyzed\n"
                            )
                        research_context += "\n"
