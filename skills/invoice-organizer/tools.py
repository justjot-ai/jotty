...
async def organize_invoices_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not invoice_dir:
        return {"success": False, "error": "invoice_directory is required"}

    invoice_path = Path(os.path.expanduser(invoice_dir))
    if not invoice_path.exists():
        return {"success": False, "error": f"Directory does not exist: {invoice_dir}"}
                ...
                if invoice_file.suffix.lower() == ".pdf":
                    try:
                        ...
                        with open(invoice_file, "rb") as f:
                            ...
                            content = "\n".join(
                                [page.extract_text() for page in pdf_reader.pages[:3]]
                            )[:2000]
...
async def _organize_invoices(
    invoices: List[Dict], strategy: str, output_path: Path, rename_format: str
) -> List[Dict]:
