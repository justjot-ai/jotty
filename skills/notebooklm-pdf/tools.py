    ...
    try:
        ...
        if content_file:
            ...
            if not content_path.exists():
                return {"success": False, "error": f"Content file not found: {content_file}"}
...
async def _try_browser_automation(
    content: str, title: str, output_file: Path, params: Dict[str, Any]
) -> Dict[str, Any]:
    """
            ...
            async with async_playwright() as p:
                ...
                if needs_sign_in:
                    if headless_mode:
                        ...
                        logger.warning(
                            "NotebookLM requires sign-in, but browser is in headless mode."
                        )
                        ...
                        logger.info(
                            "Please sign in manually in the browser window, then press Enter to continue..."
                        )
                            ...
                            try:
                                ...
                                if downloads_dir.exists():
                                    ...
                                    if pdf_files:
                                        ...
                                        if output_file.exists():
                                            logger.info(
                                                f"✅ PDF copied from Downloads: {output_file}"
                                            )
                                            pdf_exported = True
                                            break
                    except Exception as e:
...
async def _fallback_pandoc(
    ...
    try:
        ...
        try:
            ...
            if doc_converter_path.exists():
                spec = importlib.util.spec_from_file_location(
                    "document_converter_tools", doc_converter_path
                )
        ...
        return {"success": False, "error": f"Pandoc fallback failed: {str(e)}"}


async def _direct_pandoc_call(
    input_path: Path, output_path: Path, title: str, params: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if output_path.exists():
            ...
            return {"success": False, "error": "Pandoc completed but output file not found"}

    except Exception as e:
        logger.error(f"Direct Pandoc call failed: {e}", exc_info=True)
        return {"success": False, "error": f"Direct Pandoc call failed: {str(e)}"}
