...
Return ONLY the JSON array, no other text:"""
        ...
        if inspect.iscoroutinefunction(generate_tool):
            gap_result = await generate_tool(
                {"prompt": gap_analysis_prompt, "model": "sonnet", "timeout": 300}
            )
        else:
            gap_result = generate_tool(
                {"prompt": gap_analysis_prompt, "model": "sonnet", "timeout": 300}
            )
        ...
        if followup_queries:
            logger.info(
                f"🎯 Stage 3: Performing targeted follow-up research ({len(followup_queries)} queries)..."
            )
            ...
            logger.info(
                f"📊 Stage 3 complete: {total_after_followup} total results (added {total_after_followup - total_initial_results})"
            )
        ...
        try:
                        ...
                        if charts:
                            ...
                            if anomalies:
                                ...
                                for anomaly in anomalies[:5]:  # Top 5
                                    severity_icon = (
                                        "🔴" if anomaly.get("severity") == "high" else "🟡"
                                    )
                            ...
                            if forecasts.get("revenue"):
                                ...
                                chart_markdown += (
                                    f"{forecast.get('forecast_growth_rate', 0):.1f}% growth rate "
                                )
            ...
            logger.warning(
                f"Visualization generation failed (continuing without charts/tables): {e}"
            )
            import traceback
...
**START WRITING THE COMPLETE REPORT NOW. BEGIN WITH THE HEADER AND EXECUTIVE SUMMARY, THEN CONTINUE WITH ALL SECTIONS IN FULL DETAIL. DO NOT STOP UNTIL YOU HAVE WRITTEN ALL {target_pages} PAGES OF CONTENT:**"""
        ...
        logger.info(
            f"📝 Prompt size: ~{len(claude_prompt)//1024}KB, Research data: {len(research_content)//1024}KB"
        )
        ...
        try:
            if inspect.iscoroutinefunction(generate_tool):
                claude_result = await generate_tool(
                    {"prompt": claude_prompt, "model": "sonnet", "timeout": timeout_seconds}
                )
            else:
        ...
        if not claude_result.get("success"):
            return {
                ...
                "research_summary": {
                    k: {"success": v.get("success"), "count": v.get("count", 0)}
        ...
        if word_count < target_pages * 400:  # Less than 400 words per page
            logger.warning(
                f"⚠️  Report may be too short ({word_count} words, target: {target_pages * 500}+)"
            )
...
"""
        ...
        write_result = write_tool({"path": str(md_path), "content": full_markdown})
            ...
            if send_tool:
                ...
                if inspect.iscoroutinefunction(send_tool):
                    telegram_result = await send_tool(
                        {"file_path": pdf_output_path, "caption": caption}
                    )
                else:
