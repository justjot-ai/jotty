...
async def create_implementation_plan_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not spec_page_id:
        return {"success": False, "error": "spec_page_id is required"}
    ...
    try:
        ...
        fetch_tool = notion_skill.tools.get("fetch_tool") or notion_skill.tools.get(
            "fetch_page_tool"
        )

        if not fetch_tool:
            return {"success": False, "error": "Notion fetch tool not found"}
        ...
        create_params = {"title": plan_title, "content": plan_content}
        ...
        if output_database_id and tasks:
            add_tool = notion_skill.tools.get("add_to_database_tool") or notion_skill.tools.get(
    ...
    {{"name": "Milestone name", "date": "YYYY-MM-DD", "description": "..."}}
                ...
                if inspect.iscoroutinefunction(generate_tool):
                    result = await generate_tool(
                        {"prompt": prompt, "model": "sonnet", "timeout": 180}
                    )
                else:
        ...
        [
            {
                ...
                "description": "Specification reviewed and plan finalized",
            },
            {
