    ...
    def query(
        ...
        try:
            ...
            if self.semantic_layer.is_mongodb:
                ...
                if self.use_connectorx and hasattr(
                    self.semantic_layer.query_engine, "query_to_dataframe"
                ):
            ...
            return DataSourceResult(success=False, query=question, error=error or "Query failed")
            ...
            "tables": [
                {
                    "name": t.name,
                    "columns": [
                        {"name": c.name, "type": c.normalized_type.value} for c in t.columns
                    ],
                }
    ...
    def query(self, question: str, collection: str = None, **kwargs: Any) -> DataSourceResult:
        ...
        try:
            ...
            if self.semantic_layer:
                ...
                return DataSourceResult(success=False, query=question, error=result.get("error"))
            ...
            if not collection:
                return DataSourceResult(
                    success=False, error="Collection name required for direct pipeline execution"
                )
