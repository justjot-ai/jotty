...
async def _process_single_topic(
    topic: str, topic_index: int, registry, params: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if inspect.iscoroutinefunction(search_tool):
            search_result = await search_tool({"query": topic, "max_results": details_per_topic})
        else:
            search_result = search_tool({"query": topic, "max_results": details_per_topic})
        ...
        if not details:
            return {"success": False, "topic": topic, "error": "No search results found"}
...
Format the synthesis with clear sections and bullet points where appropriate."""
        ...
        if inspect.iscoroutinefunction(summarize_tool):
            synthesis_result = await summarize_tool(
                {"content": details_content, "prompt": synthesize_prompt, "model": model}
            )
        else:
        ...
        if not synthesized_content:
            return {"success": False, "topic": topic, "error": "No synthesis generated"}
        ...
        if params.get("create_ideas", True):
            ...
            if use_mcp_client:
                try:
                        ...
                        async with MCPClient(server_path=server_path) as client:
                            mcp_result = await client.call_tool(
                                "create_idea",
                                {
                                    ...
                                    "tags": params.get(
                                        "tags", ["trending", topic.lower().replace(" ", "-")]
                                    ),
                                    "sections": [
                                        {
                                            "title": "Synthesis",
                                        ...
                                        {
                                            "title": "Sources",
                                            "content": _format_topic_details_for_synthesis(
                                                details, topic
                                            ),
                                            "type": "text",
                                        },
                    ...
                    logger.warning(
                        f"MCP client failed for {topic}: {e}, falling back to HTTP API..."
                    )
                    use_mcp_client = False
            ...
            if idea_result and idea_result.get("success"):
                ...
                logger.warning(
                    f"   ⚠️  Idea creation failed for {topic}: {idea_result.get('error') if idea_result else 'Unknown error'}"
                )
        ...
        return {"success": False, "topic": topic, "error": f"Processing failed: {str(e)}"}
...
async def trending_topics_to_ideas_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if source == "reddit":
            ...
            search_query = (
                f"{query} site:reddit.com" if query != "trending topics" else "trending reddit"
            )

            if inspect.iscoroutinefunction(search_tool):
                search_result = await search_tool(
                    {"query": search_query, "max_results": max_topics * 2}  # Get more to filter
                )
            else:
                search_result = search_tool({"query": search_query, "max_results": max_topics * 2})
            ...
            if v2v_skill:
                ...
                if search_tool:
                    ...
                    if inspect.iscoroutinefunction(search_tool):
                        v2v_result = await search_tool({"query": query, "max_results": max_topics})
                    else:
                        v2v_result = search_tool({"query": query, "max_results": max_topics})

                    if v2v_result.get("success"):
                        results = v2v_result.get("results", [])
                        topics = [
                            r.get("title", "") for r in results[:max_topics] if r.get("title")
                        ]
            ...
            if inspect.iscoroutinefunction(search_tool):
                search_result = await search_tool(
                    {"query": f"{query} trending 2026", "max_results": max_topics}
                )
            else:
        ...
        if not topics:
            return {"success": False, "error": f"No trending topics found from source: {source}"}
        ...
        tasks = [
            _process_single_topic(topic, i, registry, params) for i, topic in enumerate(topics)
        ]
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                processed_results.append(
                    {"success": False, "topic": topics[i], "error": str(result)}
                )
            else:
