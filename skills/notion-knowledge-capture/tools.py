...
async def capture_knowledge_to_notion_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'success': False,
            'error': result.get('error', 'Failed to create Notion page')
        }
...
def _generate_title(content: str, content_type: str) -> str:
    ...
    return title or f"{content_type.replace('_', ' ').title()} - {datetime.now().strftime('%Y-%m-%d')}"
