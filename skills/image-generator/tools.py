...
def _save_image_to_base64(image) -> str:
...
def _load_pipeline(model_id: str):
    """Load the appropriate diffusion pipeline for a model ID.

    Returns (pipeline, device) or raises ImportError/RuntimeError.
    """
    import torch
...
def _default_dimensions(model_id: str):
    """Return (width, height) defaults based on model type."""
    if any(k in model_id.lower() for k in ("xl", "sdxl", "flux")):
        return 1024, 1024
    return 512, 512
...
def generate_image_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": "torch is not installed. Run: pip install torch"}
    ...
    try:
        ...
        default_w, default_h = _default_dimensions(model_id)
        width = params.get("width", default_w)
        height = params.get("height", default_h)

        pipeline, device = _load_pipeline(model_id)
        ...
        return {"success": False, "error": f"Missing dependency: {e}. Run: pip install diffusers transformers"}
    ...
    try:
        ...
        for idx, prompt in enumerate(prompts):
            ...
            if output_dir:
                ...
                filepath = os.path.join(output_dir, f"image_{idx:03d}.png")
                image.save(filepath)
        ...
        return {"success": True, "model": model_id, "count": len(results), "images": results}
