...
async def meeting_intelligence_pipeline_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis_types = params.get(
            "analysis_types", ["speaking_ratios", "action_items", "decisions"]
        )
        insights_params = {
        ...
        if not insights_result.get("success"):
            return {
                "success": False,
                "error": f"Insights analysis failed: {insights_result.get('error')}",
            }
        ...
        logger.info(
            f"✅ Analysis complete: {len(action_items)} action items, {len(decisions)} decisions"
        )
        ...
        if not meeting_result.get("success"):
            return {
                "success": False,
                "error": f"Meeting preparation failed: {meeting_result.get('error')}",
            }
        ...
        if params.get("send_comm", True):
            ...
            if comm_skill:
                ...
                if comm_tool:
                    ...
                    progress = (
                        [f"Completed: {item}" for item in action_items[:3]]
                        if action_items
                        else ["Meeting completed"]
                    )
                    plans = (
                        [f"Next: {item}" for item in action_items[3:6]]
                        if len(action_items) > 3
                        else ["Follow up on decisions"]
                    )
                    problems = decisions if decisions else ["No blockers"]

                    comm_params = {
                        "comm_type": comm_type,
                        "content": {"progress": progress, "plans": plans, "problems": problems},
                    }
                    ...
                    if comm_result.get("success"):
                        ...
                        logger.warning(
                            f"⚠️  Communication generation failed: {comm_result.get('error')}"
                        )
