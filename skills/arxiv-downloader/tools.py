...
def _request_with_retry(
    url: str, params: Optional[Dict] = None, max_retries: int = 4, timeout: int = 30
...
def download_arxiv_paper_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not arxiv_input:
            return {"success": False, "error": "arxiv_id parameter is required"}

        # Extract arXiv ID from various formats
        arxiv_id = _extract_arxiv_id(arxiv_input)
        if not arxiv_id:
            return {"success": False, "error": f"Invalid arXiv ID or URL: {arxiv_input}"}
        ...
        if not metadata:
            return {"success": False, "error": f"Failed to fetch metadata for arXiv:{arxiv_id}"}
        ...
        if extract_mode == "text":
            ...
            if not content:
                return {"success": False, "error": "Failed to download LaTeX source"}
        ...
        return {"success": False, "error": f"Error downloading arXiv paper: {str(e)}"}
...
def search_arxiv_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": f"Error searching arXiv: {str(e)}"}
