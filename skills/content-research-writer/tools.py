    ...
    try:
        ...
        if action == 'outline':
            result = await _create_outline(
                generate_tool, topic, content_type, target_audience, 
                target_length, writing_style
            )
        
        elif action == 'research':
            result = await _conduct_research(
                generate_tool, topic, research_topics
            )
        
        elif action == 'improve_hook':
            result = await _improve_hook(
                generate_tool, topic, draft_content, writing_style
            )
        
        elif action == 'review_section':
            result = await _review_section(
                generate_tool, topic, section_to_review or draft_content, 
                writing_style
            )
...
4. Expert quotes if available"""
        ...
        if result.get('success'):
            return {
                'research': {
                    'findings': result.get('text', ''),
                    'sources': research_findings
                },
                'citations': citations
            }
    ...
    return {
        'research': {'findings': 'Research pending'},
        'citations': []
    }


async def _improve_hook(
    generate_tool,
    topic: str,
    draft_content: str,
    writing_style: str
