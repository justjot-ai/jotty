...
async def create_design_artwork_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not design_brief:
        return {"success": False, "error": "design_brief is required"}
    ...
    try:
        ...
        if output_format == "png":
            artwork_path = await _create_png_artwork(
                design_brief, design_philosophy, dimensions, style
            )
        else:
            artwork_path = await _create_pdf_artwork(
                design_brief, design_philosophy, dimensions, style
            )
...
Output only the philosophy, no explanations."""
                ...
                if inspect.iscoroutinefunction(generate_tool):
                    result = await generate_tool(
                        {"prompt": prompt, "model": "sonnet", "timeout": 120}
                    )
                else:
...
async def _create_png_artwork(
    brief: str, philosophy: str, dimensions: Tuple[int, int], style: str
) -> str:
    """Create PNG artwork."""
...
def _draw_organic(draw: ImageDraw.Draw, width: int, height: int, brief: str):
    ...
    for i in range(3):
        ...
        bbox = [offset_x - radius, offset_y - radius, offset_x + radius, offset_y + radius]
