...
async def analyze_meeting_insights_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not transcript_files:
        return {
            'success': False,
            'error': 'transcript_files is required'
        }
    ...
    if not transcripts:
        return {
            'success': False,
            'error': 'No valid transcript files found'
        }
...
Return JSON format with specific examples."""
    ...
    return {
        'analysis': 'Analysis pending - requires AI processing',
        'pattern': 'Leadership style'
    }
...
def _generate_report(insights: Dict, statistics: Dict, recommendations: List[str], transcripts: List[Dict]) -> str:
