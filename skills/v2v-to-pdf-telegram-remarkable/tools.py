...
async def v2v_to_pdf_and_send_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not v2v_skill:
            return {"success": False, "error": "v2v-trending-search skill not available"}

        search_tool = v2v_skill.tools.get("search_v2v_trending_tool")
        if inspect.iscoroutinefunction(search_tool):
            search_result = await search_tool({"query": query, "format": "markdown"})
        else:
            search_result = search_tool({"query": query, "format": "markdown"})

        if not search_result.get("success"):
            return {"success": False, "error": f"V2V search failed: {search_result.get('error')}"}
                ...
                if remarkable_skill:
                    ...
                    if send_remarkable_tool:
                        ...
                        if remarkable_sent:
                            ...
                            logger.warning(
                                f"⚠️  reMarkable send failed: {remarkable_result.get('error')}"
                            )
                    else:
