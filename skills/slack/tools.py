...
def send_message_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if result.get("success"):
        return tool_response(
            channel=result.get("channel"), ts=result.get("ts"), message=result.get("message")
        )
...
def read_messages_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = {"channel": params["channel"], "limit": min(params.get("limit", 10), 100)}
...
def add_reaction_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = {"channel": params["channel"], "timestamp": params["timestamp"], "name": emoji_name}
    ...
    if result.get("success"):
        return tool_response(
            channel=params["channel"], timestamp=params["timestamp"], reaction=emoji_name
        )
...
def get_user_info_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = {"user": params["user"], "include_locale": params.get("include_locale", False)}
