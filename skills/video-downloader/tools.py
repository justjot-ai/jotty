...
async def _download_with_ytdlp(
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if download_thumbnail:
            thumbnail_ext = (
                info.get("thumbnail", "").split(".")[-1] if info.get("thumbnail") else "jpg"
            )
        ...
        return {
            ...
            "thumbnail_path": (
                str(thumbnail_file) if thumbnail_file and thumbnail_file.exists() else None
            ),
            "metadata": metadata,
            "file_size": file_size,
        }
...
async def _download_with_youtubedl(
    ...
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        ...
        return {
            "success": True,
            "video_path": str(video_file),
            "metadata": {"title": info.get("title", ""), "duration": info.get("duration", 0)},
            "file_size": video_file.stat().st_size if video_file.exists() else 0,
        }
