...
async def brainstorm_domains_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"success": True, "suggestions": suggestions, "recommendations": recommendations}


async def _generate_domain_suggestions(
    project_description: str, keywords: List[str], preferred_tlds: List[str], max_suggestions: int
) -> List[Dict]:
...
def _generate_recommendations(suggestions: List[Dict]) -> Dict:
    ...
    sorted_suggestions = sorted(
        suggestions, key=lambda x: x.get("brandability_score", 0), reverse=True
    )
