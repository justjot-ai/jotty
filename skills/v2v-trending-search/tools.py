...
async def search_v2v_trending_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        search_queries = [
            ...
            f"v2v.ai {query}",
            f"{query} v2v",
            "site:v2v.ai trending" if query == "trending topics" else f"site:v2v.ai {query}",
        ]
        ...
        for attempt_query in search_queries:
            if inspect.iscoroutinefunction(search_tool):
                search_result = await search_tool(
                    {"query": attempt_query, "max_results": max_results}
                )
            else:
                search_result = search_tool({"query": attempt_query, "max_results": max_results})
        ...
        if not results:
            ...
            if inspect.iscoroutinefunction(search_tool):
                search_result = await search_tool(
                    {"query": f"v2v.ai {query}", "max_results": max_results}
                )
            else:
        ...
        if not results:
            return {
                "success": False,
                "error": "No results found for V2V.ai search. V2V.ai may require authentication or the site structure may have changed.",
        ...
        # Format as markdown
        markdown_content = "# V2V.ai Trending Search Results\n\n"
        ...
        return {"success": False, "error": f"V2V search failed: {str(e)}"}
