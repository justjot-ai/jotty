...
async def test_webapp_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not app_url:
        return {"success": False, "error": "app_url is required"}
    ...
    try:
        async with async_playwright() as p:
            ...
            test_results = {"url": app_url, "title": await page.title(), "actions_performed": []}
                ...
                for action in actions:
                    ...
                    try:
                        if action_type == "click":
                            ...
                            test_results["actions_performed"].append(
                                f"Filled {selector} with: {value}"
                            )
                        ...
                        test_results["actions_performed"].append(
                            f"Failed: {action_type} on {selector}"
                        )
