...
def upload_to_remarkable_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not pdf_path:
            return {"success": False, "error": "pdf_path parameter is required"}
        ...
        return {"success": True, "document_id": doc.ID, "path": folder, "filename": pdf_file.name}
    except Exception as e:
        return {"success": False, "error": f"Error uploading to reMarkable: {str(e)}"}
...
def register_remarkable_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if len(one_time_code) != 8:
            return {"success": False, "error": "one_time_code must be 8 characters"}
        ...
        body = {"code": one_time_code, "deviceDesc": "desktop-windows", "deviceID": device_id}
        ...
        if response.status_code == 200:
            ...
            with open(config_file, "w") as f:
                json.dump({"device_token": device_token, "device_id": device_id}, f, indent=2)
            ...
            return {"success": False, "error": f"Registration failed: HTTP {response.status_code}"}
    except Exception as e:
        return {"success": False, "error": f"Error registering device: {str(e)}"}
...
def check_remarkable_status_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": f"Error checking status: {str(e)}"}
