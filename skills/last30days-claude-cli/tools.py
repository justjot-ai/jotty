...
async def last30days_claude_cli(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        queries.append(
            {"query": f"{topic} reddit", "type": "reddit", "max_results": max_results_per_query}
        )

        # X/Twitter searches
        queries.append(
            {"query": f"{topic} twitter", "type": "x", "max_results": max_results_per_query}
        )
        queries.append(
            {"query": f"{topic} x.com", "type": "x", "max_results": max_results_per_query}
        )

        # Web searches
        queries.append(
            {"query": f"{topic} 2026", "type": "web", "max_results": max_results_per_query}
        )
        queries.append(
            {"query": f"{topic} latest", "type": "web", "max_results": max_results_per_query}
        )
        ...
        if not quick:
            ...
            queries.append(
                {"query": f"{topic} news", "type": "web", "max_results": max_results_per_query}
            )
        ...
        for i, result in enumerate(results):
            ...
            for item in search_results:
                ...
                elif query_type == "reddit" and (
                    "reddit" in title.lower() or "reddit" in snippet.lower()
                ):
                ...
                elif query_type == "x" and (
                    "twitter" in title.lower()
                    or "x.com" in title.lower()
                    or "twitter" in snippet.lower()
                ):
        ...
        if emit == "json":
            return {"success": True, "output": research_data, "format": "json"}
        elif emit == "md":
            markdown = _format_as_markdown(topic, date_range, research_data, tool)
            return {"success": True, "output": markdown, "format": "md"}
        else:  # compact
            compact = _format_as_compact(topic, date_range, research_data, tool)
            return {"success": True, "output": compact, "format": "compact"}
