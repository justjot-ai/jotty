...
async def send_to_remarkable_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            if folder and folder != "/":
                ...
                mkdir_result = subprocess.run(
                    [rmapi_path, "mkdir", folder], capture_output=True, text=True, timeout=30
                )
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=120  # Longer timeout for large files
            )
            ...
            if result.returncode == 0:
                ...
                return {"success": False, "error": f"rmapi upload failed: {error_msg[:500]}"}
        ...
        return {"success": False, "error": "Upload timeout. File may still be uploading."}
    except Exception as e:
        logger.error(f"reMarkable upload error: {e}", exc_info=True)
        return {"success": False, "error": f"Failed to upload to reMarkable: {str(e)}"}
