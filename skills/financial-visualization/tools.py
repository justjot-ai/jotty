...
async def extract_financial_data_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    data_types = params.get(
        "data_types",
        ["financial_statements", "valuation_metrics", "key_ratios", "price_data", "growth_metrics"],
    )
    use_llm = params.get("use_llm", True)

    if not research_content:
        return {"success": False, "error": "research_content is required"}
...
If data is not available, use null. Return ONLY the JSON, no other text."""
                ...
                if inspect.iscoroutinefunction(generate_tool):
                    llm_result = await generate_tool(
                        {"prompt": extraction_prompt, "model": "sonnet", "timeout": 120}
                    )
                else:
...
async def generate_intelligent_charts_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "IntelligentVisualizationOrchestrator not available, using standard charts"
            )
    ...
    if not ticker or not company_name:
        return {"success": False, "error": "ticker and company_name are required"}
    ...
    extract_result = await extract_financial_data_tool(
        {"research_content": research_content, "use_llm": True}
    )
    ...
    if not requested_charts and enable_intelligence:
        chart_types = await orchestrator.intelligently_select_charts(
...
async def generate_financial_charts_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    company_name = params.get("company_name", "")
    research_data = params.get("research_data", {})
    chart_types = params.get(
        "chart_types", ["revenue_growth", "profitability", "valuation_metrics", "financial_health"]
    )
    output_dir = params.get("output_dir", os.path.expanduser("~/jotty/charts"))
    chart_format = params.get("format", "png")
...
async def _generate_chart_insights(
    chart_type: str, chart_data: Dict[str, Any], ticker: str, company_name: str
) -> str:
...
**Output:** Just the insight paragraph, no markdown formatting."""
        ...
        if inspect.iscoroutinefunction(generate_tool):
            result = await generate_tool(
                {"prompt": insights_prompt, "model": "sonnet", "timeout": 60}
            )
        else:
            result = generate_tool({"prompt": insights_prompt, "model": "sonnet", "timeout": 60})
...
async def _generate_chart(
    ...
    if chart_type == "revenue_growth":
                ...
                for i in range(1, len(revenues)):
                    if revenues[i - 1] > 0:
                        ...
                        ax.annotate(
                            ...
                            fontweight="bold",
                            color=(
                                FINANCIAL_COLORS["success"]
                                if growth > 0
                                else FINANCIAL_COLORS["danger"]
                            ),
                            arrowprops=dict(arrowstyle="->", lw=1, alpha=0.5),
                        )

            ax.set_xlabel(
                "Year", fontsize=CHART_STYLE["font_size"] + 1, fontweight="bold", color="#333"
            )
            ax.set_ylabel(
                "Revenue (₹ Crore)",
                fontsize=CHART_STYLE["font_size"] + 1,
                fontweight="bold",
                color="#333",
            )
            ax.set_title(
                f"{company_name} ({ticker}) - Revenue Growth Trend",
        ...
        if metrics:
            ...
            ax.set_title(
                f"{company_name} ({ticker}) - Profitability Analysis",
                fontsize=CHART_STYLE["title_size"],
                fontweight="bold",
                pad=20,
                color="#1a1a1a",
            ...
            for i, (bar, val, bench) in enumerate(zip(bars, values, benchmark_values)):
                ...
                color_indicator = (
                    FINANCIAL_COLORS["success"] if val >= bench else FINANCIAL_COLORS["warning"]
                )
                ax.text(
                    bar.get_x() + bar.get_width() / 2.0,
                    height + max(values) * 0.02,
                    f"{val:.1f}% {performance}",
                    ha="center",
                    va="bottom",
                    fontsize=10,
                    fontweight="bold",
        ...
        if metrics:
            ...
            ax.set_title(
                f"{company_name} ({ticker}) - Valuation Analysis",
                fontsize=CHART_STYLE["title_size"],
                fontweight="bold",
                pad=20,
                color="#1a1a1a",
        ...
        if not ratios and not valuation:
            ...
            if categories:
                ...
                ax.set_ylim(0, 100)
                ax.set_title(
                    f"{company_name} ({ticker}) - Multi-Dimensional Analysis",
                    fontsize=CHART_STYLE["title_size"],
                    fontweight="bold",
                    pad=20,
                )
        ...
        health_metrics = {
            ...
            "Leverage": (
                100 - min((ratios.get("debt_to_equity") or 0) * 20, 100)
        ...
        if any(v is not None and v > 0 for v in health_metrics.values()):
            ...
            colors_list = [
                (
                    FINANCIAL_COLORS["success"]
                    if v >= 70
                    else FINANCIAL_COLORS["warning"] if v >= 40 else FINANCIAL_COLORS["danger"]
                )
                for v in values
            ]

            bars = ax.barh(
                categories, values, color=colors_list, alpha=0.8, edgecolor="white", linewidth=2
            )
            ax.set_xlabel(
                "Health Score (0-100)", fontsize=CHART_STYLE["font_size"] + 1, fontweight="bold"
            )
            ax.set_title(
                f"{company_name} ({ticker}) - Financial Health Scorecard",
                fontsize=CHART_STYLE["title_size"],
                fontweight="bold",
                pad=20,
            )
        ...
        ax.text(
            0.5,
            0.5,
            f'Chart type "{chart_type}" not yet implemented',
            ha="center",
            va="center",
            transform=ax.transAxes,
            fontsize=12,
    ...
    if chart_data_for_insights:
        chart_insights = await _generate_chart_insights(
            chart_type, chart_data_for_insights, ticker, company_name
        )
        if chart_insights:
...
async def _generate_table(
    table_type: str, ticker: str, company_name: str, extracted_data: Dict[str, Any], format: str
) -> tuple:
    ...
    if table_type == "financial_statements":
        ...
        if format == "markdown":
            table_lines = [
                ...
                "| Metric | "
                + " | ".join(sorted(revenue_data.keys() if revenue_data else ["N/A"]))
                + " |",
                "|--------|"
            ...
            if revenue_data:
                ...
                revenue_row = (
                    "| Revenue (₹ Cr) | "
                    + " | ".join([f"{revenue_data.get(y, 'N/A')}" for y in years])
                    + " |"
                )
            ...
            if profit_data:
                ...
                profit_row = (
                    "| Net Profit (₹ Cr) | "
                    + " | ".join([f"{profit_data.get(y, 'N/A')}" for y in years])
                    + " |"
                )
