...
class IntelligentVisualizationOrchestrator:
    ...
    async def analyze_data_completeness(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        recommended_charts = []
        for chart_type, config in sorted(
            self.chart_priority_map.items(), key=lambda x: x[1]["priority"], reverse=True
        ):
                            ...
                            anomalies.append(
                                {
                                    ...
                                    "severity": (
                                        "high"
                                        if abs(gr - mean_growth) > 3 * std_growth
                                        else "medium"
                                    ),
    ...
    async def generate_trend_forecast(
        self, extracted_data: Dict[str, Any], metric: str = "revenue", periods: int = 2
    ) -> Dict[str, Any]:
        """
...
**Output:** Write ONLY the narrative paragraphs, no markdown formatting, no headers, just the analysis text."""
            ...
            if inspect.iscoroutinefunction(generate_tool):
                result = await generate_tool(
                    {"prompt": narrative_prompt, "model": "sonnet", "timeout": 90}
                )
            else:
    ...
    async def intelligently_select_charts(
        self, extracted_data: Dict[str, Any], analysis_result: Dict[str, Any]
    ) -> List[str]:
        """
    ...
    async def create_section_placements(
        self, charts: List[str], report_sections: List[str]
    ) -> Dict[str, List[str]]:
        """
