...
async def screener_analyze_pdf_telegram_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not symbols:
            return {"success": False, "error": "symbols parameter is required"}
        ...
        if not screener_skill:
            return {"success": False, "error": "screener-financials skill not available"}

        get_financials_tool = screener_skill.tools.get("get_company_financials_tool")
        if not get_financials_tool:
            return {"success": False, "error": "get_company_financials_tool not found"}
        ...
        if not companies_data:
            return {"success": False, "error": "Failed to fetch data for any company"}
        ...
        if not summarize_tool:
            return {"success": False, "error": "Claude LLM summarize tool not found"}
        ...
        markdown_content = _create_markdown_report(
            companies_analyzed, companies_data, analysis_text, analysis_type
        )
        ...
        pdf_title = (
            params.get("title")
            or f"Financial Analysis: {', '.join([c['name'] for c in companies_analyzed])}"
        )
        ...
        if not pdf_result.get("success"):
            return {"success": False, "error": f'PDF conversion failed: {pdf_result.get("error")}'}

        # Try multiple possible return keys
        pdf_path = (
            pdf_result.get("output_path")
            or pdf_result.get("output_file")  # document-converter returns 'output_path'
            ...
            if telegram_skill:
                ...
                if send_file_tool:
                    ...
                    if telegram_sent:
                        ...
                        logger.warning(
                            f"   ⚠️  Telegram send failed: {telegram_result.get('error')}"
                        )
                else:
