...
async def research_to_pdf_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not llm_skill:
            return tool_error("LLM skill not available. Ensure claude-cli-llm skill is installed.")
...
Format in Markdown with clear headings."""
        ...
        summary_result = await llm_tool({"prompt": analysis_prompt, "max_tokens": 4000})
...
"""
        ...
        pdf_result = await doc_tool(
            {"content": full_document, "format": "pdf", "title": f"Research Report - {topic}"}
        )
