...
async def create_slack_gif_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        frames = await _generate_frames(description, width, height, frame_count, animation_type)
        ...
        if not meets_requirements:
            logger.warning(f"GIF size ({file_size_kb:.1f}KB) exceeds limit ({max_size_kb}KB)")
...
async def _generate_frames(
    description: str, width: int, height: int, frame_count: int, animation_type: str
) -> list:
