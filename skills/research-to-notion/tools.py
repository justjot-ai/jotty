...
async def research_to_notion_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not research_type or not research_query:
            return {"success": False, "error": "research_type and research_query are required"}
        ...
        if research_type == "leads":
            ...
            lead_params = {
                ...
                **{
                    ...
                    if k in ["industry", "location", "company_size", "pain_points", "technologies"]
                },
            }
            ...
            if not research_result.get("success"):
                return {
                    "success": False,
                    "error": f"Lead research failed: {research_result.get('error')}",
                }
            ...
            if not research_result.get("success"):
                return {
                    "success": False,
                    "error": f"Competitive research failed: {research_result.get('error')}",
                }
            ...
            if inspect.iscoroutinefunction(search_tool):
                research_result = await search_tool({"query": research_query, "max_results": 10})
            else:
                research_result = search_tool({"query": research_query, "max_results": 10})

            if not research_result.get("success"):
                return {
                    "success": False,
                    "error": f"Topic research failed: {research_result.get('error')}",
                }
        ...
        if not content_result.get("success"):
            return {
                "success": False,
                "error": f"Content writing failed: {content_result.get('error')}",
            }
            ...
            if notion_skill:
                ...
                if doc_tool:
                    ...
                    if notion_result.get("success"):
                        ...
                        logger.warning(
                            f"⚠️  Notion documentation failed: {notion_result.get('error')}"
                        )
