...
async def generate_transformer_paper_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not claude_skill:
            return {"success": False, "error": "claude-cli-llm skill not found"}
        ...
        math_note = (
            "Include LaTeX equations (use \\frac, \\sum, \\mathbf, etc.) for key formulas."
            if include_math
            else ""
        )
        ...
        max_tokens_map = {"short": 4000, "medium": 8000, "long": 16000}
...
Write the actual paper content now. Do not describe what you will write. Write full paragraphs with detailed explanations."""
        ...
        paper_content = (
            claude_result.get("text")
            or claude_result.get("summary")
            or claude_result.get("content")
            or claude_result.get("result", "")
        )

        if not paper_content:
            return {"success": False, "error": "No content generated from Claude LLM"}

        logger.info(
            f"Generated paper content: {len(paper_content)} characters (~{len(paper_content)//5} words)"
        )
        ...
        if not document_converter_skill:
            return {"success": False, "error": "document-converter skill not available"}
        ...
        pdf_file = (
            ...
            or pdf_result.get("output_file")
            or pdf_result.get("pdf_path")
            or str(pdf_output_file)
        ...
        return {"success": False, "error": f"Failed to generate transformer paper: {str(e)}"}
...
def _parse_content_blocks_for_markdown(text: str, include_math: bool) -> list:
    ...
    for para in paragraphs:
        ...
        if (
            ...
            and any(
                cmd in para_clean
                for cmd in ["frac", "sum", "int", "sqrt", "mathbf", "text", "textcolor"]
            )
        ):
            ...
            if para_clean.count("\\") >= 2 and len(para_clean) < 500:
                ...
                blocks.append(
                    {"type": "equation", "latex": para_clean, "numbered": True, "boxed": False}
                )
            else:
                # Equation within text
                blocks.append({"type": "text", "content": para_clean})
        # Check for lists (bullet points or numbered)
        elif (
            para_clean.startswith("-")
            or para_clean.startswith("*")
            or (len(para_clean) > 2 and para_clean[0].isdigit() and para_clean[1:3] in [". ", ") "])
        ):
            # Extract list items
            items = []
        ...
        # Check for key concepts
            ...
            keyword in para_clean.lower()
            for keyword in ["key concept", "important note", "note:", "remember:", "key point"]
        ):
            title = "Key Concept"
            content = para_clean
            ...
            blocks.append({"type": "keybox", "title": title, "content": content})
        # Regular text paragraph
            ...
            if len(para_clean) > 80:  # Minimum paragraph length
                ...
                len(para_clean) > 30
                and not para_clean[0].isdigit()
                and not para_clean.startswith("-")
            ):
