...
def repurpose_content_tool(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not outputs:
            return {"success": False, "error": "outputs parameter is required (list of formats)"}
            ...
            if output_format == "twitter_thread":
                ...
                for sentence in sentences:
                    if len(current_chunk) + len(sentence) + 20 < chunk_size:
                        ...
                        if current_chunk:
                            tweets.append(
                                f"{tweet_num}/{len(sentences)//3 + 1} {current_chunk.strip()}"
                            )
                            tweet_num += 1
                            current_chunk = sentence + ". "
                        if len(tweets) >= 10:
                ...
                slides = [
                    f"Slide {i+1}: {sentences[i][:100] if i < len(sentences) else 'Content'}"
                    for i in range(8)
                ]
        ...
        return {"success": False, "error": f"Error repurposing content: {str(e)}"}
