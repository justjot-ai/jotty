...
class SkillVenvManager:
    ...
    def create_venv(self, skill_name: Optional[str] = None, python: str = "python3") -> Path:
        ...
        try:
            ...
            result = subprocess.run(
                [python, "-m", "venv", str(venv_path)], capture_output=True, text=True, timeout=60
            )
    ...
    def install_packages(
        self, packages: List[str], skill_name: Optional[str] = None, upgrade: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=600  # 10 minutes for large packages
            )

            if result.returncode != 0:
                return {"success": False, "error": result.stderr, "output": result.stdout}

            return {"success": True, "output": result.stdout, "packages": packages}

        except subprocess.TimeoutExpired:
            return {"success": False, "error": "Installation timeout (exceeded 10 minutes)"}
        except Exception as e:
    ...
    def list_installed_packages(self, skill_name: Optional[str] = None) -> List[str]:
        ...
        try:
            result = subprocess.run(
                [str(pip_path), "list", "--format=json"], capture_output=True, text=True, timeout=30
            )
    ...
    def execute_in_venv(
        self, command: List[str], skill_name: Optional[str] = None, cwd: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not venv_path.exists():
            return {"success": False, "error": f"Venv not found: {venv_path}"}
...
def get_venv_manager(
    venv_base_dir: Optional[str] = None, shared_venv: bool = True
) -> SkillVenvManager:
