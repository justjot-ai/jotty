...
def convert_with_pandoc(
    input_file: Path, output_file: Path, title: str, author: str = "NotebookLM PDF Generator"
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": f"Pandoc conversion error: {str(e)}"}
...
async def generate_pdf_with_notebooklm(
    ...
    if not profile_dir:
        profile_dir = os.getenv(
            "NOTEBOOKLM_USER_DATA_DIR", str(Path.home() / ".notebooklm_browser")
        )
    ...
    try:
        async with async_playwright() as p:
            ...
            if not content_added:
                await browser.close()
                return {"success": False, "error": "Could not add content to notebook"}
            ...
            if pdf_exported and output_file.exists():
                ...
                return {"success": False, "error": "Could not export PDF from NotebookLM"}
...
Examples:
  ...
  python3 scripts/notebooklm_pdf.py --file doc.md --pandoc-only
    ...
    parser.add_argument(
        "--profile-dir", help="Browser profile directory (default: ~/.notebooklm_browser)"
    )
    parser.add_argument(
        "--headless",
        action="store_true",
    ...
    parser.add_argument(
        "--pandoc-only", action="store_true", help="Use Pandoc only, skip NotebookLM"
    )
