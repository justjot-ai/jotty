...
def infer_param_type(param_name: str, default_value=None) -> Optional[str]:
    ...
    if param_name.endswith('_id') or param_name.endswith('_name'):
        return 'str'
    if param_name.startswith('is_') or param_name.startswith('has_') or param_name.startswith('should_'):
        return 'bool'
    if param_name.endswith('_count') or param_name == 'count' or param_name == 'limit' or param_name == 'max':
        return 'int'
    ...
    if func_node.returns is None:
        ...
        if inferred_return:
            ...
            if ':' in func_line and '->' not in func_line:
                ...
                # Handle type imports
                needs_import = inferred_return in ('Dict', 'List', 'Tuple', 'Set', 'Any', 'Optional')
