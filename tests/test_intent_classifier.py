...
import pytest
from Jotty.core.execution.intent_classifier import (
    IntentClassifier,
    TaskIntent,
...
class TestIntentClassifier:
        ...
        for question, expected_tools in test_cases:
            ...
            for tool in expected_tools:
                assert tool in detected, \
                    f"Expected {tool} in tools for: {question}"
    ...
    def test_attachment_tool_detection(self):
        ...
        # Audio attachment
        result = classifier._auto_detect_tools("Process this file", ["audio.mp3"], TaskIntent.FACT_RETRIEVAL)
        assert 'whisper' in result

        # PDF attachment
        result = classifier._auto_detect_tools("Read this document", ["doc.pdf"], TaskIntent.FACT_RETRIEVAL)
        assert 'document-reader' in result

        # Image attachment
        result = classifier._auto_detect_tools("Describe this image", ["image.jpg"], TaskIntent.FACT_RETRIEVAL)
    ...
    def test_multi_step_detection(self):
        ...
        analysis = classifier._heuristic_fallback(
            "What is the capital of the country where Rockhopper penguins live?",
            None
        )
