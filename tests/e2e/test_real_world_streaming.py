...
async def main():
    ...
    print(
        f'    {"🔴" if fast_path else "🟢"} Fast path (direct LLM): {"YES — bypassed pipeline" if fast_path else "NO — used agent pipeline"}'
    )
    print(
        f'    {"🟢" if used_architect else "🔴"} Architect (planning):  {"YES" if used_architect else "NO — skipped"}'
    )
    print(
        f'    {"🟢" if used_auditor else "🔴"} Auditor (validation):  {"YES" if used_auditor else "NO — skipped"}'
    )
    print(
        f'    {"🟢" if used_tools else "🔴"} Tool/skill use:        {"YES" if used_tools else "NO — no tools invoked"}'
    )
