...
class Score:
    ...
    def weighted(self) -> float:
        if not self.success:
            return max(0.05, self.quality * 0.1)
...
def result(s):
    ...
    print(
        f"  {st}  Grade: {clr}{s.grade}{C.E}  Quality: {s.quality:.0%}  Latency: {s.latency:.1f}s"
    )
    if s.error:
        print(f"  {C.R}Error: {s.error[:150]}{C.E}")
...
async def test_4_real_llm() -> Score:
    ...
    try:
        ...
        # Test 1: factual
        r1 = qa(
            question="Name the 3 most popular Python web frameworks in 2025. Just names, comma-separated."
        )
        text1 = r1.answer
        ...
        q2 = (
            ...
            if len(text2) > 20
            and (
                "test" in text2.lower() or "decouple" in text2.lower() or "depend" in text2.lower()
            )
            else 0.5
        )
        sc.quality = (q1 + q2) / 2
...
async def test_6_learning() -> Score:
    ...
    try:
        ...
        w = sm.credit_weights
        weights_ok = w.get("base_reward") is not None and w.get("cooperation_bonus") is not None
        ...
        sc.notes = {
            ...
            "weights": {
                k: f"{w.get(k):.3f}"
                for k in ["base_reward", "cooperation_bonus", "predictability_bonus"]
            },
    ...
    try:
        ...
        results = {
            ...
            "reviewer": EpisodeResult(
                output="Approved with 2 comments",
                success=True,
                trajectory=[{"s": 3}],
                tagged_outputs=[],
                episode=1,
                ...
                architect_results=[],
                auditor_results=[],
                agent_contributions={"reviewer": 0.7},
            ),
        }
...
async def test_8_full_pipeline() -> Score:
    ...
    try:
        ...
        has_keywords = any(
            kw in output.lower() for kw in ["sql", "nosql", "schema", "relational", "document"]
        )
...
async def main():
    ...
    tests = [
        (
            1,
            "Init Speed & Lazy Loading",
            "Is init < 500ms? Are 23 components truly lazy?",
            test_1_init_speed,
        ),
        (
            2,
            "Lifecycle Management",
            "startup → run → shutdown via async context mgr",
            test_2_lifecycle,
        ),
        (
            3,
            "Intent Parsing (NL→Graph)",
            "Parse 3 complex prompts into structured tasks",
            test_3_intent_parsing,
        ),
        (
            4,
            "Real LLM Call (DSPy+Claude)",
            "2 real Claude calls through DSPy integration",
            test_4_real_llm,
        ),
        (
            5,
            "Swarm Intelligence",
            "Stigmergy, Byzantine, Benchmarks, Persistence",
            test_5_swarm_intelligence,
        ),
        (6, "Learning Pipeline", "Episode tracking, credit weights, intelligence", test_6_learning),
        (
            7,
            "Multi-Agent Coordination",
            "Task board, parallel dispatch, result aggregation",
            test_7_multi_agent,
        ),
        (
            8,
            "Full Pipeline (Real E2E)",
            "Orchestrator.run() with real LLM end-to-end",
    ...
    print(f"\n  {'─'*65}")
    print(
        f"  {C.B}{'OVERALL':<38} {'':>5} {overall:>5.0%} {total_lat:>8.1f}s  {passed}/{len(scores)}{C.E}"
    )
    ...
    if scores[2].success:
        strengths.append(
            "Smart intent parsing: NL → structured task graphs with operations/integrations"
        )
    if scores[3].success:
        strengths.append("LLM integration solid: DSPy + Claude Sonnet works end-to-end")
    if scores[4].success:
        strengths.append(
            "Swarm intelligence: stigmergy, byzantine fault tolerance, benchmarks all functional"
        )
    if scores[5].success:
        strengths.append(
            "Real learning: episode tracking, adaptive credit weights, transfer learning"
        )
    if scores[6].success:
        strengths.append(
            "Multi-agent: parallel task dispatch, result aggregation, trajectory merging"
        )
    if scores[7].success:
        strengths.append(
            f"Full pipeline: Orchestrator.run() delivers real output in {scores[7].latency:.0f}s"
        )
    for s in strengths:
        print(f"  {C.G}  + {s}{C.E}")
    ...
    if scores[7].success and scores[7].latency > 60:
        weaknesses.append(
            f"Full pipeline latency is HIGH ({scores[7].latency:.0f}s) — architect/auditor overhead"
        )
    if scores[7].success and scores[7].latency > 30:
        weaknesses.append(
            "Single simple Q&A task shouldn't need full architect→actor→auditor pipeline"
        )
    weaknesses.append(
        "Ensemble auto-detection was too aggressive (fixed: now respects explicit flag)"
    )
    weaknesses.append(
        "Web scraping skills get 403 errors on Reddit/Medium (needs proxy or fallback)"
    )
    if not weaknesses:
