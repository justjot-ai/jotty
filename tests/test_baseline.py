...
class TestMemoryImports:
    ...
    def test_can_import_consolidation_engine_components(self):
        ...
        from core.memory import BrainMode, HippocampalExtractor, SharpWaveRippleConsolidator
...
class TestPlanDecomposition:
    ...
    def test_extract_comparison_entities_three_way(self):
        ...
        planner = TaskPlanner()
        entities = planner._extract_comparison_entities("Compare React vs Vue vs Angular")
    ...
    def test_extract_comparison_entities_no_comparison(self):
        ...
        planner = TaskPlanner()
        entities = planner._extract_comparison_entities("Research AI trends and create a PDF")
        assert entities == []
    ...
    def test_maybe_decompose_plan_comparison_with_composite(self):
        ...
        skills = [
            ...
            {"name": "telegram-sender", "tools": [{"name": "send_telegram_file_tool"}]},
            {
                ...
                "tools": [{"name": "search_summarize_pdf_telegram_tool"}],
            },
        ]
        task = (
            "Research Paytm vs PhonePe payment gateway comparison, create PDF and send via Telegram"
        )
    ...
    def test_maybe_decompose_plan_already_multi_step(self):
        ...
        steps = [
            ExecutionStep(
                skill_name="web-search", tool_name="search_web_tool", params={}, description="s1"
            ),
            ExecutionStep(
