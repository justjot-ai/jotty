...
async def test_markdown_generation():
    ...
    agents = [
        AgentConfig(name="analyzer", agent=SimpleAgent1(), outputs=["output"]),
        AgentConfig(name="generator", agent=SimpleAgent2(), parameter_mappings={"input": "output"}),
    ]
    ...
    result = await pipeline.optimize(
        task="Generate markdown", context={}, gold_standard="Agent2: Agent1: Generate markdown"
    )
...
async def test_mermaid_generation():
    ...
    pipeline = create_optimization_pipeline(
        agents=agents, max_iterations=2, required_pass_count=1, output_path="./test_outputs/mermaid"
    )
    pipeline.config.evaluation_function = evaluate
...
async def test_with_teacher():
    ...
    agents = [AgentConfig(name="student", agent=StudentAgent(), outputs=["output"])]

    # Teacher agent is separate (not in main pipeline)
    teacher_agent_config = AgentConfig(
        name="teacher", agent=TeacherAgent(), metadata={"is_teacher": True}
    )
    ...
    result = await pipeline.optimize(
        task="Solve problem", context={}, gold_standard="Correct answer"
    )
    ...
    for it in result["iterations"]:
        print(
            f"  Iteration {it['iteration']}: score={it['evaluation_score']}, teacher={it.get('has_teacher_output', False)}"
        )
