...
class TestClaudeAPIIntegration:
    ...
    def test_simple_agent_with_claude(self):
        ...
        config = SwarmConfig(
            actors=[agent],
            max_rounds=1,
            enable_learning=False  # Disable learning for simple test
        )
    ...
    def test_multi_agent_parameter_resolution(self):
        ...
        # Create swarm
        config = SwarmConfig(
            actors=[topic_agent, insight_agent],
            max_rounds=2,
    ...
    def test_refactored_components_work_together(self):
        ...
        lm = dspy.LM(
            model='anthropic/claude-3-5-haiku-20241022',  # Use Haiku for faster test
            max_tokens=200
        )
        dspy.configure(lm=lm)
