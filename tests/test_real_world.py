...
async def test_comparison_report():
    ...
    result = await sm.run(
        ...
        ensemble=False,
        status_callback=lambda stage, detail='': trail.append((time.time()-t0, stage, detail[:90])),
    )
    ...
    checks = {
        ...
        'Pricing': any(w in output.lower() for w in ['price', 'pricing', '$', 'free', '/month', 'plan']),
        'Recommendation': any(w in output.lower() for w in ['recommend', 'best', 'choose', 'suggest', 'winner']),
    }
...
async def test_cap_theorem():
    ...
    checks = {
        'CAP_explained': any(w in output.lower() for w in ['consistency', 'availability', 'partition']),
        'DB_examples': sum(1 for db in ['mongodb', 'cassandra', 'postgres', 'dynamo', 'redis',
                                         'hbase', 'cockroach', 'spanner', 'zookeeper']
                          if db in output.lower()) >= 2,
        'CP_AP': 'cp' in output.lower() and 'ap' in output.lower(),
        'Decision_matrix': any(w in output.lower() for w in ['matrix', 'decision', 'when to', 'choose', 'trade-off']),
