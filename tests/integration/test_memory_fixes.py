...
async def test_all_fixes():
    ...
    memory = SwarmMemory(agent_name="plantuml_fix_test", config=memory_config)
    ...
    improvements = retrieve_improvements_from_memory(
        memory=memory, expert_name=config.name, domain=config.domain, max_results=10
    )
    ...
    for imp in improvements:
        ...
        if (
            "syntax" in pattern.lower()
            or "plantuml" in pattern.lower()
            or "mermaid" in pattern.lower()
        ):
    ...
    print(
        f"Consolidation: {consolidation_result.get('consolidated', 0) if procedural_count >= 2 else 0} patterns"
    )
