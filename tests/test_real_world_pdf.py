...
async def test_research_to_pdf():
    ...
    content_checks = {
        ...
        'frameworks': sum(1 for fw in ['crewai', 'autogen', 'langgraph', 'jotty', 'swarm']
                         if fw in output.lower()) >= 2,
        'analysis': any(w in output.lower() for w in ['analysis', 'findings', 'research', 'report']),
        'recommendations': any(w in output.lower() for w in ['recommend', 'conclusion', 'outlook', 'future']),
