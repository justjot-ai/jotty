...
async def test_professional_scenarios(use_renderer: bool = True, max_scenarios: int = 10):
    ...
    print("=" * 80)
    print(
        f"PROFESSIONAL MERMAID EXPERT TEST - {min(max_scenarios, len(PROFESSIONAL_SCENARIOS))} SCENARIOS"
    )
    ...
    memory = SwarmMemory(agent_name="mermaid_professional_test", config=memory_config)
        ...
        try:
            diagram = await asyncio.wait_for(
                expert.generate_mermaid(
                    description=scenario["description"], diagram_type=scenario["diagram_type"]
                ),
            ...
            type_match = metadata["diagram_type"] == scenario["diagram_type"] or (
                scenario["diagram_type"] == "flowchart"
                and metadata["diagram_type"] in ["graph", "flowchart"]
            )
            ...
            if scenario["complexity"] == "high":
                has_complexity = (
                    metadata.get("has_subgraphs", False)
                    or metadata.get("has_alt_else", False)
                    or metadata.get("has_parallel", False)
                    or metadata["lines"] > 10
                )
            ...
            result = {
                ...
                "success": is_valid
                and type_match
                and element_coverage >= 0.6
                and (scenario["complexity"] == "medium" or has_complexity),
            ...
            print(
                f"   {status} Valid: {is_valid}, Type: {metadata['diagram_type']} ({'✓' if type_match else '✗'}), Elements: {len(found_elements)}/{len(scenario['expected_elements'])} ({element_coverage:.0%})"
            )
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            f"     Type: {result.get('diagram_type', 'unknown')} ({'✓' if result.get('type_match') else '✗'})"
        )
        if result.get("elements_found"):
            print(
                f"     Elements: {len(result['elements_found'])}/{len(PROFESSIONAL_SCENARIOS[i-1]['expected_elements'])} found"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-renderer", action="store_true", help="Disable renderer validation (faster)"
    )
    parser.add_argument("--max-scenarios", type=int, default=10, help="Maximum scenarios to test")
    args = parser.parse_args()

    results = asyncio.run(
        test_professional_scenarios(
            use_renderer=not args.no_renderer, max_scenarios=args.max_scenarios
        )
    )
    successful = sum(1 for r in results if r.get("success", False))
    total = len(results)
