...
async def test_complex_scenarios():
    ...
    memory = HierarchicalMemory(
        agent_name="mermaid_expert_quick_test",
    ...
    for i, scenario in enumerate(COMPLEX_SCENARIOS, 1):
        ...
        try:
            diagram = await asyncio.wait_for(
                expert.generate_mermaid(description=scenario['description']),
                timeout=30
            )
            ...
            results.append({"scenario": scenario['name'], "valid": False, "error": str(e), "success": False})
