...
from core.monitoring import CostTracker, MonitoringFramework, ExecutionStatus
...
def test_monitoring_disabled():
    ...
    try:
        ...
        assert perf_metrics.total_executions == 0, f"Expected 0 executions, got {perf_metrics.total_executions}"
...
def test_monitoring_enabled():
    ...
    try:
        ...
        perf_metrics = monitor.get_performance_metrics()
        assert perf_metrics.total_executions == 1, f"Expected 1 execution, got {perf_metrics.total_executions}"
        assert perf_metrics.successful_executions == 1, f"Expected 1 success, got {perf_metrics.successful_executions}"
...
def test_config_defaults():
    ...
    try:
        ...
        assert config.enable_cost_tracking == False, f"Expected False, got {config.enable_cost_tracking}"
        assert config.enable_monitoring == False, f"Expected False, got {config.enable_monitoring}"
        assert config.enable_efficiency_metrics == False, f"Expected False, got {config.enable_efficiency_metrics}"
