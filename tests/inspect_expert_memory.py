...
def inspect_memory_for_expert(memory: HierarchicalMemory, expert_name: str, domain: str):
    ...
    expert_procedural = [
        entry for entry in procedural.values()
        if entry.context.get('expert_name') == expert_name or 
           entry.context.get('domain') == domain or
           f"expert_{domain}" in entry.context.get('expert_name', '')
    ]
    
    expert_semantic = [
        entry for entry in semantic.values()
        if entry.context.get('expert_name') == expert_name or 
           entry.context.get('domain') == domain or
           f"expert_{domain}" in entry.context.get('expert_name', '')
    ]
    ...
    expert_meta = [
        entry for entry in meta.values()
        if entry.context.get('expert_name') == expert_name or 
           entry.context.get('domain') == domain or
           f"expert_{domain}" in entry.context.get('expert_name', '')
    ]
...
def main():
    ...
    plantuml_procedural = len([
        e for e in plantuml_memory.memories[MemoryLevel.PROCEDURAL].values()
        if e.context.get('domain') == 'plantuml' or 'plantuml' in str(e.context.get('expert_name', ''))
    ])
    plantuml_semantic = len([
        e for e in plantuml_memory.memories[MemoryLevel.SEMANTIC].values()
        if e.context.get('domain') == 'plantuml' or 'plantuml' in str(e.context.get('expert_name', ''))
    ])
    
    mermaid_procedural = len([
        e for e in mermaid_memory.memories[MemoryLevel.PROCEDURAL].values()
        if e.context.get('domain') == 'mermaid' or 'mermaid' in str(e.context.get('expert_name', ''))
    ])
    mermaid_semantic = len([
        e for e in mermaid_memory.memories[MemoryLevel.SEMANTIC].values()
        if e.context.get('domain') == 'mermaid' or 'mermaid' in str(e.context.get('expert_name', ''))
    ])
