...
async def test_plantuml_expert_comprehensive():
    ...
    memory = SwarmMemory(agent_name="plantuml_expert_test", config=memory_config)
...
@enduml""",
    ...
    if procedural_count_after >= 3:
        ...
        print(
            f"⚠️  Not enough improvements for consolidation (have {procedural_count_after}, need at least 3)"
        )
    ...
    print(f"✅ Improvements in memory: {len(expert.improvements)}")
    print(
        f"✅ Memory levels: {procedural_count} PROCEDURAL, {semantic_count} SEMANTIC, {meta_count} META"
    )
    print()
