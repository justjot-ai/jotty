...
async def test_math_latex_expert():
    ...
    for i, test_case in enumerate(TEST_CASES, 1):
        ...
        try:
            ...
            is_valid, error_msg, metadata = validate_math_latex_syntax(output, use_renderer=True)
            ...
            print(
                f"📊 Elements: {len(found_elements)}/{len(test_case['required_elements'])} ({element_coverage:.0f}%)"
            )
            if found_elements:
            ...
            type_match = (test_case["type"] == "display" and has_display) or (
                test_case["type"] == "inline" and has_inline
            )

            print(f"📋 Type: {test_case['type']} ({'✓' if type_match else '✗'})")
            print(
                f"📋 Delimiters: Display ({'✓' if has_display else '✗'}), Inline ({'✓' if has_inline else '✗'})"
            )
            ...
            if "must be trained" in str(e):
                ...
                results.append(
                    {"case": test_case["name"], "status": "error", "error": "Expert not trained"}
                )
            else:
                print(f"❌ Error: {e}")
