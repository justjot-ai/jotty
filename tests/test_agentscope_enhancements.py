...
class TestFeedbackChannelRequest:
    ...
    async def test_request_with_reply(self):
        from core.agents.feedback_channel import (
...
class TestSmartAgentSlackBroadcast:
    ...
    def test_broadcast_targets_all_except_sender(self):
        from core.agents.axon import SmartAgentSlack, AgentCapabilities

        slack = SmartAgentSlack()

        # Register agents with proper AgentCapabilities objects
        for name in ['agent_a', 'agent_b', 'agent_c']:
            slack.register_agent(name, capabilities=AgentCapabilities(
                agent_name=name, preferred_format='text',
                acceptable_formats=['text'], max_input_size=16000,
...
class TestGetScopedTools:
    ...
    def test_scoped_tools_names_format(self):
        ...
        registry = get_unified_registry()
        names = registry.get_scoped_tools(
            "create a chart", max_tools=8, format='names'
        )
        assert isinstance(names, list)
        for n in names:
...
class TestPipelineUtils:
    ...
    async def test_sequential_pipeline_chains(self):
        ...
        def _make_result(**overrides):
            defaults = dict(
                output="", success=True, trajectory=[], tagged_outputs=[],
                episode=0, execution_time=0.1,
