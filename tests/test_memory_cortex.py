...
class TestRetrievalMixinAdvanced:
    ...
    def test_retrieve_by_task_type(self, minimal_jotty_config):
        ...
        memory.store("date filter pattern", MemoryLevel.SEMANTIC, {},
                      "test", domain="sql", task_type="date_filter")
        memory.store("join pattern", MemoryLevel.SEMANTIC, {},
                      "test", domain="sql", task_type="join_query")
...
class TestSerializationAdvanced:
    ...
    def test_to_dict_memory_entry_fields(self, minimal_jotty_config):
        ...
        expected_fields = [
            'key', 'content', 'level', 'context', 'created_at', 'last_accessed',
            'goal_values', 'default_value', 'access_count', 'ucb_visits',
            'token_count', 'is_protected', 'protection_reason', 'causal_links', 'metadata'
        ]
...
class TestStatisticsAdvanced:
    ...
    def test_consolidated_knowledge_includes_all_sections(self, minimal_jotty_config):
        ...
        memory.causal_links["cl1"] = CausalLink(
            cause="action A", effect="result B", confidence=0.8
        )
