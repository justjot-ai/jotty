...
COMPLEX_SCENARIOS = [
    ...
    },
    {
        ...
        "expected_elements": ["Pending", "Payment", "Processing", "Shipped", "Delivered", "Cancelled"]
    },
    {
    ...
    },
    {
        ...
        "expected_elements": ["API Gateway", "Auth Service", "User Service", "Order Service", "Payment Service"]
    },
    {
    ...
    },
    {
        ...
        "expected_elements": ["Premium Customer", "VIP Support", "Priority Queue", "Standard Queue"]
    },
    {
...
async def test_complex_scenarios():
    ...
    memory = HierarchicalMemory(
        agent_name="mermaid_expert_complex_test",
    ...
    for i, scenario in enumerate(COMPLEX_SCENARIOS, 1):
        ...
        try:
            ...
            # Print result
            if result['success']:
                print(f"   ✅ SUCCESS - Valid syntax, {len(found_elements)}/{len(scenario['expected_elements'])} elements found")
            else:
                print(f"   ⚠️  PARTIAL - Valid: {is_valid}, Elements: {len(found_elements)}/{len(scenario['expected_elements'])}")
                if not is_valid:
    ...
    if len(expert.improvements) > 0:
        ...
        print(f"   Sample improvement: {expert.improvements[0].get('learned_pattern', 'N/A')[:80]}...")
    else:
