...
def sync_and_show_memory(expert_name: str, domain: str, improvements: list):
    ...
    memory = SwarmMemory(agent_name=f"{expert_name}_memory", config=memory_config)
    ...
    synced_count = sync_improvements_to_memory(
        memory=memory, improvements=improvements, expert_name=expert_name, domain=domain
    )
...
def main():
    ...
    if plantuml_improvements:
        plantuml_memory = sync_and_show_memory(
            "plantuml_expert_test", "plantuml", plantuml_improvements
        )
    ...
    if mermaid_improvements:
        mermaid_memory = sync_and_show_memory(
            "mermaid_expert_test", "mermaid", mermaid_improvements
        )
