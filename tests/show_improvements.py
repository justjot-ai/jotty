...
async def show_improvements():
    ...
    agents = [
        AgentConfig(name="main_agent", agent=improving_agent, outputs=["output"]),
    ...
    result = await pipeline.optimize(
        task="Generate correct answer", context={}, gold_standard=gold_standard
    )
    ...
    if result.get("final_result"):
        ...
        if result["iterations"]:
            ...
            print()
            print(
                f"📈 IMPROVEMENT: Score improved from {first_score:.2f} to {final_score:.2f} (+{final_score - first_score:.2f})"
            )
